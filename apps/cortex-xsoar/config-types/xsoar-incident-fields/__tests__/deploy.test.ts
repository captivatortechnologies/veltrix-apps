...
describe('Cortex XSOAR Incident Fields Deploy Handler', () => {
  ...
  it('updates an existing field in place, carrying its live version forward', async () => {
    await withMockFetch(
      [
        { status: 200, body: JSON.stringify([{ id: 'incident_sourceip', version: 4 }]) },
        { status: 200, body: '{}' },
      ],
      async () => {
        const result = await deploy(
        ...
        expect(result.success).toBe(true)
        expect(result.message).toContain('Deployed 1 incident field(s)')
      },
    )
  })
  ...
  it('refuses to modify a built-in/locked field', async () => {
    await withMockFetch(
      [{ status: 200, body: JSON.stringify([{ id: 'incident_sourceip', system: true }]) }],
      async () => {
        const result = await deploy(
          makeCtx([{ name: 's1', fields: { cliName: 'sourceip', name: 'Source IP', type: 'shortText' } }]),
        )
  ...
  it('reports a clear error when no credential is available', async () => {
    const ctx = makeCtx([{ name: 's1', fields: { cliName: 'sourceip', name: 'Source IP', type: 'shortText' } }])
    ctx.credential = null
