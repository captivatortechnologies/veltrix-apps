...
import type { IncidentFieldRollbackEntry } from './deploy'

/**
 * Roll back incident fields using the state captured during deploy:
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  const previousState = (ctx.rollbackData as { previousState?: IncidentFieldRollbackEntry[] })?.previousState
  ...
  try {
    ...
    return { success: true, message: `Rolled back ${reverted.length} incident field(s): ${reverted.join(', ')}` }
  } catch (error) {
    return {
      success: false,
