...
/**
 * Health check for job configuration:
 *   1. XSOAR API reachability + credential validity (a job search)
 *   2. Every declared job still exists on the server
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed && reachable.live) {
    const names = new Set(reachable.live.filter((j) => j.name).map((j) => j.name as string))
    for (const spec of specs) {
      const present = names.has(spec.name)
      checks.push({
        name: `job:${spec.name}`,
        passed: present,
        message: present ? `Job "${spec.name}" is present` : `Job "${spec.name}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: LiveJob[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: LiveJob[] }> {
