...
/**
 * Health for the teams config = Sysdig Secure answers on its REST API with the
 * configured Bearer token. Read-only probe of the exact API surface this
 * config type writes to: GET /api/teams.
 */
  ...
  try {
    ...
    if (res.status === 401 || res.status === 403) {
      ...
      checks.push({
        ...
        passed,
        message: passed ? `Sysdig Secure teams API reachable (HTTP ${res.status}).` : `Sysdig Secure returned HTTP ${res.status}.`,
        latencyMs,
      })
    }
  } catch (error) {
