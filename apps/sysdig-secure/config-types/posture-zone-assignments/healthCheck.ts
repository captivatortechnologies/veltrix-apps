...
/**
 * Health for the posture-zone-assignments config = Sysdig Secure answers on
 * its REST API with the configured Bearer token. Read-only probe of the
 * Zones API this config type depends on: GET /platform/v1/zones.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.request('GET', '/platform/v1/zones')
    ...
    if (res.status === 401 || res.status === 403) {
      ...
      checks.push({
        ...
        passed,
        message: passed ? `Sysdig Secure zones API reachable (HTTP ${res.status}).` : `Sysdig Secure returned HTTP ${res.status}.`,
        latencyMs,
      })
    }
  } catch (error) {
