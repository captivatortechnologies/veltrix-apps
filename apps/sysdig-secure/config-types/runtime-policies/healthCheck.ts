...
/**
 * Health for the runtime-policies config = Sysdig Secure answers on its policies
 * REST API with the configured Bearer token. Read-only probe of the exact API
 * surface this config type writes to: GET /api/v2/policies. A 2xx confirms the
 * token authenticates and the policies API is reachable; 401/403 prove
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.request('GET', '/api/v2/policies')
    ...
    if (res.status === 401 || res.status === 403) {
      ...
      checks.push({
        ...
        passed,
        message: passed ? `Sysdig Secure policies API reachable (HTTP ${res.status}).` : `Sysdig Secure returned HTTP ${res.status}.`,
        latencyMs,
      })
    }
  } catch (error) {
