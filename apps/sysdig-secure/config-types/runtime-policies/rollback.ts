...
/**
 * Undo a runtime-policies deploy from rollbackData.previous (written by
 * deploy()). Per entry, reverse the action taken:
 *   created → DELETE the policy we added
 *   updated → PUT the prior policy body back onto the same id (restore)
 *   deleted → POST the prior policy body to re-create it (a new id is assigned)
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
      ...
      switch (entry.action) {
        case 'created':
          if (entry.policyId != null) {
            await client.deletePolicy(entry.policyId)
            removed++
          } else {
            skipped++
          }
          break
        case 'updated':
          if (entry.policyId != null && entry.prior) {
        ...
        case 'deleted':
          if (entry.prior) {
            ...
            await client.createPolicy(body as SysdigPolicy)
            recreated++
          } else {
            skipped++
          }
    ...
    return {
      success: true,
      message: `Rolled back runtime policies: ${restored} restored, ${recreated} re-created, ${removed} removed${skipped ? `, ${skipped} skipped` : ''}.`,
    }
  } catch (error) {
