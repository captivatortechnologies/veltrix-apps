...
/**
 * What this app manages in Sysdig Secure, rendered with the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/sysdig-secure/meta')
  ...
  if (loading) return <Spinner label="Loading Sysdig Secure app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Sysdig Secure custom Falco (threat-detection) rules as code. Author rules in the
          Configuration Canvas and deploy them through the pipeline — validate, deploy, health check,
          drift detection and rollback are handled per configuration type. Rules are applied over the
          Sysdig Secure REST API (<code>/api/secure/rules</code>) with a Bearer API token.
        </p>
