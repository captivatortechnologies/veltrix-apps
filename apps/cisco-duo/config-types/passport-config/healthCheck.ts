...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.getV5(PATH)
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'duo-admin-passport-config',
    passed,
    message: passed ? 'Reached the Duo Admin API Passport config endpoint' : `Duo error: ${duoErrorMessage(resp)}`,
    latencyMs,
  })
