...
**App:** `cisco-duo` · **Category:** IAM · **Version:** 0.5.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Cisco Duo API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Cisco Duo API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + groupswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + groupsread
  rbac-->>pipeline: permitted ✓ / denied
