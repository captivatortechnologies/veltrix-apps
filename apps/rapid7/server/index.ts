// ========================================================================
// Rapid7 InsightVM App — Server Entry Module
//
// Registers InsightVM-specific API routes as a Fastify plugin, prefixed with
// /api/apps/rapid7/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration
// Canvas, and every write goes through the pipeline handlers (InsightVM v3 API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('insightvm-sites', 'read')],
