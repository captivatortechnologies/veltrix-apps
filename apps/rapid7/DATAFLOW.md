# 🎯 Rapid7 InsightVM — Request Flows
...
**App:** `rapid7` · **Category:** COMPLIANCE · **Version:** 1.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Rapid7 InsightVM API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Rapid7 InsightVM API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + insightvm-siteswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + insightvm-sitesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: insightvm-siteswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
