...
/**
 * Shows what this app manages in a Microsoft Entra tenant, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/microsoft-entra-id/meta')
  ...
  if (loading) return <Spinner label="Loading Microsoft Entra ID app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Microsoft Entra ID (formerly Azure AD) configuration as code through the Microsoft
          Graph API. Create a configuration in the Configuration Canvas and deploy it through the
          pipeline — validate, deploy, health check, drift detection, and rollback are all handled
          per configuration type. Conditional Access policies deploy as report-only by default, so a
          new policy is never enforced until you review its impact and enable it.
        </p>
