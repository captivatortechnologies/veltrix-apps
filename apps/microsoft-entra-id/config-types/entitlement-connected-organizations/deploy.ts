...
import { extractConnectedOrgSpecs, parseArray, type ConnectedOrgSpec, type LiveConnectedOrg } from './validate'
...
export function buildBody(spec: ConnectedOrgSpec): Record<string, unknown> {
  return {
    displayName: spec.name,
    description: spec.description || '',
    state: spec.state,
    identitySources: parseArray(spec.identitySources) ?? [],
  }
}

function snapshotLive(live: LiveConnectedOrg): Record<string, unknown> {
  return {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractConnectedOrgSpecs(ctx.canvas).filter((s) => s.name)

  const listed = await client.getAll<LiveConnectedOrg>(`${BASE}${SELECT}`)
  if (!listed.ok) {
    return { success: false, message: `Failed to list connected organizations: ${graphErrorMessage(listed.lastError!)}` }
  }
  const liveByName = new Map<string, LiveConnectedOrg>()
  const liveById = new Map<string, LiveConnectedOrg>()
  for (const o of listed.items) {
    if (o.displayName) liveByName.set(o.displayName.toLowerCase(), o)
    if (o.id) liveById.set(o.id, o)
  }
  ...
  for (const spec of specs) {
    ...
    if (liveMatch?.id) {
      ...
      const created = parseJson<LiveConnectedOrg>(resp.body)
  ...
  if (failures.length) {
    return { success: false, message: `Some connected organizations failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} connected organization(s)`, rollbackData: { entries } }
}
