...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  // Note: the client secret is write-only, so a restored provider keeps whatever
  // secret was last applied — only displayName / clientId are rolled back.
  ...
  return { success: true, message: `Rolled back identity providers: ${deleted} deleted, ${restored} restored` }
}
