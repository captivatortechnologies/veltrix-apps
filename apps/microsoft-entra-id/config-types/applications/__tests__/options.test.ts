...
describe('applications options', () => {
  it('routes "ownerPrincipals" to the merged users+servicePrincipals picker', async () => {
    mockGraphFetch((u) => {
    ...
    const opts = await applicationOptions(baseCtx)
    expect(opts).toEqual([
  ...
  it('routes every other source straight through to entraOptions', async () => {
    ...
    const opts = await applicationOptions({ ...baseCtx, source: 'not-a-real-source' })
