...
import { extractActivityTimeoutSpecs, type ActivityTimeoutSpec, type LiveActivityTimeoutPolicy } from './validate'
...
export function buildBody(spec: ActivityTimeoutSpec): Record<string, unknown> {
  return {
...
function snapshotLive(live: LiveActivityTimeoutPolicy): Record<string, unknown> {
  return {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractActivityTimeoutSpecs(ctx.canvas).filter((s) => s.name)

  const listed = await client.getAll<LiveActivityTimeoutPolicy>(`${BASE}${SELECT}`)
  if (!listed.ok) {
    return { success: false, message: `Failed to list activity based timeout policies: ${graphErrorMessage(listed.lastError!)}` }
  }
  const liveByName = new Map<string, LiveActivityTimeoutPolicy>()
  const liveById = new Map<string, LiveActivityTimeoutPolicy>()
  ...
  for (const spec of specs) {
    ...
    if (liveMatch?.id) {
      ...
      const created = parseJson<LiveActivityTimeoutPolicy>(resp.body)
  ...
  if (failures.length) {
    return {
      success: false,
      message: `Some activity based timeout policies failed: ${failures.join('; ')}`,
      rollbackData: { entries },
    }
  }
  return {
    success: true,
    message: `Deployed ${entries.length} activity based timeout policy(ies)`,
    rollbackData: { entries },
  }
}
