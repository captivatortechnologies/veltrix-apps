...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      // We updated this one — restore its prior PATCH-able metadata.
  ...
  return { success: true, message: `Rolled back terms of use agreements: ${deleted} deleted, ${restored} restored` }
}
