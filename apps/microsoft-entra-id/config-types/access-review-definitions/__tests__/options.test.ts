import accessReviewOptions from '../options'
import entraOptions from '../../lib/entraOptions'

describe('access-review-definitions options', () => {
  it('is a direct re-export of the shared entraOptions provider', () => {
    expect(accessReviewOptions).toBe(entraOptions)
  })

  it('serves the "accessPackages" source (scopeAccessPackageId picker)', async () => {
    ...
    const opts = await accessReviewOptions({
      appId: 'microsoft-entra-id',
      customerId: 'cust-1',
      configTypeId: 'access-review-definitions',
