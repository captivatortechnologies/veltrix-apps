...
// --- Entra token-issuance-policy constraints ---------------------------------

export const MAX_DISPLAY_NAME_LENGTH = 256
/** The expected root key of a token issuance policy definition object. */
export const DEFINITION_ROOT_KEY = 'TokenIssuancePolicy'

export interface TokenIssuanceSpec {
  itemId?: string
  ...
  definition: string
  /** Application object ids, names or ids to assign this policy to — resolved at deploy time. */
  appliesTo: string[]
}

/** A token issuance policy as returned by Graph. */
export interface LiveTokenIssuancePolicy {
  id?: string
  displayName?: string
  definition?: string[]
}
...
export function extractTokenIssuanceSpecs(canvas: CanvasSnapshot): TokenIssuanceSpec[] {
...
export default function validate(ctx: PipelineContext): ValidationResult {
  ...
  const specs = extractTokenIssuanceSpecs(ctx.canvas)
  ...
  specs.forEach((spec, i) => {
    ...
    if (!spec.name) {
      ...
      if (seenNames.has(key)) {
        errors.push({
          field: `${prefix}.name`,
          message: `Duplicate token issuance policy "${spec.name}" — each may only be declared once per canvas`,
          code: 'duplicate_name',
        })
      }
      seenNames.add(key)
