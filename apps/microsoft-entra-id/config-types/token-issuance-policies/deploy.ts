...
import { extractTokenIssuanceSpecs, type TokenIssuanceSpec, type LiveTokenIssuancePolicy } from './validate'
import {
...
/** tokenIssuancePolicy is assignable to APPLICATIONS ONLY — verified directly
 *  (see config-types/lib/policyAppliesTo.ts header: `GET
 *  serviceprincipal-post-tokenissuancepolicies` 404s; `POST
 *  /applications/{id}/tokenIssuancePolicies/$ref` is the real, documented
 *  operation). */
...
export function buildBody(spec: TokenIssuanceSpec): Record<string, unknown> {
  return {
...
function snapshotLive(live: LiveTokenIssuancePolicy): Record<string, unknown> {
  return {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractTokenIssuanceSpecs(ctx.canvas).filter((s) => s.name)

  const listed = await client.getAll<LiveTokenIssuancePolicy>(`${BASE}${SELECT}`)
  if (!listed.ok) {
    return { success: false, message: `Failed to list token issuance policies: ${graphErrorMessage(listed.lastError!)}` }
  }
  const liveByName = new Map<string, LiveTokenIssuancePolicy>()
  const liveById = new Map<string, LiveTokenIssuancePolicy>()
  ...
  for (const spec of specs) {
    ...
    if (liveMatch?.id) {
      ...
      const created = parseJson<LiveTokenIssuancePolicy>(resp.body)
      policyId = created?.id
  ...
  if (failures.length) {
    return {
      success: false,
      message: `Some token issuance policies failed: ${failures.join('; ')}`,
      rollbackData: { entries },
    }
  }
  return {
    success: true,
    message: `Deployed ${entries.length} token issuance policy(ies)`,
    rollbackData: { entries },
  }
}
