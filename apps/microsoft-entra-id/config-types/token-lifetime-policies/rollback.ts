...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    for (const a of e.appliesTo ?? []) {
      ...
      const resp = await client.delete(`/servicePrincipals/${a.id}/tokenLifetimePolicies/${e.id}/$ref`)
  ...
  return {
    success: true,
    message: `Rolled back token lifetime policies: ${deleted} deleted, ${restored} restored, ${unassigned} assignment(s) removed`,
  }
}
