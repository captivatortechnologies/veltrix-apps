import homeRealmDiscoveryPolicyOptions from '../options'
...
describe('home-realm-discovery-policies options', () => {
  it('is a direct re-export of the shared entraOptions provider', () => {
    expect(homeRealmDiscoveryPolicyOptions).toBe(entraOptions)
  })
  ...
  it('serves the "servicePrincipals" source for the appliesTo field', async () => {
    ...
    const opts = await homeRealmDiscoveryPolicyOptions({
      appId: 'microsoft-entra-id',
      customerId: 'cust-1',
      configTypeId: 'home-realm-discovery-policies',
