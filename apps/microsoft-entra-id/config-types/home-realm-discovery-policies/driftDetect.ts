...
import { canonicalDefinition, extractHomeRealmSpecs, type LiveHomeRealmPolicy } from './validate'
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  const specs = extractHomeRealmSpecs(ctx.deployedConfig).filter((s) => s.name)
