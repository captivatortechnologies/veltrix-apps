...
import { extractHomeRealmSpecs, type HomeRealmSpec, type LiveHomeRealmPolicy } from './validate'
import {
...
const POLICY_TYPE_NAME = 'homeRealmDiscoveryPolicies'
/** homeRealmDiscoveryPolicy is assignable to service principals ONLY
...
export function buildBody(spec: HomeRealmSpec): Record<string, unknown> {
  return {
...
function snapshotLive(live: LiveHomeRealmPolicy): Record<string, unknown> {
  return {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractHomeRealmSpecs(ctx.canvas).filter((s) => s.name)

  const listed = await client.getAll<LiveHomeRealmPolicy>(`${BASE}${SELECT}`)
  if (!listed.ok) {
    return { success: false, message: `Failed to list home realm discovery policies: ${graphErrorMessage(listed.lastError!)}` }
  }
  const liveByName = new Map<string, LiveHomeRealmPolicy>()
  const liveById = new Map<string, LiveHomeRealmPolicy>()
  ...
  for (const spec of specs) {
    ...
    if (liveMatch?.id) {
      ...
      const created = parseJson<LiveHomeRealmPolicy>(resp.body)
      policyId = created?.id
  ...
  if (failures.length) {
    return {
      success: false,
      message: `Some home realm discovery policies failed: ${failures.join('; ')}`,
      rollbackData: { entries },
    }
  }
  return {
    success: true,
    message: `Deployed ${entries.length} home realm discovery policy(ies)`,
    rollbackData: { entries },
  }
}
