...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    for (const a of e.appliesTo ?? []) {
      ...
      const resp = await client.delete(`/servicePrincipals/${a.id}/homeRealmDiscoveryPolicies/${e.id}/$ref`)
  ...
  return {
    success: true,
    message: `Rolled back home realm discovery policies: ${deleted} deleted, ${restored} restored, ${unassigned} assignment(s) removed`,
  }
}
