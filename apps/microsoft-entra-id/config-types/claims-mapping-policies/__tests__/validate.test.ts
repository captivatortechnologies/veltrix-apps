...
const VALID_DEF = '{"ClaimsMappingPolicy":{"Version":1,"IncludeBasicClaimSet":"true","ClaimsSchema":[]}}'
...
describe('claims-mapping validate', () => {
  it('accepts a valid policy', () => {
    const r = validate(ctxWith([{ name: 'EmployeeID', fields: { name: 'EmployeeID', definition: VALID_DEF } }]))
  ...
  it('rejects invalid JSON', () => {
    const r = validate(ctxWith([{ name: 'P', fields: { name: 'P', definition: 'nope' } }]))
    expect(r.valid).toBe(false)
