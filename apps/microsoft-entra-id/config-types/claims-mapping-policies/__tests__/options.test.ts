import claimsMappingPolicyOptions from '../options'
...
describe('claims-mapping-policies options', () => {
  it('is a direct re-export of the shared entraOptions provider', () => {
    expect(claimsMappingPolicyOptions).toBe(entraOptions)
  })
  ...
  it('serves the "servicePrincipals" source for the appliesTo field', async () => {
    ...
    const opts = await claimsMappingPolicyOptions({
      appId: 'microsoft-entra-id',
      customerId: 'cust-1',
      configTypeId: 'claims-mapping-policies',
