# 🔷 Microsoft Entra ID — Request Flows
...
**App:** `microsoft-entra-id` · **Category:** IAM · **Version:** 0.8.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Microsoft Entra ID API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 42 config types · 10 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Microsoft Entra ID API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + named-locationswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + named-locationsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: named-locationswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
