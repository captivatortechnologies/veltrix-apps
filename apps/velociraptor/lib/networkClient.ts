...
//
// Thin HTTP client for the GENERIC platform IPAM service, which owns the shared
// per-(provider, region) Network, the per-customer CIDR blocks, and the GLOBAL
// subnet ledger. The app never allocates CIDRs itself — overlap detection must be
// global across every tenant and app, so it lives in the platform. This module
// only speaks its two-endpoint contract:
//
...
//
// Configuration is env-driven: the base URL of the platform's internal API is
// read from VELTRIX_NETWORK_API_URL, with an optional service token in
// VELTRIX_NETWORK_API_TOKEN. When the base URL is unset the allocator is treated
// as "not configured" and callers degrade gracefully.
// =============================================================================
