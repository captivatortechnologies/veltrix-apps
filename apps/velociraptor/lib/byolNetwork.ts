 ...
 * policy can later widen for larger distributed stacks.
 */
...
export async function resolvePlanNetwork(
  ...
  // Cloud stack, but the allocator is not wired yet → soft flag.
  ...
  try {
    ...
    console.error('[velociraptor] network allocation peek failed:', err)
...
export async function reserveDeployNetwork(
  ...
  try {
    ...
    console.error('[velociraptor] network allocation reserve failed:', err)
    return { tags }
  }
}
