...
// `velociraptor_byol_state_event` (lifecycle log) and `velociraptor_byol_usage`
// (daily metered ledger) tables. Foundation for usage-based cloud billing.
//
...
export async function recordStateEvent(
  ...
  await db.$executeRawUnsafe(
    `INSERT INTO velociraptor_byol_state_event (infrastructure_id, customer_id, status, node_count)
...
export async function listStateEventsForWindow(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    `(
       SELECT * FROM velociraptor_byol_state_event
...
export async function upsertUsage(
  ...
  await db.$executeRawUnsafe(
    `INSERT INTO velociraptor_byol_usage
...
export async function listUsage(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    `SELECT * FROM velociraptor_byol_usage
     WHERE ${scope}period_start >= $1 AND period_start < $2
