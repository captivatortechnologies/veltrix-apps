// ========================================================================
// Velociraptor App — Server Entry Module
//
// Registers Velociraptor-specific API routes as a Fastify plugin, prefixed with
// /api/apps/velociraptor/ and protected by app-level auth + permission
// middleware. Artifact CONFIGURATION authoring happens in the Configuration
// Canvas and every config write goes through the pipeline handlers (VQL over the
// gRPC API). This foundation exposes read-only /meta + /settings routes that back
// the Overview page. The app additionally OWNS BYOL stack provisioning: the /byol
// routes derive a resource plan from a stack's topology, persist it, and emit
// provisioning events for downstream workers — the declarative provisioning
// foundation ships in infra/spec.ts.
// ========================================================================
...
function topologyInputFor(infra: store.ByolDto) {
  return {
    ...
    // node_tiers-native: counts come straight off the generic per-tier list.
    tiers: infra.tiers,
  }
}
...
async function emit(events: AppEventPublisher, topic: string, payload: unknown): Promise<void> {
  try {
    ...
    console.error(`[velociraptor] publish ${topic} failed:`, err)
  }
}
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('artifacts', 'read')],
  ...
  // service token holding `usage:write`. Node-hours only — Velociraptor has no
