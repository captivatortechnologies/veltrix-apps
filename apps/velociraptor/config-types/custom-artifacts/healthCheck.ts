...
/**
 * Health for custom-artifacts = the Velociraptor server answers a VQL query over
 * the gRPC API with the configured api-client config. Read-only: runs
 * `SELECT * FROM info()`; any rows back count as reachable + authenticated.
 *
