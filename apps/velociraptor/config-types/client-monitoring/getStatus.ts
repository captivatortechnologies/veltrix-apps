...
/** Deployment status for a client-monitoring configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['velociraptor-server', 'standalone'] })
