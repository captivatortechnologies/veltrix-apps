...
/**
 * Health check for Check Point address-ranges configuration:
 *   1. Management API reachability + credential validity (login + show-address-ranges)
 *   2. Every declared range (by name) still exists in the management database
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  let live: LiveAddressRange[] = []

  try {
    live = await listAllAddressRanges(client)
    checks.push({
    ...
    checks.push({
      name: 'checkpoint_reachable',
      passed: false,
      message: error instanceof Error ? error.message : 'show-address-ranges failed',
      latencyMs: Date.now() - started,
    })
  } finally {
    await client.logout()
  ...
  if (live.length > 0 || checks[0]?.passed) {
    const names = new Set(live.filter((r) => r.name).map((r) => addressRangeKey(r.name as string)))
    for (const spec of specs) {
      const present = names.has(addressRangeKey(spec.name))
      checks.push({
        name: `range:${spec.name}`,
        passed: present,
        message: present ? `Address range "${spec.name}" is present` : `Address range "${spec.name}" is missing`,
      })
    }
  }
