...
**App:** `checkpoint` · **Category:** NETWORK · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Check Point API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 11 config types · 2 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Check Point API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + network-hostswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + network-hostsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: network-hostswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
