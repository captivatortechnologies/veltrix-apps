...
/**
 * Shows what this app manages on a Check Point Management Server, using the
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/checkpoint/meta')
  ...
  if (loading) return <Spinner label="Loading Check Point app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Check Point Security Management Server configuration as code through the Management
          API (web_api). Create a configuration in the Configuration Canvas and deploy it through the
          pipeline — validate, deploy, health check, drift detection and rollback are all handled per
          configuration type. Every deploy runs inside one Management API session: login, apply the
          declared changes, publish them together (or discard the whole session on any error), then
          logout.
        </p>
