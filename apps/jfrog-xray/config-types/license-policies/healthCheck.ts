...
import { extractLicensePolicySpecs, findPolicy, type XrayLicenseCriteria, type XrayLicensePolicy } from './_shared'

/**
 * Health check for the license-policies configuration:
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const specs = extractLicensePolicySpecs(ctx.canvas).filter((s) => s.name)

  const started = Date.now()
  let live: XrayLicensePolicy[] | null = null
  try {
  ...
  if (live) {
    for (const spec of specs) {
      ...
      checks.push({
        ...
        passed: present,
        message: present ? `License policy "${spec.name}" is present` : `License policy "${spec.name}" is missing`,
      })
    }
  }
