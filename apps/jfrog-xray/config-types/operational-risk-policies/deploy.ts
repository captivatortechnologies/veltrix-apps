...
import { buildPolicyBody, extractOperationalRiskPolicySpecs, findPolicy, type XrayOperationalRiskCriteria } from './_shared'

export type { PolicyRollbackEntry }

/**
 * Deploy JFrog Xray operational-risk policies over the Xray REST API v2 — the SAME
 * endpoints as security-policies/license-policies (a policy of `type: "operational_risk"`):
 ...
 * CRUD-by-name primitives shared with security/license-policies via lib/xrayPolicies.ts.
 */
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractOperationalRiskPolicySpecs(ctx.canvas).filter((s) => s.name && s.ruleName)
  const rollbackState: PolicyRollbackEntry<XrayOperationalRiskCriteria>[] = []
  const deployed: string[] = []

  try {
    const live = await listPolicies<XrayOperationalRiskCriteria>(client)
    ...
    for (const spec of specs) {
      ...
      if (existing) {
        const prior = await getPolicyByName<XrayOperationalRiskCriteria>(client, spec.name)
    ...
    return {
      success: true,
      message: `Deployed ${deployed.length} Xray operational-risk polic${deployed.length === 1 ? 'y' : 'ies'} to ${host}: ${deployed.join(', ')}`,
    ...
    return {
      success: false,
      message: `Xray operational-risk-policy deployment failed after ${deployed.length} of ${specs.length}: ${errorMessage(error)}`,
      artifacts: { host, deployedPolicies: deployed },
      rollbackData: { previous: rollbackState },
    }
  }
