...
/**
 * Report the current deployment status of the operational-risk-policies configuration.
...
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['jfrog-xray-instance'] })
