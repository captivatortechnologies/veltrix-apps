...
import { extractOperationalRiskPolicySpecs, findPolicy, type XrayOperationalRiskCriteria, type XrayOperationalRiskPolicy } from './_shared'

/**
 * Health check for the operational-risk-policies configuration:
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const specs = extractOperationalRiskPolicySpecs(ctx.canvas).filter((s) => s.name)

  const started = Date.now()
  let live: XrayOperationalRiskPolicy[] | null = null
  try {
  ...
  if (live) {
    for (const spec of specs) {
      ...
      checks.push({
        ...
        passed: present,
        message: present ? `Operational-risk policy "${spec.name}" is present` : `Operational-risk policy "${spec.name}" is missing`,
      })
    }
  }
