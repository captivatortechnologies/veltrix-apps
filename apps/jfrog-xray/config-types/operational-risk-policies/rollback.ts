...
import type { XrayOperationalRiskCriteria } from './_shared'

/**
 * Roll back Xray operational-risk policies using the state captured during deploy:
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  const previous = (ctx.rollbackData as { previous?: PolicyRollbackEntry<XrayOperationalRiskCriteria>[] } | null)?.previous
  ...
  try {
    ...
    return { success: true, message: `Rolled back ${reverted.length} Xray operational-risk polic${reverted.length === 1 ? 'y' : 'ies'}: ${reverted.join(', ')}` }
  } catch (error) {
    return {
      success: false,
