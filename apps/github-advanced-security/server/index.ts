// ========================================================================
// GitHub Advanced Security App — Server Entry Module
//
// Registers the app's API routes as a Fastify plugin, prefixed with
// /api/apps/github-advanced-security/ and protected by app-level auth +
// permission middleware. All security CONFIGURATION authoring happens in the
// Configuration Canvas, and every config write goes through the pipeline
// handlers (GitHub REST API). This foundation exposes read-only /meta + /settings
// routes that back the Overview page.
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('repo-security', 'read')],
