# 🛡️ GitHub Advanced Security — Request Flows
...
**App:** `github-advanced-security` · **Category:** COMPLIANCE · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **GitHub Advanced Security API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 13 config types · 10 API families.</sub>
...
sequenceDiagram
  ...
  participant api as GitHub Advanced Security API
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + repo-securitywrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + repo-securityread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: repo-securitywrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
