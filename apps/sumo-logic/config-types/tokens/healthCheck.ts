...
/**
 * Health for the tokens config = Sumo Logic answers on the Management API with
 * the configured Access ID / Access Key. Read-only: GET /tokens. Any response
 * below 500 counts as reachable.
 *
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/tokens`, { headers, timeoutMs: 8000 })
