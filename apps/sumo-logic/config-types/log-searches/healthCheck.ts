...
/**
 * Health for the log-searches config = Sumo Logic answers on the Management
 * API with the configured Access ID / Access Key. Read-only:
 * GET /logSearches?limit=1. Any response below 500 counts as reachable.
 *
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/logSearches?limit=1`, { headers, timeoutMs: 8000 })
