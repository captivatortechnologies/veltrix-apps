...
/**
 * Health for the ingest-budgets config = Sumo Logic answers on the Management
 * API v2 with the configured Access ID / Access Key. Read-only:
 * GET /ingestBudgets. Any response below 500 counts as reachable.
 *
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/ingestBudgets?limit=1`, { headers, timeoutMs: 8000 })
