...
/**
 * Health for the scheduled-views config = Sumo Logic answers on the Management
 * API with the configured Access ID / Access Key. Read-only: GET /scheduledViews.
 * Any response below 500 counts as reachable (auth nuances surface at deploy
 * time).
 *
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/scheduledViews?limit=1`, { headers, timeoutMs: 8000 })
