...
/**
 * Health for the data-forwarding-destinations config = Sumo Logic answers on
 * the Management API with the configured Access ID / Access Key. Read-only:
 * GET /logsDataForwarding/destinations. Any response below 500 counts as
 * reachable.
 *
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/logsDataForwarding/destinations?limit=1`, { headers, timeoutMs: 8000 })
