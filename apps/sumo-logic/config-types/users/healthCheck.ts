...
/**
 * Health for the users config = Sumo Logic answers on the Management API with
 * the configured Access ID / Access Key. Read-only: GET /users?limit=1. Any
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/users?limit=1`, { headers, timeoutMs: 8000 })
