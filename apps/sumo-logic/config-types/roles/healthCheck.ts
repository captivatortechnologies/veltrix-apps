...
/**
 * Health for the roles config = Sumo Logic answers on the Management API with the
 * configured Access ID / Access Key. Read-only: GET /roles. Any response below
 * 500 counts as reachable (auth nuances surface at deploy time).
 *
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/roles?limit=1`, { headers, timeoutMs: 8000 })
