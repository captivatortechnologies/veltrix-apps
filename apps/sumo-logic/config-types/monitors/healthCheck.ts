...
/**
 * Health for the monitors config = Sumo Logic answers on the Management API
 * with the configured Access ID / Access Key. Read-only: GET /monitors/root
 * (the one Monitors Library endpoint guaranteed to exist with no arguments).
 * Any response below 500 counts as reachable.
 *
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/monitors/root`, { headers, timeoutMs: 8000 })
