...
/**
 * Health for the field-extraction-rules config = Sumo Logic answers on the
 * Management API with the configured Access ID / Access Key. Read-only:
 * GET /extractionRules. Any response below 500 counts as reachable (auth nuances
 * surface at deploy time, not here).
 *
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/extractionRules`, { headers, timeoutMs: 8000 })
