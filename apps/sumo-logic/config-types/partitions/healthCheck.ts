...
/**
 * Health for the partitions config = Sumo Logic answers on the Management API
 * with the configured Access ID / Access Key. Read-only: GET /partitions. Any
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/partitions?limit=1`, { headers, timeoutMs: 8000 })
