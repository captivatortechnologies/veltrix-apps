...
/**
 * Health for the folders config = Sumo Logic answers on the Management API v2
 * with the configured Access ID / Access Key. Read-only:
 * GET /content/folders/personal (always present, needs no arguments). Any
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await sumoRequest(`${base}/content/folders/personal`, { headers, timeoutMs: 8000 })
