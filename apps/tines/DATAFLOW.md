...
**App:** `tines` · **Category:** SOAR · **Version:** 0.1.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Tines API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 7 config types · 4 API families.</sub>
...
sequenceDiagram
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + teamswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + teamsread
  rbac-->>pipeline: permitted ✓ / denied
