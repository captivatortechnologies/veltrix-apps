// ========================================================================
// Tines App — Server Entry Module
//
// Registers Tines-specific API routes as a Fastify plugin, prefixed with
// /api/apps/tines/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration
// Canvas, and every write goes through the pipeline handlers (Tines REST API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('teams', 'read')],
