...
/**
 * Health for the intel-feed config = Darktrace answers a DSA-signed read with the
 * configured token pair. Read-only + lightweight: GET /intelfeed?sources=true
 * (returns just the watched-list source labels, not every entry). A 2xx confirms
