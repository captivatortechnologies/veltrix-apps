...
**App:** `darktrace` · **Category:** NETWORK · **Version:** 0.2.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Darktrace API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
<sub>Applies to: 2 config types · 2 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Darktrace API
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + intelfeedwrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + intelfeedread
  rbac-->>pipeline: permitted ✓ / denied
