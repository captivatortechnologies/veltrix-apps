...
/**
 * Shows what this app manages in a Teleport cluster, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/teleport/meta')
  ...
  if (loading) return <Spinner label="Loading Teleport app details..." />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Teleport infrastructure access &amp; PAM configuration as code through the Teleport
          Proxy web API - the same JSON+YAML surface the Teleport Web UI itself uses. Create a
