...
// `sonarqube_byol_infrastructure` table.
//
...
// search_head_count columns, and no region satellite tables). `customerId` and
// `cloudProviderId` are plain columns referencing PLATFORM entities; the app
// never foreign-keys across the boundary.
// =============================================================================
...
import { normalizeControlPlaneLayout, type ControlPlaneLayout } from '../byolPlacement'
import { recordStateEvent } from './usage'

/** Serialize the generic per-tier `node_tiers` column (defaults to an empty array). */
function nodeTiersJson(input: ByolInput): string {
  return JSON.stringify(input.nodeTiers ?? [])
}

/** Total stack nodes across the scalable tiers — the node-hours billing multiplier. */
...
export interface ByolInput {
  ...
  // Generic per-tier node counts + placement (node_tiers-native). Always
  // populated by lib/byolInput.ts's readByol, in [application, search] order.
  nodeTiers?: NodeTierDto[]
}

export async function listByol(db: PlatformDatabaseClient, customerId: string): Promise<ByolDto[]> {
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM sonarqube_byol_infrastructure WHERE customer_id = $1::uuid ORDER BY updated_at DESC',
    customerId,
  )
...
export async function getByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM sonarqube_byol_infrastructure WHERE id = $1::uuid AND customer_id = $2::uuid',
    id,
    customerId,
  )
...
export async function createByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    ...
    `INSERT INTO sonarqube_byol_infrastructure
       (name, deployment_type, environment_type, hosting_type, region,
        ...
        control_plane_layout, heavy_forwarder_count, instance_type, node_tiers)
...
export async function updateByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    `UPDATE sonarqube_byol_infrastructure SET
       name = $2, deployment_type = $3, environment_type = $4, hosting_type = $5,
       region = $6,
       cloud_provider_id = COALESCE($7::uuid, cloud_provider_id),
       control_plane_layout = $8, heavy_forwarder_count = $9,
       instance_type = $10,
       network_mode = $11, dns_mode = $12, cloud_account_connection_id = $13::uuid,
       node_tiers = $14::jsonb,
       updated_at = now()
     WHERE id = $1::uuid
     RETURNING *`,
...
export async function setByolStatus(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'UPDATE sonarqube_byol_infrastructure SET status = $2, updated_at = now() WHERE id = $1::uuid RETURNING *',
    id,
    status,
  )
...
export async function deleteByol(db: PlatformDatabaseClient, id: string): Promise<void> {
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM sonarqube_byol_infrastructure WHERE id = $1::uuid',
    id,
  )
  if (rows[0]) await emitStateEvent(db, mapByol(rows[0]), 'decommissioned')
  await db.$executeRawUnsafe('DELETE FROM sonarqube_byol_infrastructure WHERE id = $1::uuid', id)
}
 ...
 *  customer scoping — bus events are already platform-trusted. */
export async function getByolCore(
  db: PlatformDatabaseClient,
  id: string,
): Promise<{ id: string; name: string; customerId: string } | null> {
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT id, name, customer_id FROM sonarqube_byol_infrastructure WHERE id = $1::uuid',
    id,
  )
  const r = rows[0]
