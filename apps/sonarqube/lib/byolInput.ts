...
//   • tiers['application'] → SonarQube web/compute application nodes (ALB targets)
//   • tiers['search']      → Elasticsearch search nodes
//
...
// `nodeTiers` in declared order [application, search].
// =============================================================================

import { normalizeControlPlaneLayout, parsePlacement, validatePlacement, type ClusterPlacement } from './byolPlacement'

/** SonarQube's two scalable BYOL node tiers, in persisted/display order. */
const TIER_KEYS = ['application', 'search'] as const
...
/**
 * Minimum node count per tier. Application nodes stay permissive (≥1). A
 * DISTRIBUTED Elasticsearch search cluster needs at least three nodes to form a
 * real quorum, so the search minimum is 3 for distributed deployments (a
 * single-node deployment collapses to one all-in-one box, so every tier is
 * effectively ≥1 there).
 */
...
export function readByol(body: any): { data: Record<string, unknown>; error?: string } {
  ...
  // "Distributed" is the multi-node SonarQube stack (app nodes + Elasticsearch
  // search + PostgreSQL on separate nodes); single instance is the all-in-one box.
  ...
  // Carried on the record for SDK-form round-tripping; unused by SonarQube's
  // resource plan (SonarQube has no consolidated control plane / forwarder tier).
