...
test('createByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  const insert = calls.find((c) => c.sql.includes('INSERT INTO sonarqube_byol_infrastructure'))
  assert.ok(insert)
...
test('updateByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  const update = calls.find((c) => c.sql.includes('UPDATE sonarqube_byol_infrastructure'))
  assert.ok(update)
