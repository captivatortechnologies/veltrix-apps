...
// SDK's <ByolInfrastructureManager> sends (application / search), per-tier
...
test('readByol defaults single-instance topology fields', () => {
  const { data } = readByol({
    ...
    tiers: [{ key: 'application', count: 1 }, { key: 'search', count: 1 }],
  })
...
test('readByol emits nodeTiers in [application, search] order regardless of input order', () => {
  const { data } = readByol(
    distributedBody({
      tiers: [
        { key: 'search', count: 3 },
  ...
  const tiers = data.nodeTiers as Array<{ key: string; count: number }>
  assert.deepEqual(tiers.map((t) => t.key), ['application', 'search'])
...
test('readByol requires at least 1 Application nodes', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'application', count: 0 }, { key: 'search', count: 3 }] }))
...
test('readByol requires at least 3 Search nodes for a distributed cluster', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'application', count: 1 }, { key: 'search', count: 2 }] }))
...
test('readByol accepts a distributed search cluster of exactly 3', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'application', count: 1 }, { key: 'search', count: 3 }] }))
  assert.equal(error, undefined)
})
...
test('readByol rejects region-granularity placement unless the network is dedicated (BYOC)', () => {
  const { error } = readByol(
    distributedBody({
      tiers: [{ key: 'application', count: 2 }, { key: 'search', count: 4, placement: regionPlacement }],
    }),
  )
