...
//     it returns null for an infra. SonarQube has no simple built-in ingest
//     metric, so the poller is unused today — node-hours are the live dimension
//     (the poller stays wired so a future SonarQube volume metric can plug in).
//
 ...
 * omit it to record node-hours only (the current default — SonarQube has no
 * simple built-in ingest metric).
 */
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM sonarqube_byol_infrastructure',
  )
