...
/**
 * Health for the quality-gates config = SonarQube answers its Web API and the token
 * authenticates. Read-only:
 *   - GET /api/system/status  → the server is UP (unauthenticated; returns version)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    ...
    checks.push({
      ...
      passed,
      message: passed
        ? `SonarQube is UP${version ? ` (v${version})` : ''}.`
        : `SonarQube status is "${status ?? `HTTP ${res.status}`}".`,
    ...
    checks.push({
      name: 'sonarqube_status',
      passed: false,
      message: `SonarQube unreachable: ${error instanceof Error ? error.message : 'error'}`,
      latencyMs: Date.now() - started,
    })
  }
  ...
  try {
    ...
    checks.push({
      name: 'token_valid',
      passed: valid,
      message: valid ? 'API token authenticates.' : 'API token was not accepted by SonarQube.',
    ...
    checks.push({
      name: 'token_valid',
      passed: false,
      message: `Token validation failed: ${error instanceof Error ? error.message : 'error'}`,
      latencyMs: Date.now() - authStarted,
    })
  }
