...
/**
 * Health for the firewall-policies config = the Aqua Console answers on its
 * firewall-policies REST API with the configured credential. Read-only probe:
 * GET /api/v2/firewall_policies/<probe-name>.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.request('GET', '/api/v2/firewall_policies/veltrix-connectivity-probe')
    ...
    if (res.status === 401 || res.status === 403) {
      ...
      checks.push({
        ...
        passed,
        message: passed
          ? `Aqua firewall policies API reachable (HTTP ${res.status}).`
