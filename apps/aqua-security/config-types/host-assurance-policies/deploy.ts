...
const ASSURANCE_TYPE = 'host' as const

/**
 * Deploy Aqua host assurance policies over the Console REST API:
 *   find:    GET    /api/v2/assurance_policy/host/<name>
 *   create:  POST   /api/v2/assurance_policy/host
 *   update:  PUT    /api/v2/assurance_policy/host/<name>
 *   remove:  DELETE /api/v2/assurance_policy/host/<name>  (for a disabled policy)
 *
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  try {
    ...
    return {
      success: true,
      message: `Applied ${applied.length} host assurance policy(ies): ${applied.join(', ') || '(none)'}`,
      artifacts: { applied },
