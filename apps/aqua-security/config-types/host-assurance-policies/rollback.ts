...
/**
 * Undo an host-assurance-policies deploy from rollbackData.previous (written
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    ...
    return {
      success: true,
      message: `Rolled back host assurance policies: ${restored} restored, ${recreated} re-created, ${removed} removed${skipped ? `, ${skipped} skipped` : ''}.`,
    }
  } catch (error) {
