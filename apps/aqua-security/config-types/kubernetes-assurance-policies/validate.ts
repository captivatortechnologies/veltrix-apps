...
/**
 * Validate kubernetes-assurance-policy items: a non-empty unique name, at least one
...
export default async function validate(ctx: PipelineContext): Promise<ValidationResult> {
  ...
  if (specs.length === 0) {
    errors.push({ field: 'items', message: 'Add at least one kubernetes assurance policy.', code: 'EMPTY' })
  }
