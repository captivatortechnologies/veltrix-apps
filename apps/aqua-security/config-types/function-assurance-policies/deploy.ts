...
const ASSURANCE_TYPE = 'function' as const

/**
 * Deploy Aqua function assurance policies over the Console REST API:
 *   find:    GET    /api/v2/assurance_policy/function/<name>
 *   create:  POST   /api/v2/assurance_policy/function
 *   update:  PUT    /api/v2/assurance_policy/function/<name>
 *   remove:  DELETE /api/v2/assurance_policy/function/<name>  (for a disabled policy)
 *
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  try {
    ...
    return {
      success: true,
      message: `Applied ${applied.length} function assurance policy(ies): ${applied.join(', ') || '(none)'}`,
      artifacts: { applied },
