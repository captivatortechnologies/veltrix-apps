...
/**
 * Health for the function-assurance-policies config = the Aqua Console answers
 ...
 * /api/v2/assurance_policy/function/<probe-name> (a 404 for a name that does not
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.request('GET', '/api/v2/assurance_policy/function/veltrix-connectivity-probe')
    ...
    if (res.status === 401 || res.status === 403) {
      ...
      checks.push({
        ...
        passed,
        message: passed
          ? `Aqua function assurance policy API reachable (HTTP ${res.status}).`
