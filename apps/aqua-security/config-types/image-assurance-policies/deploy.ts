...
/**
 * Deploy Aqua image assurance policies over the Console REST API:
 *   find:    GET    /api/v2/assurance_policy/image/<name>
 *   create:  POST   /api/v2/assurance_policy/image
 *   update:  PUT    /api/v2/assurance_policy/image/<name>
 *   remove:  DELETE /api/v2/assurance_policy/image/<name>  (for a disabled policy)
 *
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  try {
    ...
    return {
      success: true,
      message: `Applied ${applied.length} image assurance policy(ies): ${applied.join(', ') || '(none)'}`,
      artifacts: { applied },
