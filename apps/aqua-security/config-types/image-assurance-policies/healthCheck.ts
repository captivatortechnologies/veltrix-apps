...
/**
 * Health for the image-assurance-policies config = the Aqua Console answers
 ...
 * /api/v2/assurance_policy/image/<probe-name> (a 404 for a name that does not
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.request('GET', '/api/v2/assurance_policy/image/veltrix-connectivity-probe')
    ...
    if (res.status === 401 || res.status === 403) {
      ...
      checks.push({
        ...
        passed,
        message: passed
          ? `Aqua image assurance policy API reachable (HTTP ${res.status}).`
