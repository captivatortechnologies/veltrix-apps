...
const RUNTIME_TYPE = 'host' as const

/**
 * Deploy Aqua host runtime policies over the Console REST API:
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  try {
    ...
    return {
      success: true,
      message: `Applied ${applied.length} host runtime policy(ies): ${applied.join(', ') || '(none)'}`,
      artifacts: { applied },
    ...
    return {
      success: false,
      message: `Host runtime policy deploy failed after ${applied.length} policy(ies): ${error instanceof Error ? error.message : 'Unknown error'}`,
      artifacts: { applied },
      rollbackData: { previous },
    }
  }
