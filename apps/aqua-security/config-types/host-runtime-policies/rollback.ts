...
/**
 * Undo a host-runtime-policies deploy from rollbackData.previous
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    ...
    return {
      success: true,
      message: `Rolled back host runtime policies: ${restored} restored, ${recreated} re-created, ${removed} removed${skipped ? `, ${skipped} skipped` : ''}.`,
    }
  } catch (error) {
