...
/**
 * Health for the enforcer-groups config = the Aqua Console answers on its
 * hostsbatch (Enforcer Group) REST API with the configured credential.
 * Read-only probe: GET /api/v1/hostsbatch/<probe-name>.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.request('GET', '/api/v1/hostsbatch/veltrix-connectivity-probe')
    ...
    if (res.status === 401 || res.status === 403) {
      ...
      checks.push({
        ...
        passed,
        message: passed
          ? `Aqua enforcer groups API reachable (HTTP ${res.status}).`
