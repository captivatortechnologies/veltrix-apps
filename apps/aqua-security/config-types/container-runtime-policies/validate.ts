...
/**
 * Validate container-runtime-policy items: a non-empty unique name, at least
...
export default async function validate(ctx: PipelineContext): Promise<ValidationResult> {
  ...
  if (specs.length === 0) {
    errors.push({ field: 'items', message: 'Add at least one container runtime policy.', code: 'EMPTY' })
  }
