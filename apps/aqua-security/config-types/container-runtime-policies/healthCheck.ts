...
/**
 * Health for the container-runtime-policies config = the Aqua Console answers
 * on its runtime-policies REST API with the configured credential. Read-only
 * probe of the exact API surface this config type writes to: GET
 * /api/v2/runtime_policies/<probe-name>.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.request('GET', '/api/v2/runtime_policies/veltrix-connectivity-probe')
    ...
    if (res.status === 401 || res.status === 403) {
      ...
      checks.push({
        ...
        passed,
        message: passed
          ? `Aqua runtime policies API reachable (HTTP ${res.status}).`
