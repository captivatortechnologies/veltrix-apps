...
/**
 * Health check for IPv6 outbound port configuration:
 *   1. The outbound-ports-v6 list is readable via ACS (also proves token validity)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
      ...
      checks: [
        {
          name: 'acs_reachable',
          passed: false,
          message: `Could not read IPv6 outbound ports via ACS: ${acsErrorMessage(res)}`,
        },
      ],
    }
  }
  ...
  for (const spec of specs) {
    ...
    checks.push({
      ...
      message:
        missing.length === 0
          ? `Port ${port}: all ${spec.subnets.length} IPv6 destination(s) present`
          : `Port ${port}: missing ${missing.length} IPv6 destination(s): ${missing.join(', ')}`,
    })
  }
