...
/**
 * Health check for IPv6 allow list configuration:
 *   1. Each feature's IPv6 allow list is readable via ACS (also proves token validity)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  for (const spec of specs) {
    ...
    checks.push(
      await timedCheck(`allowlist-v6:${spec.feature}:readable`, async () => {
        ...
        return `IPv6 allow list for "${spec.feature}" is readable (${live.length} subnet(s))`
      }),
    )
    ...
    if (live !== null) {
      ...
      checks.push(
        await timedCheck(`allowlist-v6:${spec.feature}:subnets`, async () => {
  ...
  if (checks.length === 0) {
    checks.push({
      name: 'configuration',
      passed: false,
      message: 'No IPv6 allow list features declared in the canvas',
    })
  }
