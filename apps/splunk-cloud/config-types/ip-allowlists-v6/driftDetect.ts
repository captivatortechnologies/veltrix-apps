...
/**
 * Detect drift between the deployed IPv6 allow lists and the live ACS state.
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  const specs = extractAllowlistV6Specs(ctx.deployedConfig).filter((s) => s.feature)
  ...
  for (const spec of specs) {
    try {
      const res = await acsRequest(
        acs,
        'GET',
        `/access/${encodeURIComponent(spec.feature)}/ipallowlists-v6`,
      )
