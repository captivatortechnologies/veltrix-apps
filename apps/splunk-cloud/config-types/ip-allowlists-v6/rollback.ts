...
import type { AllowlistV6RollbackEntry } from './deploy'

/**
 * Roll back IPv6 allow list changes using the delta captured during deploy:
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  const previousState = (ctx.rollbackData as { previousState?: AllowlistV6RollbackEntry[] })
    ?.previousState
  ...
  try {
    for (const entry of previousState) {
      const path = `/access/${encodeURIComponent(entry.feature)}/ipallowlists-v6`
    ...
    return {
      success: true,
      message: `Rolled back IPv6 allow list changes for ${reverted.length} feature(s) on stack "${stack}": ${reverted.join(', ')}`,
    }
  } catch (error) {
    return {
      success: false,
