# ☁️ Splunk Cloud Platform — Request Flows
...
**App:** `splunk-cloud` · **Category:** SIEM · **Version:** 1.10.12  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Splunk Cloud Platform API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Splunk Cloud Platform API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + indexeswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + indexesread
  rbac-->>pipeline: permitted ✓ / denied
