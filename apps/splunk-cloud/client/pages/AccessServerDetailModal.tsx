...
export default function AccessServerDetailModal({
  ...
  // configured hostname. Management defaults to 443, Splunk Web to 8000.
  const reachHost = device?.online && device.addresses?.[0] ? device.addresses[0] : device?.name || server.hostname
  const mgmtPort = server.port?.trim() || '443'
  const webUiPort = server.webPort?.trim() || '8000'
  // Management API is always HTTPS. Splunk Web defaults to HTTP (8000) — only TLS
  // if Splunk Web SSL is explicitly enabled — so link it as http.
