...
**App:** `wazuh` · **Category:** SIEM · **Version:** 0.5.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Wazuh API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
<sub>Applies to: 10 config types · 5 API families.</sub>
...
sequenceDiagram
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + cdb-listswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + cdb-listsread
  rbac-->>pipeline: permitted ✓ / denied
