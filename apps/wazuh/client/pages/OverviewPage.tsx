...
/**
 * What this app manages in a Wazuh deployment, rendered with the platform
...
export default function OverviewPage() {
  ...
  if (loading) return <Spinner label="Loading Wazuh app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Wazuh (open-source SIEM / XDR / HIDS) configuration as code. Author a configuration in the
          Configuration Canvas and deploy it through the pipeline — validate, deploy, health check, drift
          detection and rollback are handled per configuration type. Configuration is applied over the Wazuh
          manager REST API (55000), which is token-authenticated and self-signed by default.
        </p>
