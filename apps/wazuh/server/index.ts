// ========================================================================
// Wazuh App — Server Entry Module
//
// Registers Wazuh-specific API routes as a Fastify plugin, prefixed with
// /api/apps/wazuh/ and protected by app-level auth + permission middleware.
// CONFIGURATION authoring happens in the Configuration Canvas and every config
// write goes through the pipeline handlers over the Wazuh REST API (55000).
//
// This module exposes read-only /meta + /settings AND OWNS BYOL cluster
// provisioning: the /byol routes derive a resource plan from a Wazuh cluster's
// topology (infra/spec.ts), persist it, and emit provisioning events for
// downstream workers.
// ========================================================================
...
const DESTROY_STEPS: Array<{ key: string; title: string; detail: string }> = [
  ...
  { key: 'done', title: 'Resources destroyed', detail: 'All resources for this cluster have been removed.' },
]
...
async function emit(events: AppEventPublisher, topic: string, payload: unknown): Promise<void> {
  try {
    ...
    console.error(`[wazuh] publish ${topic} failed:`, err)
  }
}
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('cdb-lists', 'read')],
  ...
  // cluster's topology, persists it, opens a deployment run, flips the record to
  ...
  fastify.post('/byol/:id/deploy', {
    ...
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Atomically reserve the cluster's subnet + derive the tenant/cost tags
      ...
      // provisioning worker see the exact subnet this cluster was given.
      ...
      await emit(events, 'infrastructure.deploy.requested', {
        ...
        // access link when the cluster is ready.
  ...
  // service token holding `usage:write`. Node-hours only until the cluster's
  // indexer ingest poller is wired (see collector.ts).
