// Shared helpers for the Wazuh custom-decoders config type (validate + deploy + drift).
//
// A custom decoder file is an XML document uploaded to the manager's user ruleset
// (etc/decoders) and compiled into the active decoder set when the manager
// restarts. The canvas `comment` field is audit-only metadata and is never written
// into the file body.

/** A decoder filename: a safe basename ending in .xml. */
export const DECODERS_FILENAME_RE = /^[A-Za-z0-9._-]+\.xml$/
