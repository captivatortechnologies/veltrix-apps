...
/**
 * Undo a CDB-list deploy from rollbackData.previous (written by deploy()): for
 * each entry, PUT the prior raw file body back, or DELETE the file we created
 * (its prior body was null). Applied over the Wazuh REST API (55000).
 */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  if (!credential) {
    return { success: false, message: 'Missing credential for CDB list rollback' }
  }
  ...
  try {
    ...
    for (const { filename, content } of previous) {
      if (content != null) {
        ...
        const url = `${baseUrl}/lists/files/${encodeURIComponent(filename)}`
    ...
    return { success: true, message: `Rolled back CDB lists: ${restored} restored, ${deleted} removed.` }
  } catch (error) {
