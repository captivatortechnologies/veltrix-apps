...
/** Deployment status for an API-security-settings configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['manager', 'manager-master', 'manager-worker'] })
