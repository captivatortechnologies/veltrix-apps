...
/**
 * Deploy Wazuh custom rule files over the REST API (55000):
 *   read (rollback): GET ${base}/rules/files/<filename>                (best-effort — 404 = new file)
 *   apply:           PUT ${base}/rules/files/<filename>?overwrite=true  (raw rules XML)
 *   reload:          PUT ${base}/manager/restart                        (best-effort — a rule change needs a restart to take effect)
 *
...
/** Read the live rules file body (best-effort) for the rollback snapshot; null on any miss. */
async function readFile(base: string, headers: Record<string, string>, filename: string): Promise<string | null> {
  try {
    const res = await wazuhRequest(`${base}/rules/files/${encodeURIComponent(filename)}`, { headers })
...
/** Best-effort manager restart to reload the ruleset; never throws — the outcome is reported. */
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  if (!credential) {
    return { success: false, message: 'Missing credential for custom-rules deployment' }
  }
  ...
  try {
    ...
    for (const item of items) {
      ...
      const body = String(item.fields.rulesXml ?? '')
      const url = `${baseUrl}/rules/files/${encodeURIComponent(filename)}?overwrite=true`
    ...
    return {
      success: true,
      message: `Applied ${applied.length} rule file(s): ${applied.join(', ') || '(none)'}. Manager restart: ${restart}`,
      artifacts: { applied },
    ...
    return {
      success: false,
      message: `Custom-rules deploy failed after ${applied.length} file(s): ${error instanceof Error ? error.message : 'Unknown error'}`,
      artifacts: { applied },
      rollbackData: { previous },
    }
  }
