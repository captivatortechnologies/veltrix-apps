...
/**
 * Undo a custom-rules deploy from rollbackData.previous (written by deploy()): for
 * each entry, PUT the prior raw file body back, or DELETE the file we created (its
 * prior body was null). Applied over the Wazuh REST API (55000). A manager restart
 * to re-activate the prior ruleset is deferred to the operator.
 */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  if (!credential) {
    return { success: false, message: 'Missing credential for custom-rules rollback' }
  }
  ...
  try {
    ...
    for (const { filename, content } of previous) {
      if (content != null) {
        ...
        const url = `${baseUrl}/rules/files/${encodeURIComponent(filename)}`
    ...
    return { success: true, message: `Rolled back rule files: ${restored} restored, ${deleted} removed.` }
  } catch (error) {
