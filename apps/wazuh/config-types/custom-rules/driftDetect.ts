...
/**
 * Drift for custom rules: compare the ruleset XML we declare against the live file
 * on the manager. Best-effort — a file that can't be read (missing / transient
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  for (const item of items) {
    ...
    const declared = String(item.fields.rulesXml ?? '').trim()
    if (!filename || !declared) continue

    let liveBody: string
    try {
      const res = await wazuhRequest(`${baseUrl}/rules/files/${encodeURIComponent(filename)}`, { headers: auth })
