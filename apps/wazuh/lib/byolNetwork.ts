...
// the POLICY here — which provider/region a cluster targets and what subnet size
...
/** The subnet a cluster carries (superset of the plan's `network` block). */
 ...
 * A self-hosted cluster (no cloud provider) returns null — no Veltrix Network
...
/**
 * The subnet prefix (mask length) a cluster requests. The resolved network model
 * gives every cluster a /24 within its customer's block; kept as a function so
 * the policy can later widen for larger distributed clusters.
 */
 ...
 * `networkUnavailable` flag when a cloud cluster's allocator is unreachable).
 */
export async function resolvePlanNetwork(
  ...
  // Cloud cluster, but the allocator is not wired yet (Phase 2a) → soft flag.
  ...
  try {
    ...
    console.error('[wazuh] network allocation peek failed:', err)
...
export async function reserveDeployNetwork(
  ...
  try {
    ...
    console.error('[wazuh] network allocation reserve failed:', err)
    return { tags }
  }
}
