...
//   * ingest_gb: from an INJECTED poller (e.g. the Wazuh indexer's OpenSearch
//     _cat/indices on the manager). Skipped when no poller is supplied or it
//     returns null for an infra — because reaching a cluster's indexer requires
//     the component/connectivity/credential context that BYOL records do not
//     carry yet (follow-up: link a BYOL cluster to its manager component so
//     ingest can be polled).
//
 ...
 * omit it to record node-hours only (the current default until the cluster's
 * indexer link exists).
 */
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM wazuh_byol_infrastructure',
  )
