...
// `wazuh_byol_state_event` (lifecycle log) and `wazuh_byol_usage` (daily metered
// ledger) tables. Foundation for usage-based cloud billing.
//
...
export async function recordStateEvent(
  ...
  await db.$executeRawUnsafe(
    `INSERT INTO wazuh_byol_state_event (infrastructure_id, customer_id, status, node_count)
...
export async function listStateEventsForWindow(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    `(
       SELECT * FROM wazuh_byol_state_event
