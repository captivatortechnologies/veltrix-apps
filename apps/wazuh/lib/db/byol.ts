...
// `wazuh_byol_infrastructure` table (and its region satellites).
//
...
import { mapByol, mapRegion, type ByolDto, type ByolTierDto, type RegionDto, type Row } from './mappers'
import {
...
/** Null out a tier's placement unless it's genuinely multi-site — mirrors
 *  `placementJson`'s column convention so `node_tiers` and the legacy
 *  indexer_placement/search_head_placement columns agree on single-site. */
function normalizeTierPlacement(placement: ClusterPlacement | null | undefined): ClusterPlacement | null {
  if (!placement || placement.mode !== 'multi-site') return null
  return placement
}

/**
 * Serialize the generic per-tier node list for the `node_tiers` JSONB column.
 * Falls back to deriving an [indexer, worker] pair from the legacy
 * indexerCount/searchHeadCount + placement fields when the caller doesn't
 * supply `nodeTiers` directly (defensive — every current caller goes through
 * `readByol()`, which always populates it).
 */
function nodeTiersJson(input: ByolInput): string {
  const tiers: ByolTierDto[] =
    input.nodeTiers ??
    [
      { key: 'indexer', count: input.indexerCount, placement: input.indexerPlacement ?? null },
      { key: 'worker', count: input.searchHeadCount, placement: input.searchHeadPlacement ?? null },
    ]
  return JSON.stringify(tiers.map((t) => ({ ...t, placement: normalizeTierPlacement(t.placement) })))
}
...
export interface ByolInput {
  ...
  /** Generic per-tier node counts + placement, in display order [indexer, worker]. Derived from indexerCount/searchHeadCount + placements when omitted. */
  nodeTiers?: ByolTierDto[]
...
async function attachRegions(db: PlatformDatabaseClient, infra: ByolDto): Promise<ByolDto> {
  const [indexer, searchHead] = await Promise.all([
    db.$queryRawUnsafe<Row[]>(
      'SELECT * FROM wazuh_byol_indexer_region WHERE infrastructure_id = $1::uuid',
      infra.id,
    ),
    db.$queryRawUnsafe<Row[]>(
      'SELECT * FROM wazuh_byol_search_head_region WHERE infrastructure_id = $1::uuid',
      infra.id,
    ),
  ])
...
export async function listByol(db: PlatformDatabaseClient, customerId: string): Promise<ByolDto[]> {
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM wazuh_byol_infrastructure WHERE customer_id = $1::uuid ORDER BY updated_at DESC',
    customerId,
  )
...
export async function getByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM wazuh_byol_infrastructure WHERE id = $1::uuid AND customer_id = $2::uuid',
    id,
    customerId,
  )
...
export async function createByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    ...
    `INSERT INTO wazuh_byol_infrastructure
...
export async function updateByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    `UPDATE wazuh_byol_infrastructure SET
...
export async function setByolStatus(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'UPDATE wazuh_byol_infrastructure SET status = $2, updated_at = now() WHERE id = $1::uuid RETURNING *',
    id,
    status,
  )
...
export async function deleteByol(db: PlatformDatabaseClient, id: string): Promise<void> {
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM wazuh_byol_infrastructure WHERE id = $1::uuid',
    id,
  )
  if (rows[0]) await emitStateEvent(db, mapByol(rows[0]), 'decommissioned')
  await db.$executeRawUnsafe('DELETE FROM wazuh_byol_infrastructure WHERE id = $1::uuid', id)
}
...
export async function getByolCore(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT id, name, customer_id FROM wazuh_byol_infrastructure WHERE id = $1::uuid',
    id,
  )
  const r = rows[0]
