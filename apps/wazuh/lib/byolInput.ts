...
//
// The three shared node knobs (indexerCount / searchHeadCount / heavyForwarderCount)
// keep the SDK form's Splunk-shaped names; the Wazuh topology maps them to
// indexer nodes / manager-worker nodes / dashboard nodes (see lib/byolTopology.ts).
//
...
export function readByol(body: any): { data: Record<string, unknown>; error?: string } {
  ...
  if (isDistributed) {
    ...
    if (searchHeadCount < 2) return { data: {}, error: 'Distributed deployments require at least 2 manager workers' }
  }
  ...
  if (isDistributed) {
    ...
    if (searchErr) return { data: {}, error: `Manager worker placement: ${searchErr}` }
    ...
    for (const [label, p] of [
      ...
      ['Manager worker', searchHeadPlacement],
    ] as const) {
