...
/**
 * Health check for identity alias configuration:
 ...
 *   3. Every declared alias is still present
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    // Check 3..n: each declared alias is still present
    ...
    for (const spec of specs) {
      ...
      checks.push(
        await timedCheck(`alias:${key}`, async () => {
          const live = await findAliasByLabel(client, kind, spec.mountAccessor, spec.name)
          if (!live) throw new Error(`Identity alias "${key}" is not present`)
          return `Identity alias "${key}" is present`
        }),
      )
    }
  }
