...
import { extractQuotaSpecs } from './validate'

/**
 * Health check for rate limit quota configuration:
 ...
 *   3. Every declared quota is still present
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    // Check 3..n: each declared quota is still present
    const specs = extractQuotaSpecs(ctx.canvas).filter((s) => s.name && Number.isFinite(s.rate) && s.rate > 0)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`quota:${spec.name}`, async () => {
          const live = await getQuota(client, spec.name)
          if (!live) throw new Error(`Rate limit quota "${spec.name}" is not present`)
          return `Rate limit quota "${spec.name}" is present`
        }),
      )
    }
  }
