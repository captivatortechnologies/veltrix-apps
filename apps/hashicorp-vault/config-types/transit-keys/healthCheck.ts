...
import { extractTransitKeySpecs, keyKey } from './validate'

/**
 * Health check for transit key configuration:
 ...
 *   3. Every declared key is still present with the expected type
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    // Check 3..n: each declared key is still present with the expected type
    const specs = extractTransitKeySpecs(ctx.canvas).filter((s) => s.mount && s.name && s.type)
    for (const spec of specs) {
      const key = keyKey(spec.mount, spec.name)
      checks.push(
        await timedCheck(`key:${key}`, async () => {
          const live = await getTransitKey(client, spec.mount, spec.name)
          if (!live) throw new Error(`Transit key "${key}" is not present`)
          const liveType = (live.type ?? '').toLowerCase()
          if (liveType !== spec.type) {
            throw new Error(`Transit key "${key}" is type "${live.type}", expected "${spec.type}"`)
          }
          return `Transit key "${key}" (${spec.type}) is present`
        }),
      )
    }
  }
