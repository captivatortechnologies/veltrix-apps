...
import { extractEntitySpecs } from './validate'

/**
 * Health check for identity entity configuration:
 ...
 *   3. Every declared entity still exists (GET /identity/entity/name/{name})
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    // Check 3..n: each declared entity still exists
    const specs = extractEntitySpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`entity:${spec.name}`, async () => {
          const live = await getEntity(client, spec.name)
          if (!live) throw new Error(`Identity entity "${spec.name}" does not exist`)
          return `Identity entity "${spec.name}" exists`
        }),
      )
    }
  }
