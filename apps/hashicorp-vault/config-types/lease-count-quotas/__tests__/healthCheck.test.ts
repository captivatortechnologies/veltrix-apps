...
const LIVE_QUOTA = {
  status: 200,
  body: { data: { name: 'db-leases', path: 'database/', max_leases: 1000 } },
}

function ctx(o: { token?: string | null } = {}) {
  return makeHealthCheckContext(
    makeCanvas(
      [{ name: 'Quota 1', fields: { name: 'db-leases', path: 'database/', maxLeases: 1000 } }],
      'lease-count-quotas',
    ),
    o,
  )
...
describe('Vault Lease Count Quotas Health Check Handler', () => {
  ...
  it('scores 100 when Vault is active, the token is accepted and the quota exists', async () => {
    ...
    try {
      ...
      expect(fetchStub.calls[2].url).toBe(`${VAULT_BASE}/sys/quotas/lease-count/db-leases`)
      expect(fetchStub.calls[0].headers['X-Vault-Token']).toBe(VAULT_TOKEN)
      expect(result.checks[2].name).toBe('quota:db-leases')
  ...
  it('accepts a DR secondary (472) as reachable and unsealed', async () => {
    const fetchStub = recordFetch([{ status: 472, body: { replication_dr_mode: 'secondary' } }, TOKEN_OK, LIVE_QUOTA])
    try {
      const result = await healthCheck(ctx())
  ...
  it('scores one missing quota out of two as 75', async () => {
    ...
    const fetchStub = recordFetch([
      HEALTHY,
      TOKEN_OK,
      { status: 200, body: { data: { name: 'first', max_leases: 10 } } },
      NOT_FOUND,
    ])
    try {
