...
/**
 * Health check for lease count quota configuration:
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`quota:${spec.name}`, async () => {
          ...
          if (!live) throw new Error(`Lease count quota "${spec.name}" is not present`)
          return `Lease count quota "${spec.name}" is present`
        }),
      )
    }
  }
