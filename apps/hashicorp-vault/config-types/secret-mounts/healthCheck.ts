...
import { findMount } from './deploy'
import { extractMountSpecs } from './validate'

/**
 * Health check for secret engine configuration:
 ...
 *   3. Every declared secret engine is still mounted with the expected type
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    // Check 3..n: each declared engine is still mounted with the expected type
    const specs = extractMountSpecs(ctx.canvas).filter((s) => s.path && s.type)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`mount:${spec.path}`, async () => {
          const live = await findMount(client, spec.path)
          if (!live) throw new Error(`Secret engine "${spec.path}" is not mounted`)
          const liveType = (live.type ?? '').toLowerCase()
          if (liveType !== spec.type) {
            throw new Error(`Secret engine "${spec.path}" is type "${live.type}", expected "${spec.type}"`)
          }
          return `Secret engine "${spec.path}" (${spec.type}) is mounted`
        }),
      )
    }
  }
