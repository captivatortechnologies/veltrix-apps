...
/**
 * Health check for login-MFA enforcement configuration:
 ...
 *   3. Every declared enforcement still exists
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    // Check 3..n: each declared enforcement still exists
    const specs = extractEnforcementSpecs(ctx.canvas).filter((s) => s.name && s.mfaMethodIds.length > 0)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`enforcement:${spec.name}`, async () => {
          const live = await getEnforcement(client, spec.name)
          if (!live) throw new Error(`Login-MFA enforcement "${spec.name}" is not present`)
          return `Login-MFA enforcement "${spec.name}" is present`
        }),
      )
    }
  }
