...
import { extractSentinelPolicySpecs, sentinelKey, type SentinelScope } from './validate'

/**
 * Health check for Sentinel policy configuration:
 ...
 *   3. Every declared policy is still present
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    // Check 3..n: each declared policy is still present
    const specs = extractSentinelPolicySpecs(ctx.canvas).filter(
    ...
    for (const spec of specs) {
      ...
      checks.push(
        await timedCheck(`policy:${key}`, async () => {
          const live = await getSentinelPolicy(client, scope, spec.name)
          if (!live) throw new Error(`Sentinel policy "${key}" is not present`)
          return `Sentinel policy "${key}" is present`
        }),
      )
    }
  }
