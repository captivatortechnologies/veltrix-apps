...
/**
 * Health check for password generation policy configuration:
 ...
 *   3. Every declared password policy still exists (GET /sys/policies/password/{name})
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 3..n: each declared password policy still exists
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`password-policy:${spec.name}`, async () => {
          ...
          if (!live) throw new Error(`Password policy "${spec.name}" does not exist in Vault`)
