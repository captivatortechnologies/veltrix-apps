...
import { extractNamespaceSpecs } from './validate'

/**
 * Health check for namespace configuration:
 ...
 *   3. Every declared namespace is still present
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    // Check 3..n: each declared namespace is still present
    const specs = extractNamespaceSpecs(ctx.canvas).filter((s) => s.path)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`namespace:${spec.path}`, async () => {
          const live = await getNamespace(client, spec.path)
          if (!live) throw new Error(`Namespace "${spec.path}" is not present`)
          return `Namespace "${spec.path}" is present`
        }),
      )
    }
  }
