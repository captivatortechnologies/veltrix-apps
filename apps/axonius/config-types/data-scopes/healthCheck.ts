...
import { META_ABOUT_RESOURCE } from '../saved-queries/_shared'

/**
 * Health for the data-scopes config = Axonius answers on its REST API with the
 * configured API key + secret. Read-only: GET api/settings/meta/about. Any
 * response below 500 counts as reachable (auth nuances and the Data Scopes
 * feature-flag surface at deploy time, not here).
 */
