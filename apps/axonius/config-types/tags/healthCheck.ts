...
/**
 * Health for the tags config = Axonius answers on its REST API with the configured
 * API key + secret. Read-only: GET api/settings/meta/about. Any response below 500
 * counts as reachable (auth nuances surface at deploy time, not here). Verify the
 * endpoint against a live Axonius tenant.
 */
