...
**App:** `axonius` · **Category:** COMPLIANCE · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Axonius API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
<sub>Applies to: 8 config types · 6 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Axonius API
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + saved-querieswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + saved-queriesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: saved-querieswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
