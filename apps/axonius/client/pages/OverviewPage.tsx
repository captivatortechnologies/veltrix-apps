...
/**
 * What this app manages in an Axonius tenant, rendered with the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/axonius/meta')
  ...
  if (loading) return <Spinner label="Loading Axonius app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Axonius (CAASM — Cyber Asset Attack Surface Management) configuration as code. Author a
          configuration in the Configuration Canvas and deploy it through the pipeline — validate, deploy,
          health check, drift detection and rollback are handled per configuration type. Saved queries are
          applied over the Axonius REST API (HTTPS 443, self-signed tolerated) using an API key and secret.
        </p>
