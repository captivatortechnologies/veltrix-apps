// ========================================================================
// Axonius App — Server Entry Module
//
// Registers Axonius-specific API routes as a Fastify plugin, prefixed with
// /api/apps/axonius/ and protected by app-level auth + permission middleware.
//
// The app is intentionally read-only on the platform side: saved-query authoring
// happens in the Configuration Canvas, and every write to the Axonius tenant goes
// through the pipeline handlers (Axonius REST API). These routes expose read-only
// /meta + /settings that back the Overview page.
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('saved-queries', 'read')],
