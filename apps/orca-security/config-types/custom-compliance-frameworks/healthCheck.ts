...
/**
 * Health for custom compliance frameworks = Orca answers on its REST API with
 * the configured API token. Read-only probe: GET /api/alerts/catalog/category
 * (a small authenticated payload). A 2xx proves the token is accepted for this
 * tenant.
 */
