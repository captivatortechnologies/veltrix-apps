...
**App:** `orca-security` · **Category:** CLOUD · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Orca Security API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 11 config types · 7 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Orca Security API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + custom-alertswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + custom-alertsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: custom-alertswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
