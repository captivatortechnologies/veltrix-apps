...
/**
 * Shows what this app manages in an Orca tenant, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/orca-security/meta')
  ...
  if (loading) return <Spinner label="Loading Orca Security app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Orca Security (agentless CNAPP / CSPM) configuration as code through the Orca REST API.
          Create a configuration in the Configuration Canvas and deploy it through the pipeline — validate,
          deploy, health check, drift detection and rollback are all handled per configuration type. The app
          authenticates with an Orca API token against your tenant's regional API endpoint.
        </p>
