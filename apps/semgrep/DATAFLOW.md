...
**App:** `semgrep` · **Category:** COMPLIANCE · **Version:** 0.2.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Semgrep API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + projectswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + projectsread
  rbac-->>pipeline: permitted ✓ / denied
