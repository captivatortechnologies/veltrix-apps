...
**App:** `authentik` · **Category:** IAM · **Version:** 0.4.0  
...
<sub>Applies to: 12 config types · 8 API families.</sub>
...
sequenceDiagram
  ...
  participant api as authentik API
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + applicationswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + applicationsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: applicationswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
