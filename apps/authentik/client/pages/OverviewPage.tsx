...
/**
 * What this app manages on an authentik instance, rendered with the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/authentik/meta')
  ...
  if (loading) return <Spinner label="Loading authentik app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages authentik (open-source Identity Provider) configuration as code. Author a configuration
          in the Configuration Canvas and deploy it through the pipeline — validate, deploy, health check,
          drift detection and rollback are handled per configuration type. Applications are applied over
          the authentik Core REST API, authenticated with a static API token.
        </p>
