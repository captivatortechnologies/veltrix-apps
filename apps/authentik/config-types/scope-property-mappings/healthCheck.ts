...
import type { AuthentikScopeMapping } from './_shared'

/** Health for scope-property-mappings: reachability + token, then per-item existence by name. */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  for (const item of items) {
    ...
    try {
      const found = await findByName<AuthentikScopeMapping>(listUrl, token, name, { verifyTls })
      checks.push({
        name: `mapping:${name}`,
        passed: found != null,
        message: found != null ? `Scope mapping "${name}" is present.` : `Scope mapping "${name}" is missing.`,
        latencyMs: Date.now() - itemStarted,
      })
    } catch (error) {
      checks.push({ name: `mapping:${name}`, passed: false, message: error instanceof Error ? error.message : 'Check failed', latencyMs: Date.now() - itemStarted })
    }
  }
