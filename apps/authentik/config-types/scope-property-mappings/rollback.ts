...
import { managedFieldsToPatchBody, type ManagedScopeMappingFields } from './_shared'

/** Undo a scope-mappings deploy from `rollbackData.previous`. Reverse deploy order. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  const { component, credential, connectivity, connectivityProvider, settings } = ctx
  const data = (ctx.rollbackData ?? {}) as {
    previous?: Array<{ name: string; pk: string | null; existed: boolean; prior: ManagedScopeMappingFields | null }>
  }
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      const path = `${base}/propertymappings/provider/scope/${encodeURIComponent(entry.pk)}/`
    ...
    return { success: true, message: `Rolled back scope mappings: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.` }
  } catch (error) {
