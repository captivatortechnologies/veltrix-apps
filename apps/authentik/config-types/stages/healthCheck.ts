...
import { STAGE_ENDPOINT_SEGMENT, readStageType, type AuthentikStage } from './_shared'

/**
 * Health for stages: reachability + token (probed against the identification
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await authentikRequest(`${base}/stages/identification/?page_size=1`, { headers: bearer(token), verifyTls })
    checks.push({
  ...
  for (const item of items) {
    ...
    try {
      const found = await findByName<AuthentikStage>(listUrl, token, name, { verifyTls })
      checks.push({
        name: `stage:${name}`,
        passed: found != null,
        message: found != null ? `Stage "${name}" (${type}) is present.` : `Stage "${name}" (${type}) is missing.`,
        latencyMs: Date.now() - itemStarted,
      })
    } catch (error) {
      checks.push({ name: `stage:${name}`, passed: false, message: error instanceof Error ? error.message : 'Check failed', latencyMs: Date.now() - itemStarted })
    }
  }
