...
import { managedFieldsToPatchBody, STAGE_ENDPOINT_SEGMENT, type ManagedStageFields, type StageType } from './_shared'

/** Undo a stages deploy from `rollbackData.previous`. Reverse deploy order. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  const { component, credential, connectivity, connectivityProvider, settings } = ctx
  const data = (ctx.rollbackData ?? {}) as {
    previous?: Array<{ name: string; type: StageType; pk: string | null; existed: boolean; prior: ManagedStageFields | null }>
  }
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      const path = `${base}/stages/${STAGE_ENDPOINT_SEGMENT[entry.type]}/${encodeURIComponent(entry.pk)}/`
    ...
    return { success: true, message: `Rolled back stages: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.` }
  } catch (error) {
