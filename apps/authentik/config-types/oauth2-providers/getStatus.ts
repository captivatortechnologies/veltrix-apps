...
/** Deployment status for an authentik OAuth2/OpenID providers configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['authentik-server', 'standalone'] })
