...
import { managedFieldsToPatchBody, type ManagedBrandFields } from './_shared'

/** Undo a brands deploy from `rollbackData.previous`. Reverse deploy order. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  const { component, credential, connectivity, connectivityProvider, settings } = ctx
  const data = (ctx.rollbackData ?? {}) as {
    previous?: Array<{ domain: string; pk: string | null; existed: boolean; prior: ManagedBrandFields | null }>
  }
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      const path = `${base}/core/brands/${encodeURIComponent(entry.pk)}/`
    ...
    return { success: true, message: `Rolled back brands: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.` }
  } catch (error) {
