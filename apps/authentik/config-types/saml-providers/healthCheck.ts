...
import type { AuthentikSAMLProvider } from './_shared'

/** Health for saml-providers: reachability + token, then per-item existence by name. */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  for (const item of items) {
    ...
    try {
      const found = await findByName<AuthentikSAMLProvider>(listUrl, token, name, { verifyTls })
      checks.push({
        name: `provider:${name}`,
        passed: found != null,
