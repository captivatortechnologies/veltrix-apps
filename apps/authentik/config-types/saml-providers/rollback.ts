...
import { managedFieldsToPatchBody, type ManagedSAMLProviderFields } from './_shared'

/**
 * Undo a SAML providers deploy from `rollbackData.previous`: PATCH prior
 * managed fields back, or DELETE a provider this deploy created. Reverse
 * deploy order.
 */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  const { component, credential, connectivity, connectivityProvider, settings } = ctx
  const data = (ctx.rollbackData ?? {}) as {
    previous?: Array<{ name: string; pk: number | null; existed: boolean; prior: ManagedSAMLProviderFields | null }>
  }
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      const path = `${base}/providers/saml/${entry.pk}/`
    ...
    return { success: true, message: `Rolled back SAML providers: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.` }
  } catch (error) {
