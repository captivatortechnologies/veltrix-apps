...
import { buildCreateBody, buildPatchBody, snapshotManagedFields, type AuthentikProxyProvider, type ManagedProxyProviderFields } from './_shared'

/**
 * Deploy authentik Proxy Providers over the Core API:
 *   read (identity): GET   /providers/proxy/?name=<name>   → list + exact-match
 *   create:           POST  /providers/proxy/                a `ProxyProviderRequest`
 *   update:           PATCH /providers/proxy/{pk}/            a `PatchedProxyProviderRequest`
 */
interface PreviousEntry {
  name: string
  pk: number | null
  existed: boolean
  prior: ManagedProxyProviderFields | null
}
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  try {
    for (const item of items) {
      ...
      const existing = await findByName<AuthentikProxyProvider>(listUrl, token, name, { verifyTls })
    ...
    return {
      success: true,
      message: `Applied ${applied.length} proxy provider(s): ${applied.join(', ') || '(none)'}`,
      artifacts: { applied },
    ...
    return {
      success: false,
      message: `authentik proxy provider deploy failed after ${applied.length} provider(s): ${error instanceof Error ? error.message : 'Unknown error'}`,
      artifacts: { applied },
      rollbackData: { previous },
    }
  }
