...
import type { AuthentikProxyProvider } from './_shared'

/** Health for proxy-providers: reachability + token, then per-item existence by name. */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  for (const item of items) {
    ...
    try {
      const found = await findByName<AuthentikProxyProvider>(listUrl, token, name, { verifyTls })
      checks.push({
        name: `provider:${name}`,
        passed: found != null,
      ...
      checks.push({ name: `provider:${name}`, passed: false, message: error instanceof Error ? error.message : 'Check failed', latencyMs: Date.now() - itemStarted })
    }
  }
