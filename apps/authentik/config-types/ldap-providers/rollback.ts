...
import { managedFieldsToPatchBody, type ManagedLDAPProviderFields } from './_shared'

/** Undo an LDAP providers deploy from `rollbackData.previous`. Reverse deploy order. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  const { component, credential, connectivity, connectivityProvider, settings } = ctx
  const data = (ctx.rollbackData ?? {}) as {
    previous?: Array<{ name: string; pk: number | null; existed: boolean; prior: ManagedLDAPProviderFields | null }>
  }
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      const path = `${base}/providers/ldap/${entry.pk}/`
    ...
    return { success: true, message: `Rolled back LDAP providers: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.` }
  } catch (error) {
