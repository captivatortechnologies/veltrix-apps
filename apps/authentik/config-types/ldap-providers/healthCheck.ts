...
import type { AuthentikLDAPProvider } from './_shared'

/** Health for ldap-providers: reachability + token, then per-item existence by name. */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  for (const item of items) {
    ...
    try {
      const found = await findByName<AuthentikLDAPProvider>(listUrl, token, name, { verifyTls })
      checks.push({
        name: `provider:${name}`,
        passed: found != null,
