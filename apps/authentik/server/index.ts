// ========================================================================
// authentik App — Server Entry Module
//
// Registers authentik-specific API routes as a Fastify plugin, prefixed with
// /api/apps/authentik/ and protected by app-level auth + permission middleware.
// Configuration authoring happens in the Configuration Canvas and every config
// write goes through the pipeline handlers (authentik Core REST API). This
// foundation exposes read-only /meta + /settings routes that back the Overview
// page; the per-connection connectivity test lives in handlers/testConnection.ts.
// The app additionally OWNS BYOL stack provisioning: the /byol routes derive a
// resource plan from a stack's topology, persist it, and emit provisioning events
// for downstream workers — the declarative provisioning foundation ships in
// infra/spec.ts.
// ========================================================================
...
async function emit(events: AppEventPublisher, topic: string, payload: unknown): Promise<void> {
  try {
    ...
    console.error(`[authentik] publish ${topic} failed:`, err)
  }
}
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('applications', 'read')],
  ...
  // service token holding `usage:write`. Node-hours only — authentik has no
