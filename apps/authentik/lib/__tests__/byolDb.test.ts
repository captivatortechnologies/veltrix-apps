...
test('createByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  await createByol(db, 'cust-1', {
    ...baseInput,
    nodeTiers: [
      { key: 'server', count: 2, placement: null },
      { key: 'worker', count: 3, placement: null },
    ],
  })

  const insert = calls.find((c) => c.sql.includes('INSERT INTO authentik_byol_infrastructure'))
  assert.ok(insert)
...
test('updateByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  const update = calls.find((c) => c.sql.includes('UPDATE authentik_byol_infrastructure'))
  assert.ok(update)
