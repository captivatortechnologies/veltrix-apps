...
// SDK's <ByolInfrastructureManager> sends (server / worker), per-tier minimums
...
test('readByol defaults single-instance topology fields', () => {
  const { data } = readByol({
    ...
    tiers: [{ key: 'server', count: 1 }, { key: 'worker', count: 1 }],
  })
...
test('readByol reads per-tier counts from the tiers array', () => {
  ...
  assert.equal(tiers.find((t) => t.key === 'worker')!.count, 3)
})

test('readByol emits nodeTiers in [server, worker] order regardless of input order', () => {
  const { data } = readByol(
    distributedBody({
      tiers: [
        { key: 'worker', count: 3 },
  ...
  const tiers = data.nodeTiers as Array<{ key: string; count: number }>
  assert.deepEqual(tiers.map((t) => t.key), ['server', 'worker'])
...
test('readByol defaults a missing tier to a count of 1', () => {
  const { data, error } = readByol({
    name: 'Dev',
    deploymentType: 'single',
    tiers: [{ key: 'server', count: 4 }],
  })
  assert.equal(error, undefined)
  const tiers = data.nodeTiers as Array<{ key: string; count: number }>
  assert.deepEqual(tiers, [
    { key: 'server', count: 4, placement: null },
...
test('readByol requires at least 1 Server nodes', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'server', count: 0 }, { key: 'worker', count: 1 }] }))
  assert.match(error ?? '', /Server nodes must be at least 1/)
})

test('readByol requires at least 1 Worker nodes', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'server', count: 1 }, { key: 'worker', count: 0 }] }))
  assert.match(error ?? '', /Worker nodes must be at least 1/)
})

test('readByol accepts a distributed stack with one server and one worker', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'server', count: 1 }, { key: 'worker', count: 1 }] }))
  assert.equal(error, undefined)
})
...
test('readByol allows a single-node deployment', () => {
  ...
  assert.equal(error, undefined)
  const tiers = data.nodeTiers as Array<{ key: string; count: number }>
  assert.equal(tiers.find((t) => t.key === 'worker')!.count, 1)
})
...
test('readByol accepts a valid multi-site placement on the worker tier', () => {
  const { data, error } = readByol(
    distributedBody({
      tiers: [
        { key: 'server', count: 2 },
  ...
  assert.equal(tiers.find((t) => t.key === 'worker')!.placement.mode, 'multi-site')
})
...
test('readByol rejects placement whose percentages do not total 100, naming the tier', () => {
  ...
  assert.match(error ?? '', /Worker nodes placement: .*total 100/)
})
...
test('readByol rejects more sites than nodes on the server tier', () => {
  ...
  assert.match(error ?? '', /Server nodes placement: .*Too many sites/)
})
...
test('readByol rejects region-granularity placement unless the network is dedicated (BYOC)', () => {
  const { error } = readByol(
    distributedBody({
      tiers: [{ key: 'server', count: 2 }, { key: 'worker', count: 4, placement: regionPlacement }],
    }),
  )
