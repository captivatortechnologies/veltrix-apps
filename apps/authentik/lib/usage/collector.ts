...
//   * ingest_gb: from an INJECTED poller. Skipped when no poller is supplied or
//     it returns null for an infra. authentik has no ingest-volume metric (it is
//     an identity provider, not an ingest pipeline), so the poller is unused
//     today — node-hours are the live dimension (the poller stays wired so a
//     future authentik volume metric could plug straight in).
//
 ...
 * omit it to record node-hours only (the current default — authentik has no
 * built-in ingest-volume metric).
 */
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM authentik_byol_infrastructure',
  )
