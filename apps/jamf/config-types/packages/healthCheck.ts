...
import { extractPackageSpecs, indexPackagesByName, packageKey, type LivePackage } from './validate'
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('jamf_reachable', async () => {
    const live = await listPackages(client, ctx.settings)
  ...
  if (reachable.passed && reachable.live) {
    const byName = indexPackagesByName(reachable.live)
    for (const spec of specs) {
      const present = byName.has(packageKey(spec.name))
      checks.push({
        name: `package:${spec.name}`,
        passed: present,
        message: present ? `Package "${spec.name}" is present` : `Package "${spec.name}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: LivePackage[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: LivePackage[] }> {
