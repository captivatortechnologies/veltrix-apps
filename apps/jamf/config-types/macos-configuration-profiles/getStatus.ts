...
/**
 * Report the current deployment status of Jamf Pro macOS configuration
 * profile configurations. Reads deployment and component records through
 * the platform data API.
 */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['jamf-pro-server'] })
