...
import { categoryKey, extractCategorySpecs, indexCategoriesByName, type LiveCategory } from './validate'

/**
 * Health check for Jamf Pro category configuration:
 *   1. Jamf Pro reachability + credential validity (a categories list)
 *   2. Every declared category (by name) still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed && reachable.live) {
    const byName = indexCategoriesByName(reachable.live)
    for (const spec of specs) {
      const present = byName.has(categoryKey(spec.name))
      checks.push({
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: LiveCategory[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: LiveCategory[] }> {
