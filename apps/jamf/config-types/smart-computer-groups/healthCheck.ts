...
import { listComputerGroups, type ComputerGroupRef } from './deploy'
import { extractSmartGroupSpecs, groupKey, indexGroupsByName } from './validate'

/**
 * Health check for Jamf Pro smart-group configuration:
 *   1. Jamf Pro Classic API reachability + credential validity (a groups list)
 *   2. Every declared group (by name) still exists in the tenant, and is
 *      still a smart group (`is_smart`) — flags a group someone converted to
 *      static outside Veltrix
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed && reachable.live) {
    const byName = indexGroupsByName(reachable.live)
    for (const spec of specs) {
      const found = byName.get(groupKey(spec.name))
      const passed = !!found && found.isSmart
      checks.push({
        name: `group:${spec.name}`,
        passed,
        message: !found
          ? `Smart group "${spec.name}" is missing`
          : found.isSmart
            ? `Smart group "${spec.name}" is present`
            : `Group "${spec.name}" exists but is no longer a smart group`,
      })
    }
  }
