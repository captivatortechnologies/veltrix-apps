...
import { departmentKey, extractDepartmentSpecs, indexDepartmentsByName, type LiveDepartment } from './validate'
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed && reachable.live) {
    const byName = indexDepartmentsByName(reachable.live)
    for (const spec of specs) {
      const present = byName.has(departmentKey(spec.name))
      checks.push({
        name: `department:${spec.name}`,
        passed: present,
        message: present ? `Department "${spec.name}" is present` : `Department "${spec.name}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: LiveDepartment[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: LiveDepartment[] }> {
