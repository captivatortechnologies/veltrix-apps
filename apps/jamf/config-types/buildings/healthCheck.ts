...
import { buildingKey, extractBuildingSpecs, indexBuildingsByName, type LiveBuilding } from './validate'
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed && reachable.live) {
    const byName = indexBuildingsByName(reachable.live)
    for (const spec of specs) {
      const present = byName.has(buildingKey(spec.name))
      checks.push({
        name: `building:${spec.name}`,
        passed: present,
        message: present ? `Building "${spec.name}" is present` : `Building "${spec.name}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: LiveBuilding[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: LiveBuilding[] }> {
