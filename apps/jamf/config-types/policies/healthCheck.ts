...
import { extractPolicySpecs, indexPoliciesByName, policyKey } from './validate'
import type { ClassicRef } from '../../lib/jamfClassicXml'

/**
 * Health check for Jamf Pro policy configuration:
 *   1. Jamf Pro Classic API reachability + credential validity (a policy list)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed && reachable.live) {
    const byName = indexPoliciesByName(reachable.live)
    for (const spec of specs) {
      const present = byName.has(policyKey(spec.name))
      checks.push({
