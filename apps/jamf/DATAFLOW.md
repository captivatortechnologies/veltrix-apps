...
**App:** `jamf` · **Category:** Endpoint Management · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Jamf API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
sequenceDiagram
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + scriptswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + scriptsread
  rbac-->>pipeline: permitted ✓ / denied
