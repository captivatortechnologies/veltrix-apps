...
/**
 * Report the current deployment status of a Security Monitoring Rule
 * configuration. Reads deployment and component records through the platform
 * data API — no Datadog API call is made here (that's healthCheck's job).
 */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['datadog-org'] })
