# 🔥 Palo Alto Panorama — Request Flows
...
**App:** `palo-alto-panorama` · **Category:** NETWORK · **Version:** 1.2.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Palo Alto Panorama API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 13 config types · 1 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Palo Alto Panorama API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + panorama-tagswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + panorama-tagsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: panorama-tagswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
