...
**App:** `onepassword` · **Category:** IAM · **Version:** 0.1.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **1Password API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + userswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + usersread
  rbac-->>pipeline: permitted ✓ / denied
