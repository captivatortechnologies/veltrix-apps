// ========================================================================
// 1Password App - Server Entry Module
//
// Registers app-specific API routes as a Fastify plugin, prefixed with
// /api/apps/onepassword/ and protected by app-level auth + permission
// middleware. Read-only on the platform side: authoring happens in the
// Configuration Canvas, and every write to 1Password goes through the
// pipeline handlers (the SCIM Bridge API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('users', 'read')],
