...
**App:** `cribl` · **Category:** SIEM · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Cribl API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
<sub>Applies to: 22 config types · 6 API families.</sub>
...
sequenceDiagram
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + pipelineswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + pipelinesread
  rbac-->>pipeline: permitted ✓ / denied
