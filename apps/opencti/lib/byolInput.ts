...
//   • tiers['platform'] → OpenCTI GraphQL/web platform nodes (ALB targets)
//   • tiers['worker']   → ingest / enrichment workers
//   • tiers['search']   → Elasticsearch / OpenSearch search nodes
//
...
// `nodeTiers` in declared order [platform, worker, search].
// =============================================================================

import { normalizeControlPlaneLayout, parsePlacement, validatePlacement, type ClusterPlacement } from './byolPlacement'

/** OpenCTI's three scalable BYOL node tiers, in persisted/display order. */
const TIER_KEYS = ['platform', 'worker', 'search'] as const
...
const TIER_LABELS: Record<TierKey, string> = {
  ...
  search: 'Search nodes (Elasticsearch)',
}

/**
 * Minimum node count per tier. Platform + workers stay permissive (≥1). A
 * DISTRIBUTED Elasticsearch / OpenSearch search cluster needs at least three
 * nodes to form a real quorum, so the search minimum is 3 for distributed
 * deployments (a single-node / eval deployment collapses to one all-in-one box,
 * so every tier is effectively ≥1 there).
 */
function tierMinimum(key: TierKey, isDistributed: boolean): number {
  if (key === 'search' && isDistributed) return 3
  return 1
}
...
export function readByol(body: any): { data: Record<string, unknown>; error?: string } {
  ...
  // "Distributed" is the multi-node OpenCTI stack (platform + workers + search +
  // Redis + RabbitMQ + object storage on separate nodes); single instance is the
  // all-in-one box.
  ...
  // Carried on the record for SDK-form round-tripping; unused by OpenCTI's
  // resource plan (OpenCTI has no consolidated control plane / forwarder tier).
