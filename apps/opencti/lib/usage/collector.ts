...
//     it returns null for an infra. OpenCTI has no simple built-in ingest metric,
//     so the poller is unused today — node-hours are the live dimension (the
//     poller stays wired so a future OpenCTI volume metric can plug straight in).
//
 ...
 * omit it to record node-hours only (the current default — OpenCTI has no simple
...
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM opencti_byol_infrastructure',
  )
