...
//
// NODE_TIERS-NATIVE: the infrastructure carries its per-tier node counts +
// placement ONLY in the `node_tiers` JSONB column (there are no Splunk-shaped
// indexer_count / search_head_count columns). `mapByol` surfaces it as `tiers`,
// returning `undefined` — never `[]` — when empty so the SDK's legacy
...
export function parseNodeTiers(value: unknown): NodeTierDto[] {
...
export interface ByolDto {
  ...
  /**
   * Generic per-tier node counts + placement (node_tiers-native). `undefined`
   * (never `[]`) when empty, so the SDK's legacy tier fallback stays falsy.
   */
