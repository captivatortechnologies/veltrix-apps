...
test('createByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  await createByol(db, 'cust-1', {
    ...baseInput,
    nodeTiers: [
      { key: 'platform', count: 2, placement: null },
      { key: 'worker', count: 2, placement: null },
      { key: 'search', count: 3, placement: null },
    ],
  })

  const insert = calls.find((c) => c.sql.includes('INSERT INTO opencti_byol_infrastructure'))
  assert.ok(insert)
...
test('updateByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  await updateByol(db, 'i1', {
    ...baseInput,
    nodeTiers: [
      { key: 'platform', count: 5, placement: null },
      { key: 'worker', count: 4, placement: null },
      { key: 'search', count: 3, placement: null },
    ],
  })

  const update = calls.find((c) => c.sql.includes('UPDATE opencti_byol_infrastructure'))
  assert.ok(update)
