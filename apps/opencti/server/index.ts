// ========================================================================
// OpenCTI App — Server Entry Module
//
// Registers OpenCTI-specific API routes as a Fastify plugin, prefixed with
// /api/apps/opencti/ and protected by app-level auth + permission middleware.
// Marking-definition CONFIGURATION authoring happens in the Configuration Canvas
// and every config write goes through the pipeline handlers (OpenCTI GraphQL API).
...
async function emit(events: AppEventPublisher, topic: string, payload: unknown): Promise<void> {
  try {
    ...
    console.error(`[opencti] publish ${topic} failed:`, err)
  }
}
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('marking-definitions', 'read')],
  ...
  // service token holding `usage:write`. Node-hours only — OpenCTI has no simple
