...
/**
 * Health for kill-chain-phases config = OpenCTI answers on its GraphQL API with
 * the configured API token. Read-only: `about { version }` (falls back to
 * `me { id name }`). A returned version/identity proves reachability AND that
 * the token authenticates. Verified against the OpenCTI GraphQL backend schema
 * (opencti-platform/opencti, config/schema/opencti.graphql).
 */
