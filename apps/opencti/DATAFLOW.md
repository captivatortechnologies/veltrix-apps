...
**App:** `opencti` · **Category:** Threat Intelligence · **Version:** 0.4.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **OpenCTI API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
<sub>Applies to: 17 config types · 7 API families.</sub>
...
sequenceDiagram
  ...
  participant api as OpenCTI API
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + marking-definitionswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + marking-definitionsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: marking-definitionswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
