...
/**
 * What this app manages in an OpenCTI instance, rendered with the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/opencti/meta')
  ...
  if (loading) return <Spinner label="Loading OpenCTI app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages OpenCTI (open-source cyber threat-intelligence platform) configuration as code. Author a
          configuration in the Configuration Canvas and deploy it through the pipeline — validate, deploy,
          health check, drift detection and rollback are handled per configuration type. Marking definitions
          are applied over the OpenCTI GraphQL API (HTTPS 443, Bearer token, self-signed tolerated).
        </p>
