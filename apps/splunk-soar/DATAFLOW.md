...
**App:** `splunk-soar` · **Category:** SOAR · **Version:** 1.2.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Splunk SOAR API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
sequenceDiagram
  ...
  participant api as Splunk SOAR API
  ...
  Note over rbac: componentread · credentialread + connectionwrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + connectionread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: connectionwrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
