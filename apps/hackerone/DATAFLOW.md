...
**App:** `hackerone` · **Category:** COMPLIANCE · **Version:** 0.2.0  
...
sequenceDiagram
  ...
  participant api as HackerOne API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + structured-scopeswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + structured-scopesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: structured-scopeswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
