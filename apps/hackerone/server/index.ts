// ========================================================================
// HackerOne App — Server Entry Module
//
// Registers HackerOne-specific API routes as a Fastify plugin, prefixed with
// /api/apps/hackerone/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration Canvas,
// and every write goes through the pipeline handlers (HackerOne API). No DB / no
// BYOL — HackerOne is a pure SaaS API.
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('structured-scopes', 'read')],
