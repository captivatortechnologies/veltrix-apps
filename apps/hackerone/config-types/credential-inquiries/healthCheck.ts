...
/**
 * Health for the Credential Inquiries config = HackerOne answers on the API with
 * the configured Basic-auth credential. Read-only probe: GET /me/programs?page[size]=1.
 * A 2xx (or any response below 500) counts as reachable; 401 means the API
 * identifier / token pair is bad. Note: the credential-inquiry write endpoints
 * additionally require the Team Management permission, which this probe does not
 * exercise.
 */
