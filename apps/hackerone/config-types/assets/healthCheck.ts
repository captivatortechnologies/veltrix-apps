...
/**
 * Health for the Assets config = HackerOne answers on the API with the
 * configured Basic-auth credential. Read-only probe: GET /me/programs?page[size]=1
 * (the same probe every config type in this app uses — /me/organizations would
 * work equally well, but reusing the already-flagged-healthy /me/programs probe
 * avoids exercising a second, differently-permissioned endpoint just for health).
