# 🛡️ Tenable Vulnerability Management — Request Flows
...
**App:** `tenable-vm` · **Category:** COMPLIANCE · **Version:** 1.3.1  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Tenable Vulnerability Management API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 18 config types · 7 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Tenable Vulnerability Management API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + scanswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + scansread
  rbac-->>pipeline: permitted ✓ / denied
