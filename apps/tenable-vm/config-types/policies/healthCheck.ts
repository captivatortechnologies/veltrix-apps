...
/**
 * Health check for scan-policy configuration:
 *   1. Tenable API reachability + credential validity (GET /server/status)
 *   2. Every declared policy still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared policy still exists in the tenant
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`policy:${spec.name}`, async () => {
          ...
          return `Policy "${spec.name}" is present (id ${live.id})`
        }),
      )
    }
  }
