...
/**
 * Detect drift between the deployed agent exclusion configuration and the live
 * tenant state. Looks up each declared exclusion by name in its cloud scanner
 * and diffs the managed fields (members + the normalized schedule). A disabled
 * exclusion collapses the schedule shape, so enabled state is compared before
 * any window fields.
 */
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  const specs = extractAgentExclusionSpecs(ctx.deployedConfig).filter((s) => s.name)
...
function diffSchedule(diffs: DriftDiff[], spec: AgentExclusionSpec, live: LiveAgentExclusion): void {
