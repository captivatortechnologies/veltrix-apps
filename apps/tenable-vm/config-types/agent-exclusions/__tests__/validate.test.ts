import validate, { extractAgentExclusionSpecs } from '../validate'
...
/** A fully valid enabled weekly agent exclusion, overridable per test. */
...
describe('Tenable Agent Exclusions Validate Handler', () => {
  ...
  it('validates a valid enabled agent exclusion', async () => {
  ...
  it('requires members for an enabled agent exclusion', async () => {
  ...
  it('accepts a disabled ("Always On") agent exclusion with no schedule times', async () => {
    const result = await validate(
      makeCtx([
        {
          name: 'Excl',
  ...
  it('rejects duplicate agent exclusion names (case-insensitive)', async () => {
    const result = await validate(
      makeCtx([
...
describe('extractAgentExclusionSpecs', () => {
  it('normalizes members (newlines/commas -> comma string) and weekday tags', () => {
    ...
    const specs = extractAgentExclusionSpecs({
      id: 's',
      canvasId: 'c',
      version: 1,
      name: 'n',
    ...
    expect(specs[0].scannerId).toBe('7')
  ...
  it('defaults enabled to true and scannerId to "1" when unset', () => {
