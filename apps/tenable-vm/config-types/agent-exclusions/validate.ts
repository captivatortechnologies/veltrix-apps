...
// --- Agent Exclusions API constraints ----------------------------------------

/** schedule.rrules.freq is an enum on the Tenable Agent Exclusions API. */
...
/** Cloud scanner id an agent exclusion lives under when none is supplied. */
...
export interface AgentExclusionSpec {
  sectionName: string
  name: string
  /** Cloud scanner id the exclusion lives under (path segment); defaults to "1". */
  scannerId: string
  description?: string
...
/**
 * Shape of an agent exclusion returned by
 * GET /scanners/{scannerId}/agent-exclusions and .../agent-exclusions/{id}.
 */
export interface LiveAgentExclusion {
  id?: number | string
  uuid?: string
  name?: string
  description?: string
...
/** Each canvas item describes one Tenable agent scan exclusion. */
export function extractAgentExclusionSpecs(canvas: CanvasSnapshot): AgentExclusionSpec[] {
...
/**
 * Validate agent exclusion configurations against Agent Exclusions API
 * constraints: naming, member targeting, and the schedule shape (datetime
 * format, frequency enum, interval/day-of-month ranges, weekday tokens). A
 * disabled ("Always On") exclusion skips all schedule validation.
 */
export default async function validate(ctx: PipelineContext): Promise<ValidationResult> {
  ...
  const specs = extractAgentExclusionSpecs(ctx.canvas)
  ...
  for (const spec of specs) {
    ...
    if (!spec.name) {
      errors.push({ field: `${prefix}.name`, message: 'Agent exclusion name is required', code: 'required' })
    } else {
      if (spec.name.length > MAX_EXCLUSION_NAME_LENGTH) {
        errors.push({
          field: `${prefix}.name`,
          message: `Agent exclusion name must be ${MAX_EXCLUSION_NAME_LENGTH} characters or fewer`,
          code: 'max_length',
        })
      }
      ...
      if (seenNames.has(key)) {
        errors.push({
          field: `${prefix}.name`,
          message: `Duplicate agent exclusion "${spec.name}" — each exclusion may only be declared once per canvas`,
          code: 'duplicate_name',
        })
      }
      seenNames.add(key)
