...
/**
 * Health check for folder configuration:
 *   1. Tenable API reachability + credential validity (GET /server/status)
 *   2. Every declared folder still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared folder still exists (re-found by its name)
  if (reachable.passed) {
    const specs = extractFolderSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      const label = spec.name
      checks.push(
        await timedCheck(`folder:${label}`, async () => {
          const live = await findFolder(client, spec.name)
          if (!live) throw new Error(`Folder "${label}" does not exist in the tenant`)
          return `Folder "${label}" is present`
        }),
      )
    }
  }
