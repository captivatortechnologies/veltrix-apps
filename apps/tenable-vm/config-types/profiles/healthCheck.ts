...
import { findProfile } from './deploy'
import { extractProfileSpecs } from './validate'

/**
 * Health check for profile configuration:
 *   1. Tenable API reachability + credential validity (GET /server/status)
 *   2. Every declared profile still exists in the tenant (re-found by name)
