 ...
 *   2. Every declared role still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared role still exists (re-found by name)
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`role:${spec.name}`, async () => {
          ...
          if (!live) throw new Error(`Role "${spec.name}" does not exist in the tenant`)
