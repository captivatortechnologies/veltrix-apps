...
import { extractExclusionSpecs } from './validate'
 ...
 *   2. Every declared exclusion still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared exclusion still exists in the tenant
  if (reachable.passed) {
    const specs = extractExclusionSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`exclusion:${spec.name}`, async () => {
          const live = await findExclusion(client, spec.name)
          if (!live) throw new Error(`Exclusion "${spec.name}" does not exist in the tenant`)
          return `Exclusion "${spec.name}" is present (id ${live.id})`
        }),
      )
    }
  }
