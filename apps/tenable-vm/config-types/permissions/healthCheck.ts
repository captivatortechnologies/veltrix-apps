...
import { findPermissionByName } from './deploy'
import { extractPermissionSpecs } from './validate'

/**
 * Health check for permission configuration:
 *   1. Tenable API reachability + credential validity (GET /server/status)
 *   2. Every declared permission still exists in the tenant (matched by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared permission still exists (re-found by name)
  if (reachable.passed) {
    const specs = extractPermissionSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      const label = spec.name
      checks.push(
        await timedCheck(`permission:${label}`, async () => {
          const live = await findPermissionByName(client, spec.name)
          if (!live) throw new Error(`Permission "${label}" does not exist in the tenant`)
          return `Permission "${label}" is present`
        }),
      )
    }
  }
