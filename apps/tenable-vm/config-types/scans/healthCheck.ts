...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared scan still exists in the tenant
  if (reachable.passed) {
    const specs = extractScanSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`scan:${spec.name}`, async () => {
          const live = await findScan(client, spec.name)
          if (!live || live.id === undefined) {
            throw new Error(`Scan "${spec.name}" does not exist in the tenant`)
          }
          return `Scan "${spec.name}" is present (id ${live.id})`
        }),
      )
    }
  }
