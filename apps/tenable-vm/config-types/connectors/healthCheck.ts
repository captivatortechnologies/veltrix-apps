 ...
 *   2. Every declared connector still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared connector still exists in the tenant
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`connector:${spec.name}`, async () => {
          ...
          if (!live) throw new Error(`Connector "${spec.name}" does not exist in the tenant`)
          return `Connector "${spec.name}" is present (id ${live.id})`
        }),
      )
    }
  }
