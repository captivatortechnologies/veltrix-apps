...
/**
 * Health check for asset-attribute configuration:
 *   1. Tenable API reachability + credential validity (GET /server/status)
 *   2. Every declared attribute definition still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared attribute definition still exists (found by name)
  if (reachable.passed) {
    const specs = extractAttributeSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      const label = spec.name
      checks.push(
        await timedCheck(`attribute:${label}`, async () => {
          const live = await findAttribute(client, spec.name)
          if (!live) throw new Error(`Attribute "${label}" does not exist in the tenant`)
          return `Attribute "${label}" is present`
        }),
      )
    }
  }
