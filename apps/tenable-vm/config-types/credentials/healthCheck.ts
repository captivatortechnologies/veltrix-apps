...
/**
 * Health check for credential configuration:
 *   1. Tenable API reachability + credential validity (GET /server/status)
 *   2. Every declared credential (by name) still exists in the tenant
 * Score is the percentage of passed checks (0–100).
 *
 * NOTE: this can only confirm a credential EXISTS — the secret settings are
 * write-only and cannot be read back, so their correctness is not verifiable
 * here.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared credential still exists (re-found by name)
  if (reachable.passed) {
    const specs = extractCredentialSpecs(ctx.canvas).filter((s) => s.name && s.type)
    for (const spec of specs) {
      const label = spec.name
      checks.push(
        await timedCheck(`credential:${label}`, async () => {
          const live = await findCredentialByName(client, spec.name)
          if (!live) throw new Error(`Credential "${label}" does not exist in the tenant`)
          return `Credential "${label}" is present`
        }),
      )
    }
  }
