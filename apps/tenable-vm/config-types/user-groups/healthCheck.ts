 ...
 *   2. Every declared user group (by name) still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared user group still exists (re-found by its name)
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`user-group:${spec.name}`, async () => {
          ...
          return `User group "${spec.name}" is present`
        }),
      )
    }
  }
