...
export default async function onInstall({ appId }: AppHookContext): Promise<void> {
  ...
  console.log(
    '[Defender for Endpoint] No seeding required. Next steps: create an Entra app registration with ' +
      ...
      '"Tenant ID" and "Azure Cloud" app settings. Add Machine.ReadWrite.All for machine tags / device ' +
      'values / scan definitions, Library.Manage for the Live Response library, and (commercial cloud ' +
      'only) the Graph CustomDetection.ReadWrite.All permission for custom detection rules — grant only ' +
      'the ones the config types you plan to use actually need.',
  )
}
