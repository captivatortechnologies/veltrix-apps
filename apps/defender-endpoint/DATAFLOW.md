# 🛡️ Microsoft Defender for Endpoint — Request Flows
...
**App:** `defender-endpoint` · **Category:** EDR · **Version:** 1.4.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Microsoft Defender for Endpoint API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 8 config types · 5 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Microsoft Defender for Endpoint API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + mde-file-indicatorswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + mde-file-indicatorsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: mde-file-indicatorswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
