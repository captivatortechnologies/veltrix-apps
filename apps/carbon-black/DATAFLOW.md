# 🖤 VMware Carbon Black — Request Flows
...
**App:** `carbon-black` · **Category:** EDR · **Version:** 0.6.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **VMware Carbon Black API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 12 config types · 2 API families.</sub>
...
sequenceDiagram
  ...
  participant api as VMware Carbon Black API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + reputation-overrideswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + reputation-overridesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: reputation-overrideswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
