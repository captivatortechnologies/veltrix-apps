...
describe('carbon-black watchlists healthCheck handler', () => {
  ...
  it('reports healthy after reaching the watchlist-manager endpoint', async () => {
    await withFetch([cbJson({ results: [] })], async (calls) => {
      ...
      expect(result.checks[0].name).toBe('cbc-watchlists')
