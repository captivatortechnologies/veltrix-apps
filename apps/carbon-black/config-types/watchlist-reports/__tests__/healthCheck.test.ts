...
const SEVERITY = `/threathunter/watchlistmgr/v3/orgs/${ORG_KEY}/reports/severity`

function ctx(opts: Record<string, unknown> = {}) {
  return healthContext({ configTypeId: CONFIG_TYPE, ...opts })
}

describe('carbon-black watchlist-reports healthCheck handler', () => {
  ...
  it('reports healthy after reaching the watchlist-reports endpoint', async () => {
    await withFetch([cbJson({ results: [] })], async (calls) => {
      ...
      // The shared store has no list-all, so severity is the reachability probe.
      expect(calls[0].path).toBe(SEVERITY)
      expect(calls[0].authToken).toBe(AUTH_TOKEN)
      expect(result.checks[0].name).toBe('cbc-watchlist-reports')
  ...
  it('reports unhealthy with the vendor reason when the endpoint rejects', async () => {
    await withFetch([cbError('API key lacks org.watchlists READ', 403)], async () => {
      const result = await healthCheck(ctx())

      expect(result.healthy).toBe(false)
      expect(result.score).toBe(0)
      expect(result.checks[0].passed).toBe(false)
      expect(result.checks[0].message).toContain('API key lacks org.watchlists READ')
