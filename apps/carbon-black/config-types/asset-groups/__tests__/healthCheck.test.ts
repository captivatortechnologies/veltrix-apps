...
describe('carbon-black asset-groups healthCheck handler', () => {
  ...
  it('reports healthy after reaching the asset-groups endpoint', async () => {
    await withFetch([cbJson({ results: [] })], async (calls) => {
      ...
      expect(result.checks[0].name).toBe('cbc-asset-groups')
  ...
  it('reports unhealthy with the vendor reason when the endpoint rejects', async () => {
    await withFetch([cbError('API key lacks org.asset-groups READ', 403)], async () => {
      const result = await healthCheck(ctx())

      expect(result.healthy).toBe(false)
      expect(result.score).toBe(0)
      expect(result.checks[0].passed).toBe(false)
      expect(result.checks[0].message).toContain('API key lacks org.asset-groups READ')
