...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(client.assetGroupsPath())
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'cbc-asset-groups',
    passed,
    message: passed ? 'Reached the Carbon Black asset-groups endpoint' : `Carbon Black error: ${cbErrorMessage(resp)}`,
    latencyMs,
  })
