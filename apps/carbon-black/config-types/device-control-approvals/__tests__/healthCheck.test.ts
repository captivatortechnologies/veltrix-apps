...
describe('carbon-black device-control-approvals healthCheck handler', () => {
  ...
  it('reports healthy after reaching the device-control approvals endpoint', async () => {
    await withFetch([searchPage([])], async (calls) => {
      ...
      // A single-row probe — a health check must not page the whole collection.
      expect(objectBody(calls[0])).toEqual({ criteria: {}, start: 0, rows: 1 })
      expect(result.checks[0].name).toBe('cbc-device-control-approvals')
