...
describe('carbon-black policies healthCheck handler', () => {
  ...
  it('reports healthy after reaching the policy-service summary endpoint', async () => {
    await withFetch([cbJson({ policies: [] })], async (calls) => {
      ...
      expect(result.checks[0].name).toBe('cbc-policies')
