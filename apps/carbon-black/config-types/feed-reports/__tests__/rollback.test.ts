...
const REPORT = `${FEEDS}/feed-1/reports/veltrix-item-1`
...
describe('carbon-black feed-reports rollback handler', () => {
  ...
  it('deletes a report the deploy created', async () => {
    await withFetch([cbJson({})], async (calls) => {
      ...
      expect(calls[0].path).toBe(REPORT)
  ...
  it('skips an entry whose report id was never recorded', async () => {
    await withFetch([], async (calls) => {
      const result = await rollback(ctx([entry({ reportId: '' })]))
  ...
  it('skips an entry whose feed id was never recorded', async () => {
    await withFetch([], async (calls) => {
      const result = await rollback(ctx([entry({ feedId: '' })]))

      // Without the feed id the URL would address some other feed's collection.
      expect(result.success).toBe(true)
      expect(calls).toHaveLength(0)
    })
  })
  ...
  it('treats an already-deleted report as rolled back, not as a failure', async () => {
    await withFetch([cbNotFound()], async () => {
      const result = await rollback(ctx([entry({ reportId: 'veltrix-item-1' })]))
  ...
  it('keeps going after one entry fails so the rest still roll back', async () => {
    await withFetch([cbError('boom', 500), cbJson({})], async (calls) => {
      const result = await rollback(
        ctx([
          entry({ itemId: 'item-1', title: 'First', reportId: 'veltrix-item-1' }),
          entry({ itemId: 'item-2', title: 'Second', reportId: 'veltrix-item-2' }),
        ]),
      )

      expect(result.success).toBe(false)
      expect(calls).toHaveLength(2)
      expect(calls[1].path).toBe(`${FEEDS}/feed-1/reports/veltrix-item-2`)
    })
  })
})
