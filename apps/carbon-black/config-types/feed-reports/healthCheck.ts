...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Reports are nested under a feed id; the feed-manager collection is the
  // reachability check for the service that hosts them.
  const feedsPath = `/threathunter/feedmgr/v2/orgs/${cred.orgKey}/feeds`
  const start = Date.now()
  const resp = await client.get(feedsPath)
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'cbc-feed-reports',
    passed,
    message: passed ? 'Reached the Carbon Black feed-manager endpoint' : `Carbon Black error: ${cbErrorMessage(resp)}`,
    latencyMs,
  })
