...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    if (e.existed && e.prior) {
      // Restore the pre-management forwarder. type/destination are immutable, so
      // restore is a delete + recreate of the original body.
      if (e.id) {
  ...
  return { success: true, message: `Rolled back data forwarders: ${deleted} deleted, ${restored} restored` }
}
