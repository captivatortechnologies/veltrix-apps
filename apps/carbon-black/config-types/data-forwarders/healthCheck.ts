...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(client.dataForwardersPath())
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'cbc-data-forwarders',
    passed,
    message: passed ? 'Reached the Carbon Black data-forwarder endpoint' : `Carbon Black error: ${cbErrorMessage(resp)}`,
    latencyMs,
  })
