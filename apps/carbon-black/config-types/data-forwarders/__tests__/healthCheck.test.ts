...
describe('carbon-black data-forwarders healthCheck handler', () => {
  ...
  it('reports healthy after reaching the data-forwarder endpoint', async () => {
    await withFetch([cbJson({ results: [] })], async (calls) => {
      ...
      expect(result.checks[0].name).toBe('cbc-data-forwarders')
