...
/**
 * Health for the Fusion Files config = Recorded Future answers with the
 * configured token. Fusion Files shares the same host/token as the List API, so
 * this reuses the same read-only probe: POST /list/search { limit: 1 }. A 2xx (or
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    ...
    checks.push({
      ...
      passed,
      message: passed
        ? `Recorded Future API reachable (HTTP ${res.status}).`
