...
import { listCustomHostConfigRules } from './deploy'
import { extractHostConfigRuleSpecs, ruleKey, type LiveHostConfigRule } from './validate'

/**
 * Health check for host-configuration-rule configuration:
  ...
  if (reachable.passed && reachable.live) {
    ...
    for (const spec of specs) {
      ...
      checks.push({
        name: `rule:${spec.name}`,
        passed: present,
        message: present ? `Host configuration rule "${spec.name}" is present` : `Host configuration rule "${spec.name}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: LiveHostConfigRule[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: LiveHostConfigRule[] }> {
