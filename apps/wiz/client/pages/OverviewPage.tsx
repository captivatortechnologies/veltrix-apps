...
/**
 * Shows what this app manages in a Wiz tenant, using the platform design-system
 * components from @veltrixsecops/app-sdk/ui — so the page matches the platform
 * look and picks up the app's brand color. Authoring happens in the platform's
...
export default function OverviewPage() {
  ...
  if (loading) return <Spinner label="Loading Wiz app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Wiz (CNAPP) cloud security configuration as code through the Wiz GraphQL API. Create a
          configuration in the Configuration Canvas and deploy it through the pipeline — validate, deploy,
          health check, drift detection and rollback are all handled per configuration type. The app
          authenticates as a Wiz service account (OAuth2 client credentials) against your tenant's regional
          API endpoint.
        </p>
