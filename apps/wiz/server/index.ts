// ========================================================================
// Wiz App — Server Entry Module
//
// Registers Wiz-specific API routes as a Fastify plugin, prefixed with
// /api/apps/wiz/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration
// Canvas, and every write goes through the pipeline handlers (Wiz GraphQL API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('wiz-service-accounts', 'read')],
