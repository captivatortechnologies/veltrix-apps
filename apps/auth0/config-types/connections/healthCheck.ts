...
/**
 * Health for the connections config = Auth0 mints a Management API token for the
 * stored Machine-to-Machine credential and answers a read against the Connections
 * endpoint. Read-only: mint token (client-credentials) → GET /api/v2/connections?per_page=1.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/connections?per_page=1`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status > 0 && res.status < 500
    checks.push({
      name: 'connections_readable',
      passed,
