...
/**
 * Health for the roles config = Auth0 mints a Management API token for the stored
 * Machine-to-Machine credential and answers a read against the Roles endpoint.
 * Read-only: mint token → GET /api/v2/roles?per_page=1.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/roles?per_page=1`, { headers: bearer(accessToken), timeoutMs: 8000 })
