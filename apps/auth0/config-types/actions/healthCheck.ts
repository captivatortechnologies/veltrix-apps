...
/**
 * Health for the actions config = Auth0 mints a Management API token for the
 * stored Machine-to-Machine credential and answers a read against the Actions
 * endpoint. Read-only: mint token → GET /api/v2/actions/actions?per_page=1.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/actions/actions?per_page=1`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status > 0 && res.status < 500
    checks.push({
      name: 'actions_readable',
      passed,
