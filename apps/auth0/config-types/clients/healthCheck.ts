...
/**
 * Health for the clients config = Auth0 mints a Management API token for the
 * stored Machine-to-Machine credential and answers a read against the Clients
 * endpoint. Read-only: mint token (client-credentials) → GET /api/v2/clients?per_page=1.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/clients?per_page=1`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status > 0 && res.status < 500
    checks.push({
      name: 'clients_readable',
      passed,
