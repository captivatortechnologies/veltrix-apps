...
import { GUARDIAN_FACTORS_PATH } from './_shared'

/**
 * Health for the MFA config = Auth0 mints a Management API token for the
 * stored Machine-to-Machine credential and answers a read against the
 * Guardian factors endpoint. Read-only: mint token (client-credentials) →
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/${GUARDIAN_FACTORS_PATH}`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status > 0 && res.status < 500
    checks.push({
      name: 'guardian_factors_readable',
      passed,
