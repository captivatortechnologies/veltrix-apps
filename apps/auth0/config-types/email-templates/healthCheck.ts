...
/**
 * Health for the email-templates config = Auth0 mints a Management API token for
 * the stored Machine-to-Machine credential and answers a read against the Email
 * Templates endpoint. A 404 (verify_email never customized on this tenant) still
 * proves the endpoint is reachable and the token is scoped correctly, so it counts
 * as a pass. Read-only: mint token → GET /api/v2/email-templates/verify_email.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/email-templates/verify_email`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status === 404 || (res.status > 0 && res.status < 500)
    checks.push({
      name: 'email_templates_readable',
      passed,
