...
/**
 * Health for the custom-domains config = Auth0 mints a Management API token
 * for the stored Machine-to-Machine credential and answers a read against the
 * Custom Domains endpoint. Read-only: mint token (client-credentials) →
 * GET /api/v2/custom-domains. Unlike every other endpoint in this app, this
 * one is not paginated, so there is no per_page to cap the read.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/custom-domains`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status > 0 && res.status < 500
    checks.push({
      name: 'custom_domains_readable',
      passed,
