...
/**
 * Health for the organizations config = Auth0 mints a Management API token for
 * the stored Machine-to-Machine credential and answers a read against the
 * Organizations endpoint. Read-only: mint token → GET /api/v2/organizations?per_page=1.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/organizations?per_page=1`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status > 0 && res.status < 500
    checks.push({
      name: 'organizations_readable',
      passed,
