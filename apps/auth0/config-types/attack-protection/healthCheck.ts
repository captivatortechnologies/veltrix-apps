...
import { BRUTE_FORCE_PROTECTION_PATH } from './_shared'

/**
 * Health for the Attack Protection config = Auth0 mints a Management API
 * token for the stored Machine-to-Machine credential and answers a read
 * against the Attack Protection endpoint. Read-only: mint token
 * (client-credentials) → GET /attack-protection/brute-force-protection.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/${BRUTE_FORCE_PROTECTION_PATH}`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status > 0 && res.status < 500
    checks.push({
      name: 'attack_protection_readable',
      passed,
