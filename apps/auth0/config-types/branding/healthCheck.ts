...
/**
 * Health for the branding config = Auth0 mints a Management API token for the
 * stored Machine-to-Machine credential and answers a read against the Branding
 * endpoint. Read-only: mint token → GET /api/v2/branding.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/branding`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status > 0 && res.status < 500
    checks.push({
      name: 'branding_readable',
      passed,
