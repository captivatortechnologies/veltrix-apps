...
/**
 * Health for the resource-servers config = Auth0 mints a Management API token for
 * the stored Machine-to-Machine credential and answers a read against the Resource
 * Servers endpoint. Read-only: mint token → GET /api/v2/resource-servers?per_page=1.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await auth0Fetch(`${base}/resource-servers?per_page=1`, { headers: bearer(accessToken), timeoutMs: 8000 })
    const passed = res.status > 0 && res.status < 500
    checks.push({
      name: 'resource_servers_readable',
      passed,
