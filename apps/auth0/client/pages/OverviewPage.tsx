...
/**
 * What this app manages in an Auth0 tenant, rendered with the platform
...
export default function OverviewPage() {
  ...
  if (loading) return <Spinner label="Loading Auth0 app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Auth0 (Okta-owned enterprise identity platform) configuration as code. Author a
          configuration in the Configuration Canvas and deploy it through the pipeline — validate, deploy,
          health check, drift detection and rollback are handled per configuration type. Applications
          (Clients) are applied over the Auth0 Management API v2, authenticated with a Machine-to-Machine
