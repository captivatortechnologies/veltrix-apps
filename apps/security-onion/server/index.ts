// ========================================================================
// Security Onion App — Server Entry Module
//
// Registers Security-Onion-specific API routes as a Fastify plugin, prefixed
// with /api/apps/security-onion/ and protected by app-level auth + permission
// middleware. Grid CONFIGURATION authoring happens in the Configuration Canvas
// and every config write goes through the pipeline handlers (SOC/Elasticsearch
// REST + Salt over managed ZTNA). The app additionally OWNS BYOL grid
// provisioning: the /byol routes derive a resource plan from a grid's topology,
// persist it, and emit provisioning events for downstream workers.
// ========================================================================
...
const DESTROY_STEPS: Array<{ key: string; title: string; detail: string }> = [
  ...
  { key: 'done', title: 'Resources destroyed', detail: 'All resources for this grid have been removed.' },
]
...
async function emit(events: AppEventPublisher, topic: string, payload: unknown): Promise<void> {
  try {
    ...
    console.error(`[security-onion] publish ${topic} failed:`, err)
  }
}
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('suricata-rules', 'read')],
  ...
  // grid's topology, persists it, opens a deployment run, flips the record to
  // `provisioning`, and emits an event for the (external) provisioning workers.
  // Workers report progress back through the app's onEvent/onWebhook hooks, which
  // advance the persisted resource + step rows.
  ...
  // tag set every resource will carry. Side-effect-free (no writes, no emit, no
  // CIDR commit) — the Plan modal fetches this before Apply calls /deploy. If the
  // allocator is unreachable the plan degrades gracefully (tags only + a soft
  // `networkUnavailable` flag) rather than failing.
  ...
  fastify.post('/byol/:id/deploy', {
    ...
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Atomically reserve the grid's subnet + derive the tenant/cost tags before
      ...
      // provisioning worker see the exact subnet this grid was given.
      ...
      await emit(events, 'infrastructure.deploy.requested', {
        ...
        // access link when the grid is ready.
  ...
  // service token holding `usage:write`. Node-hours only until the grid's
  // Elasticsearch ingest poller is wired (see collector.ts).
