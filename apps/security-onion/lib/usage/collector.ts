...
//   * ingest_gb: from an INJECTED poller (e.g. Elasticsearch _cat/indices on the
//     manager). Skipped when no poller is supplied or it returns null for an
//     infra — because reaching a grid's Elasticsearch requires the component/
//     connectivity/credential context that BYOL records do not carry yet
//     (follow-up: link a BYOL grid to its manager component so ingest can be polled).
//
...
/** Injectable ingest poller: GB ingested by an infra on the day, or null. */
 ...
 * omit it to record node-hours only (the current default until the grid's
 * Elasticsearch link exists).
 */
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM so_byol_infrastructure',
  )
