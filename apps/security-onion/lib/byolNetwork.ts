...
// the POLICY here — which provider/region a grid targets and what subnet size
...
/** The subnet a grid carries (superset of the plan's `network` block). */
 ...
 * A self-hosted grid (no cloud provider) returns null — no Veltrix Network
...
/**
 * The subnet prefix (mask length) a grid requests. The resolved network model
 * gives every grid a /24 within its customer's block; kept as a function so the
 * policy can later widen for larger distributed grids.
 */
 ...
 * `networkUnavailable` flag when a cloud grid's allocator is unreachable).
 */
export async function resolvePlanNetwork(
  ...
  // Cloud grid, but the allocator is not wired yet (Phase 2a) → soft flag.
  ...
  try {
    ...
    console.error('[security-onion] network allocation peek failed:', err)
...
export async function reserveDeployNetwork(
  ...
  try {
    ...
    console.error('[security-onion] network allocation reserve failed:', err)
    return { tags }
  }
}
