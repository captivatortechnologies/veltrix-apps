...
function toItems(list: Array<Record<string, unknown>>) {
  return list.map((fields, i) => ({ id: `i${i}`, name: String(fields.sid ?? i), fields }))
}
...
function deployCtx(fields: Array<Record<string, unknown>>, remote: RemoteExecutor | undefined): DeployContext {
  return {
    ...
    customerId: 'c1',
    configTypeId: 'suricata-rules',
    canvas: {
      id: 's1', canvasId: 'cv1', version: 1, name: 'n', toolType: 'security-onion',
      entityType: 'suricata-rules', items: toItems(fields), sections: toItems(fields), snapshot: {},
    },
...
test('deploy applies so-rule per item then a salt highstate', async () => {
  const { remote, calls } = recordingRemote()
  const res = await deploy(deployCtx([{ sid: '2010935', action: 'disable' }, { sid: '2100498', action: 'enable' }], remote))
  assert.equal(res.success, true)
  assert.deepEqual(
    calls.filter((c) => c.id === 'so-rule').map((c) => c.params),
    [{ action: 'disable', sid: '2010935' }, { action: 'enable', sid: '2100498' }],
  )
  assert.ok(calls.some((c) => c.id === 'salt-highstate'), 'runs highstate after applying rules')
  assert.deepEqual((res.rollbackData as { applied: unknown }).applied, [
    { sid: '2010935', action: 'disable' },
    { sid: '2100498', action: 'enable' },
  ])
})

test('deploy without managed connectivity fails with a clear message', async () => {
  const res = await deploy(deployCtx([{ sid: '1', action: 'disable' }], undefined))
  assert.equal(res.success, false)
  assert.match(res.message, /managed connectivity/)
})

test('rollback applies the inverse state per SID', async () => {
  const { remote, calls } = recordingRemote()
  const ctx = { remote, rollbackData: { applied: [{ sid: '2010935', action: 'disable' }] } } as unknown as RollbackContext
  const res = await rollback(ctx)
  assert.equal(res.success, true)
  assert.deepEqual(calls.find((c) => c.id === 'so-rule')?.params, { action: 'enable', sid: '2010935' })
})

test('validate rejects a non-numeric SID and an unknown state', async () => {
  const ctx = { canvas: { items: toItems([{ sid: 'abc', action: 'nuke' }]) } } as unknown as PipelineContext
  const res = await validate(ctx)
  assert.equal(res.valid, false)
  assert.ok(res.errors.some((e) => e.code === 'INVALID_SID'))
