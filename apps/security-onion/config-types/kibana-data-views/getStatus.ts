...
/** Deployment status for a kibana-data-views configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['manager', 'manager-search', 'standalone'] })
