...
/**
 * What this app manages in a BeyondTrust Password Safe instance, rendered with the
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/beyondtrust/meta')
  ...
  if (loading) return <Spinner label="Loading BeyondTrust app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages BeyondTrust Password Safe (privileged access management) configuration as code. Author a
          configuration in the Configuration Canvas and deploy it through the pipeline — validate, deploy,
          health check, drift detection and rollback are handled per configuration type. Functional accounts
          are applied over the BeyondInsight REST API inside a PS-Auth session (HTTPS, self-signed tolerated).
        </p>
