...
/** Deployment status for a functional-accounts configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['beyondtrust-passwordsafe', 'standalone'] })
