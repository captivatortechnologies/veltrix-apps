...
/**
 * Health for functional-accounts config = Password Safe accepts a PS-Auth sign-in
 * with the configured API key + run-as user. Read-only: POST /Auth/SignAppIn then
 * POST /Auth/Signout. A successful sign-in proves the endpoint resolves AND the
 * credential authenticates. Verify against a live BeyondTrust instance.
 */
