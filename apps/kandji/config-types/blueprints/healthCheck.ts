...
import { blueprintKey, extractBlueprintSpecs, indexBlueprintsByName, type LiveBlueprint } from './validate'
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed && reachable.live) {
    const byName = indexBlueprintsByName(reachable.live)
    for (const spec of specs) {
      const present = byName.has(blueprintKey(spec.name))
      checks.push({
        name: `blueprint:${spec.name}`,
        passed: present,
        message: present ? `Blueprint "${spec.name}" is present` : `Blueprint "${spec.name}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: LiveBlueprint[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: LiveBlueprint[] }> {
