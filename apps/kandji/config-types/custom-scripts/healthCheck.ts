...
import { customScriptKey, extractCustomScriptSpecs, indexCustomScriptsByName, type LiveCustomScript } from './validate'
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const specs = extractCustomScriptSpecs(ctx.canvas).filter((s) => s.name)
  ...
  if (reachable.passed && reachable.live) {
    const byName = indexCustomScriptsByName(reachable.live)
    for (const spec of specs) {
      const present = byName.has(customScriptKey(spec.name))
      checks.push({
        name: `custom-script:${spec.name}`,
        passed: present,
        message: present ? `Custom Script "${spec.name}" is present` : `Custom Script "${spec.name}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: LiveCustomScript[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: LiveCustomScript[] }> {
