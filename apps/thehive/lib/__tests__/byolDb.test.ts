...
test('createByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  await createByol(db, 'cust-1', {
    ...baseInput,
    nodeTiers: [
      { key: 'application', count: 2, placement: null },
      { key: 'cassandra', count: 3, placement: null },
      { key: 'index', count: 3, placement: null },
    ],
  })

  const insert = calls.find((c) => c.sql.includes('INSERT INTO thehive_byol_infrastructure'))
  assert.ok(insert)
...
test('updateByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  await updateByol(db, 'i1', {
    ...baseInput,
    nodeTiers: [
      { key: 'application', count: 5, placement: null },
      { key: 'cassandra', count: 4, placement: null },
      { key: 'index', count: 3, placement: null },
    ],
  })

  const update = calls.find((c) => c.sql.includes('UPDATE thehive_byol_infrastructure'))
  assert.ok(update)
