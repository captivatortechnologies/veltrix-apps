...
// SDK's <ByolInfrastructureManager> sends (application / cassandra / index),
// per-tier minimums (with the distributed Cassandra + Elasticsearch ≥3 HA rule),
// the emitted `nodeTiers` shape, and the placement / topology-authoring fields.
// =============================================================================
...
test('readByol defaults single-instance topology fields', () => {
  const { data } = readByol({
    ...
    tiers: [{ key: 'application', count: 1 }, { key: 'cassandra', count: 1 }, { key: 'index', count: 1 }],
  })
...
test('readByol reads per-tier counts from the tiers array', () => {
  ...
  assert.equal(tiers.find((t) => t.key === 'application')!.count, 2)
  assert.equal(tiers.find((t) => t.key === 'cassandra')!.count, 3)
  assert.equal(tiers.find((t) => t.key === 'index')!.count, 3)
})

test('readByol emits nodeTiers in [application, cassandra, index] order regardless of input order', () => {
  const { data } = readByol(
    distributedBody({
      tiers: [
        { key: 'index', count: 3 },
  ...
  const tiers = data.nodeTiers as Array<{ key: string; count: number }>
  assert.deepEqual(tiers.map((t) => t.key), ['application', 'cassandra', 'index'])
  assert.equal(tiers[0].count, 2)
  assert.equal(tiers[1].count, 3)
  assert.equal(tiers[2].count, 3)
})

test('readByol defaults a missing tier to a count of 1', () => {
  // Single deployment so the distributed cassandra/index ≥3 rule does not apply.
  const { data, error } = readByol({
    name: 'Dev',
    deploymentType: 'single',
    tiers: [{ key: 'application', count: 4 }],
  })
  assert.equal(error, undefined)
  const tiers = data.nodeTiers as Array<{ key: string; count: number }>
  assert.deepEqual(tiers, [
    { key: 'application', count: 4, placement: null },
    { key: 'cassandra', count: 1, placement: null },
    { key: 'index', count: 1, placement: null },
  ])
})

// --- distributed guardrails (per-tier minimums) ------------------------------

test('readByol requires at least 1 Application nodes', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'application', count: 0 }, { key: 'cassandra', count: 3 }, { key: 'index', count: 3 }] }))
  assert.match(error ?? '', /Application nodes must be at least 1/)
})

test('readByol requires at least 3 Cassandra nodes for a distributed ring', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'application', count: 1 }, { key: 'cassandra', count: 2 }, { key: 'index', count: 3 }] }))
  assert.match(error ?? '', /Cassandra nodes must be at least 3/)
})

test('readByol requires at least 3 Elasticsearch nodes for a distributed cluster', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'application', count: 1 }, { key: 'cassandra', count: 3 }, { key: 'index', count: 2 }] }))
  assert.match(error ?? '', /Elasticsearch nodes must be at least 3/)
})

test('readByol accepts a distributed data tier of exactly 3', () => {
  const { error } = readByol(distributedBody({ tiers: [{ key: 'application', count: 1 }, { key: 'cassandra', count: 3 }, { key: 'index', count: 3 }] }))
  assert.equal(error, undefined)
})

test('readByol allows a single-node deployment with one node per tier', () => {
  ...
  assert.equal(error, undefined)
  const tiers = data.nodeTiers as Array<{ key: string; count: number }>
  assert.equal(tiers.find((t) => t.key === 'cassandra')!.count, 1)
  assert.equal(tiers.find((t) => t.key === 'index')!.count, 1)
})
...
test('readByol accepts a valid multi-site placement on the index tier', () => {
  const { data, error } = readByol(
    distributedBody({
      tiers: [
        { key: 'application', count: 2 },
  ...
  assert.equal(tiers.find((t) => t.key === 'index')!.placement.mode, 'multi-site')
})
...
test('readByol rejects placement whose percentages do not total 100, naming the tier', () => {
  ...
  assert.match(error ?? '', /Elasticsearch nodes placement: .*total 100/)
})

test('readByol rejects more sites than nodes on the application tier', () => {
  const { error } = readByol(
    distributedBody({
      tiers: [
        {
  ...
  assert.match(error ?? '', /Application nodes placement: .*Too many sites/)
})
...
test('readByol rejects region-granularity placement unless the network is dedicated (BYOC)', () => {
  const { error } = readByol(
    distributedBody({
      tiers: [{ key: 'application', count: 2 }, { key: 'cassandra', count: 3 }, { key: 'index', count: 4, placement: regionPlacement }],
    }),
  )
