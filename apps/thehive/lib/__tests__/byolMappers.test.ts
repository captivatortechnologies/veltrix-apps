...
test('mapByol parses a raw JSON string node_tiers value', () => {
  const dto = mapByol({
    ...baseRow,
    node_tiers: JSON.stringify([{ key: 'application', count: 1, placement: null }]),
  })
  assert.deepEqual(dto.tiers, [{ key: 'application', count: 1, placement: null }])
})
...
test('mapByol carries a tier placement through', () => {
  ...
  const dto = mapByol({
    ...baseRow,
    node_tiers: [{ key: 'index', count: 3, placement }],
  })
