...
//     it returns null for an infra. TheHive is a case-management / SOAR platform
//     with no simple built-in ingest-volume metric, so the poller is unused today
//     — node-hours are the live dimension (the poller stays wired so a future
//     TheHive volume metric can plug straight in).
//
 ...
 * omit it to record node-hours only (the current default — TheHive has no simple
 * built-in ingest metric).
 */
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM thehive_byol_infrastructure',
  )
