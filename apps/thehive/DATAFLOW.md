...
**App:** `thehive` · **Category:** SOAR · **Version:** 0.4.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **TheHive API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
<sub>Applies to: 7 config types · 2 API families.</sub>
...
sequenceDiagram
  ...
  participant api as TheHive API
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + case-templateswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + case-templatesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: case-templateswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
