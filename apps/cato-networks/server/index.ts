// ========================================================================
// Cato Networks App - Server Entry Module
//
// Registers app-specific API routes as a Fastify plugin, prefixed with
// /api/apps/cato-networks/ and protected by app-level auth + permission
// middleware. Read-only on the platform side: authoring happens in the
// Configuration Canvas, and every write to Cato goes through the pipeline
// handlers (Cato Management Application GraphQL API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('internet-firewall-rules', 'read')],
