...
/**
 * Shows what this app manages in a Cato Networks account, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/cato-networks/meta')
  ...
  if (loading) return <Spinner label="Loading Cato Networks app details..." />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Cato Networks (SASE/SSE) configuration as code through the Cato Management
          Application (CMA) GraphQL API. Create a configuration in the Configuration Canvas and deploy
          it through the pipeline - validate, deploy, health check, drift detection, and rollback are
          all handled per configuration type. Internet/WAN Firewall, Application Control and TLS
          Inspection changes are staged in a private draft revision and published as a batch on deploy.
        </p>
