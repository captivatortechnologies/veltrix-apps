...
**App:** `cato-networks` · **Category:** NETWORK · **Version:** 0.1.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Cato Networks API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 9 config types · 4 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Cato Networks API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + internet-firewall-sectionswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + internet-firewall-sectionsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: internet-firewall-sectionswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
