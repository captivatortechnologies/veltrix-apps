...
const VALID_ACTIONS = ['ALLOW', 'BLOCK', 'CAPTIVE_PORTAL', 'PROMPT'] as const
...
export type InternetFirewallAction = (typeof VALID_ACTIONS)[number]
export type ConnectionOrigin = (typeof VALID_ORIGINS)[number]

export interface InternetFirewallRuleSpec extends RuleSpec {
  description: string
  enabled: boolean
  action: InternetFirewallAction
  connectionOrigin: ConnectionOrigin
  ruleJson?: string
}

/** Extract one Internet Firewall rule spec per canvas item. */
export function extractRuleSpecs(canvas: CanvasSnapshot): InternetFirewallRuleSpec[] {
  return items(canvas).map((item) => {
    const fields = item.fields ?? {}
    const action = (typeof fields.action === 'string' ? fields.action : 'BLOCK') as InternetFirewallAction
...
export function buildRuleData(spec: InternetFirewallRuleSpec): Record<string, unknown> {
...
/**
 * Validate Internet Firewall rule items. Static only - no target access:
