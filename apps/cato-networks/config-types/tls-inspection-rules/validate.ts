...
const VALID_ACTIONS = ['INSPECT', 'BYPASS'] as const
const VALID_CERT_ACTIONS = ['ALLOW', 'BLOCK', 'PROMPT'] as const
...
export type TlsInspectAction = (typeof VALID_ACTIONS)[number]
export type TlsInspectCertAction = (typeof VALID_CERT_ACTIONS)[number]
export type ConnectionOrigin = (typeof VALID_ORIGINS)[number]

export interface TlsInspectionRuleSpec extends RuleSpec {
  description: string
  enabled: boolean
  action: TlsInspectAction
  untrustedCertificateAction: TlsInspectCertAction
  connectionOrigin: ConnectionOrigin
  ruleJson?: string
}

/** Extract one TLS Inspection rule spec per canvas item. */
export function extractRuleSpecs(canvas: CanvasSnapshot): TlsInspectionRuleSpec[] {
  return items(canvas).map((item) => {
    const fields = item.fields ?? {}
    const action = (typeof fields.action === 'string' ? fields.action : 'INSPECT') as TlsInspectAction
    const certAction = (typeof fields.untrustedCertificateAction === 'string' ? fields.untrustedCertificateAction : 'ALLOW') as TlsInspectCertAction
    ...
    return {
      ...
      action: VALID_ACTIONS.includes(action) ? action : 'INSPECT',
      untrustedCertificateAction: VALID_CERT_ACTIONS.includes(certAction) ? certAction : 'ALLOW',
...
export function buildRuleData(spec: TlsInspectionRuleSpec): Record<string, unknown> {
  ...
  return {
    ...
    action: spec.action,
    untrustedCertificateAction: spec.untrustedCertificateAction,
    connectionOrigin: spec.connectionOrigin,
  }
}

/**
 * Validate TLS Inspection rule items. Static only - no target access:
