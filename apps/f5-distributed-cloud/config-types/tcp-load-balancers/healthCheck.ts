...
import { extractTcpLoadBalancerSpecs } from './validate'

const OBJECT_PLURAL = 'tcp_loadbalancers'

/**
 * Health check for TCP load balancer configuration:
 *   1. Namespace reachability + API Token validity
 *   2. Every declared TCP load balancer still exists in the namespace (re-fetched by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`tcp_loadbalancer:${spec.name}`, async () => {
          const live = await client.get(OBJECT_PLURAL, spec.name)
          if (!live) throw new Error(`TCP Load Balancer "${spec.name}" does not exist in namespace "${namespace}"`)
          return `TCP Load Balancer "${spec.name}" is present`
        }),
      )
    }
  }
