...
import { extractServicePolicySpecs, type LiveServicePolicySpec } from './validate'
import { buildServicePolicySpecBody } from './deploy'

const OBJECT_PLURAL = 'service_policys'

/**
 * Detect drift between the deployed service policy configuration and the
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  const specs = extractServicePolicySpecs(ctx.deployedConfig).filter((s) => s.name)

  for (const spec of specs) {
    try {
      const live = await client.get<LiveServicePolicySpec>(OBJECT_PLURAL, spec.name)
      ...
      const expectedBody = buildServicePolicySpecBody(spec)
