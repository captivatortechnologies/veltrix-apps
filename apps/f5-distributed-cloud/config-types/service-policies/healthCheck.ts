...
import { extractServicePolicySpecs } from './validate'

const OBJECT_PLURAL = 'service_policys'

/**
 * Health check for service policy configuration:
 *   1. Namespace reachability + API Token validity
 *   2. Every declared service policy still exists in the namespace (re-fetched by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`service_policy:${spec.name}`, async () => {
          const live = await client.get(OBJECT_PLURAL, spec.name)
          if (!live) throw new Error(`Service policy "${spec.name}" does not exist in namespace "${namespace}"`)
          return `Service policy "${spec.name}" is present`
        }),
      )
    }
  }
