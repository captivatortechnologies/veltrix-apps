...
import { extractOriginPoolSpecs } from './validate'

const OBJECT_PLURAL = 'origin_pools'

/**
 * Health check for origin pool configuration:
 *   1. Namespace reachability + API Token validity
 *   2. Every declared origin pool still exists in the namespace (re-fetched by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`origin_pool:${spec.name}`, async () => {
          const live = await client.get(OBJECT_PLURAL, spec.name)
          if (!live) throw new Error(`Origin pool "${spec.name}" does not exist in namespace "${namespace}"`)
          return `Origin pool "${spec.name}" is present`
        }),
      )
    }
  }
