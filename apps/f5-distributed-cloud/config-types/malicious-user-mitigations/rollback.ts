...
import type { MaliciousUserMitigationRollbackEntry } from './deploy'

const OBJECT_PLURAL = 'malicious_user_mitigations'

/**
 * Roll back malicious user mitigation policies using the state captured
 * during deploy:
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  const previousState = (ctx.rollbackData as { previousState?: MaliciousUserMitigationRollbackEntry[] })
    ?.previousState
  ...
  try {
    for (const entry of previousState) {
      if (!entry.existed) {
        ...
        if (!del.ok && del.status !== 404) {
          throw new Error(`Failed to delete mitigation policy "${entry.name}": ${f5xcErrorMessage(del)}`)
        }
      } else if (entry.prior) {
        const res = await client.replace(OBJECT_PLURAL, entry.name, entry.prior)
        if (!res.ok) {
          throw new Error(`Failed to restore mitigation policy "${entry.name}": ${f5xcErrorMessage(res)}`)
        }
      }
    ...
    return {
      success: true,
      message: `Rolled back ${reverted.length} malicious user mitigation policy(ies): ${reverted.join(', ')}`,
    }
  } catch (error) {
    return {
      success: false,
