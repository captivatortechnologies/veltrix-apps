...
import { extractMaliciousUserMitigationSpecs } from './validate'

const OBJECT_PLURAL = 'malicious_user_mitigations'

/**
 * Health check for malicious user mitigation configuration:
 *   1. Namespace reachability + API Token validity
 *   2. Every declared mitigation policy still exists in the namespace (re-fetched by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`mitigation:${spec.name}`, async () => {
          const live = await client.get(OBJECT_PLURAL, spec.name)
          if (!live) throw new Error(`Mitigation policy "${spec.name}" does not exist in namespace "${namespace}"`)
          return `Mitigation policy "${spec.name}" is present`
        }),
      )
    }
  }
