...
import { extractAppFirewallSpecs } from './validate'

const OBJECT_PLURAL = 'app_firewalls'

/**
 * Health check for App Firewall configuration:
 *   1. Namespace reachability + API Token validity
 *   2. Every declared App Firewall still exists in the namespace (re-fetched by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`app_firewall:${spec.name}`, async () => {
          const live = await client.get(OBJECT_PLURAL, spec.name)
          if (!live) throw new Error(`App Firewall "${spec.name}" does not exist in namespace "${namespace}"`)
          return `App Firewall "${spec.name}" is present`
        }),
      )
    }
  }
