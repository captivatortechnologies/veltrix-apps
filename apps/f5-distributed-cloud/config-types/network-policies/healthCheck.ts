...
import { extractNetworkPolicySpecs } from './validate'

const OBJECT_PLURAL = 'network_policys'

/**
 * Health check for network policy configuration:
 *   1. Namespace reachability + API Token validity
 *   2. Every declared network policy still exists in the namespace (re-fetched by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`network_policy:${spec.name}`, async () => {
          const live = await client.get(OBJECT_PLURAL, spec.name)
          if (!live) throw new Error(`Network policy "${spec.name}" does not exist in namespace "${namespace}"`)
          return `Network policy "${spec.name}" is present`
        }),
      )
    }
  }
