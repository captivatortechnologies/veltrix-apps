...
import { extractHealthCheckSpecs } from './validate'

const OBJECT_PLURAL = 'healthchecks'

/**
 * Health check for health-check configuration (the app's own probe of F5 XC,
 * distinct from the F5 XC health checks it manages):
 *   1. Namespace reachability + API Token validity (GET /healthchecks -
 *      a 200, even with zero items, proves the token is accepted and the
 *      namespace exists; 401/403 means the token was rejected)
 *   2. Every declared health check still exists in the namespace (re-fetched by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`healthcheck:${spec.name}`, async () => {
          const live = await client.get(OBJECT_PLURAL, spec.name)
          if (!live) throw new Error(`Health check "${spec.name}" does not exist in namespace "${namespace}"`)
          return `Health check "${spec.name}" is present`
        }),
      )
    }
  }
