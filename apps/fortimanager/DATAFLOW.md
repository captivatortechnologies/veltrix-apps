...
**App:** `fortimanager` · **Category:** NETWORK · **Version:** 0.5.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **FortiManager API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 31 config types · 1 API families.</sub>
...
sequenceDiagram
  ...
  participant api as FortiManager API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + firewall-addresseswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + firewall-addressesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: firewall-addresseswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
