...
import { cidrToIpMask, extractMulticastAddressSpecs, type LiveMulticastAddress, type MulticastAddressSpec } from './validate'
...
/** The ADOM-scoped multicast-address object path. */
export function multicastAddressUrl(adom: string): string {
  return `/pm/config/adom/${adom}/obj/firewall/multicast-address`
}

export function buildMulticastAddressBody(spec: MulticastAddressSpec): Record<string, unknown> {
  ...
  if (spec.type === 'multicastrange') {
    ...
    body.subnet = cidrToIpMask(spec.subnetCidr)
  }
  if (spec.associatedInterface) body['associated-interface'] = spec.associatedInterface
  if (spec.comment) body.comment = spec.comment
  return body
}

export function snapshotLive(live: LiveMulticastAddress): Record<string, unknown> {
  const body: Record<string, unknown> = { name: live.name }
  for (const k of ['type', 'start-ip', 'end-ip', 'subnet', 'associated-interface', 'comment'] as const) {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractMulticastAddressSpecs(ctx.canvas).filter((s) => s.name)
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const liveByName = new Map<string, LiveMulticastAddress>()
      ...
      for (const spec of specs) {
        ...
        const resp = await client.set(url, buildMulticastAddressBody(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some multicast addresses failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} firewall multicast address(es)`, rollbackData: { entries } }
}
