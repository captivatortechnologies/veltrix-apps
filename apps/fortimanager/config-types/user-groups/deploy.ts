...
import { extractUserGroupSpecs, liveMemberNames, type UserGroupSpec, type LiveUserGroup } from './validate'
...
/** The ADOM-scoped user group object path. */
export function userGroupUrl(adom: string): string {
  return `/pm/config/adom/${adom}/obj/user/group`
}

export function buildUserGroupBody(spec: UserGroupSpec): Record<string, unknown> {
  const body: Record<string, unknown> = { name: spec.name, 'group-type': spec.groupType, member: spec.members }
  if (spec.authTimeout) body.authtimeout = Number(spec.authTimeout)
  if (spec.logicType) body['logic-type'] = spec.logicType
  return body
}

export function snapshotLive(live: LiveUserGroup): Record<string, unknown> {
  const body: Record<string, unknown> = { name: live.name, member: liveMemberNames(live.member) }
  for (const k of ['group-type', 'authtimeout', 'logic-type'] as const) {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const liveByName = new Map<string, LiveUserGroup>()
      ...
      for (const spec of specs) {
        ...
        const resp = await client.set(url, buildUserGroupBody(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some user groups failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} user group(s)`, rollbackData: { entries } }
}
