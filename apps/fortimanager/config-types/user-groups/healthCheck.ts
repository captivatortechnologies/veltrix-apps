...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(userGroupUrl(settings.adom))
  ...
  checks.push({
    name: 'fmg-user-group',
    passed,
    message: passed ? `Reached the FortiManager user group table on ADOM "${settings.adom}"` : `FortiManager error: ${fmgErrorMessage(resp)}`,
    latencyMs,
  })
