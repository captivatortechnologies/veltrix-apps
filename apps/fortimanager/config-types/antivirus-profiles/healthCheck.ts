...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(antivirusProfileUrl(settings.adom))
  const latencyMs = Date.now() - start
  await client.logout()
  const passed = resp.ok

  checks.push({
    name: 'fmg-antivirus-profile',
    passed,
    message: passed ? `Reached the FortiManager antivirus profile table on ADOM "${settings.adom}"` : `FortiManager error: ${fmgErrorMessage(resp)}`,
    latencyMs,
  })
