...
// --- FortiManager firewall address group constraints -------------------------
...
export interface AddressGroupSpec {
  ...
  name: string
  /** member address object names. */
  members: string[]
  comment: string
}

/** An address group as returned by a get on the addrgrp table. */
export interface LiveAddressGroup {
  name?: string
...
export function liveMemberNames(v: LiveAddressGroup['member']): string[] {
...
export default function validate(ctx: PipelineContext): ValidationResult {
  ...
  const specs = extractAddressGroupSpecs(ctx.canvas)
