...
import { extractAddressGroupSpecs, liveMemberNames, type LiveAddressGroup } from './validate'
import { addrgrpUrl } from './deploy'
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  const specs = extractAddressGroupSpecs(ctx.deployedConfig).filter((s) => s.name)
  const diffs: Diffs = []
  try {
