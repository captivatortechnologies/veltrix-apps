...
import { extractAddressGroupSpecs, liveMemberNames, type AddressGroupSpec, type LiveAddressGroup } from './validate'
...
/** The ADOM-scoped address-group object path. */
export function addrgrpUrl(adom: string): string {
  return `/pm/config/adom/${adom}/obj/firewall/addrgrp`
}

export function buildAddressGroupBody(spec: AddressGroupSpec): Record<string, unknown> {
  const body: Record<string, unknown> = { name: spec.name, member: spec.members }
  if (spec.comment) body.comment = spec.comment
  return body
}

export function snapshotLive(live: LiveAddressGroup): Record<string, unknown> {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const url = addrgrpUrl(settings.adom)
  const specs = extractAddressGroupSpecs(ctx.canvas).filter((s) => s.name)
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const live = Array.isArray(listed.data) ? (listed.data as LiveAddressGroup[]) : []
      const liveByName = new Map<string, LiveAddressGroup>()
      ...
      for (const spec of specs) {
        ...
        const resp = await client.set(url, buildAddressGroupBody(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some address groups failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} firewall address group(s)`, rollbackData: { entries } }
}
