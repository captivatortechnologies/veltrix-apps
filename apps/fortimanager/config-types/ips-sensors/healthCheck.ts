...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(ipsSensorUrl(settings.adom))
  ...
  checks.push({
    name: 'fmg-ips-sensor',
    passed,
    message: passed ? `Reached the FortiManager IPS sensor table on ADOM "${settings.adom}"` : `FortiManager error: ${fmgErrorMessage(resp)}`,
    latencyMs,
  })
