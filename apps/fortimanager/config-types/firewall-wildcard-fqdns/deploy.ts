...
import { extractWildcardFqdnSpecs, type LiveWildcardFqdn, type WildcardFqdnSpec } from './validate'
...
/** The ADOM-scoped wildcard-FQDN custom object path. */
export function wildcardFqdnUrl(adom: string): string {
  return `/pm/config/adom/${adom}/obj/firewall/wildcard-fqdn/custom`
}

export function buildWildcardFqdnBody(spec: WildcardFqdnSpec): Record<string, unknown> {
  const body: Record<string, unknown> = { name: spec.name, 'wildcard-fqdn': spec.wildcardFqdn }
  if (spec.comment) body.comment = spec.comment
  return body
}

export function snapshotLive(live: LiveWildcardFqdn): Record<string, unknown> {
  const body: Record<string, unknown> = { name: live.name }
  for (const k of ['wildcard-fqdn', 'comment'] as const) {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractWildcardFqdnSpecs(ctx.canvas).filter((s) => s.name)
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const liveByName = new Map<string, LiveWildcardFqdn>()
      for (const w of live) if (w.name) liveByName.set(w.name.toLowerCase(), w)

      const prior = await loadPriorEntries(ctx)

      for (const spec of specs) {
        const liveMatch = liveByName.get(spec.name.toLowerCase()) ?? null
        const resp = await client.set(url, buildWildcardFqdnBody(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some wildcard FQDNs failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} wildcard FQDN(s)`, rollbackData: { entries } }
}
