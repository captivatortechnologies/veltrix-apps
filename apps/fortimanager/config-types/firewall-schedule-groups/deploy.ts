...
import { extractScheduleGroupSpecs, liveMemberNames, type LiveScheduleGroup, type ScheduleGroupSpec } from './validate'
...
/** The ADOM-scoped schedule-group object path. */
export function scheduleGroupUrl(adom: string): string {
  return `/pm/config/adom/${adom}/obj/firewall/schedule/group`
}

export function buildScheduleGroupBody(spec: ScheduleGroupSpec): Record<string, unknown> {
  return { name: spec.name, member: spec.members }
}

export function snapshotLive(live: LiveScheduleGroup): Record<string, unknown> {
  return { name: live.name, member: liveMemberNames(live.member) }
}
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractScheduleGroupSpecs(ctx.canvas).filter((s) => s.name)
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const liveByName = new Map<string, LiveScheduleGroup>()
      ...
      for (const spec of specs) {
        ...
        const resp = await client.set(url, buildScheduleGroupBody(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some schedule groups failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} firewall schedule group(s)`, rollbackData: { entries } }
}
