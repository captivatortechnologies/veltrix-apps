...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(fssoAgentUrl(settings.adom))
  ...
  checks.push({
    name: 'fmg-user-fsso',
    passed,
    message: passed ? `Reached the FortiManager FSSO agent table on ADOM "${settings.adom}"` : `FortiManager error: ${fmgErrorMessage(resp)}`,
    latencyMs,
  })
