...
import { extractIpPool6Specs, type IpPool6Spec, type LiveIpPool6 } from './validate'
...
/** The ADOM-scoped IPv6 IP pool object path. */
export function ippool6Url(adom: string): string {
  return `/pm/config/adom/${adom}/obj/firewall/ippool6`
}

export function buildIpPool6Body(spec: IpPool6Spec): Record<string, unknown> {
  const body: Record<string, unknown> = { name: spec.name, startip: spec.startIp, endip: spec.endIp }
  if (spec.comment) body.comments = spec.comment
  return body
}

export function snapshotLive(live: LiveIpPool6): Record<string, unknown> {
  const body: Record<string, unknown> = { name: live.name }
  for (const k of ['startip', 'endip', 'comments'] as const) {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractIpPool6Specs(ctx.canvas).filter((s) => s.name)
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const liveByName = new Map<string, LiveIpPool6>()
      ...
      for (const spec of specs) {
        ...
        const resp = await client.set(url, buildIpPool6Body(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some IPv6 pools failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} firewall IPv6 pool(s)`, rollbackData: { entries } }
}
