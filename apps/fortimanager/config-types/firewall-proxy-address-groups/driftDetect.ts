...
import { extractProxyAddressGroupSpecs, liveMemberNames, type LiveProxyAddressGroup } from './validate'
import { proxyAddrgrpUrl } from './deploy'
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  const specs = extractProxyAddressGroupSpecs(ctx.deployedConfig).filter((s) => s.name)
  const diffs: Diffs = []
  try {
    ...
    for (const spec of specs) {
      ...
      if (typeof g.type === 'string' && g.type !== spec.type) {
        diffs.push({ field: `${spec.name}.type`, expected: spec.type, actual: g.type, severity: 'critical' })
      }
