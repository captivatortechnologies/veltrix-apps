...
import { extractProxyAddressGroupSpecs, liveMemberNames, type LiveProxyAddressGroup, type ProxyAddressGroupSpec } from './validate'
...
/** The ADOM-scoped explicit-proxy address-group object path. */
export function proxyAddrgrpUrl(adom: string): string {
  return `/pm/config/adom/${adom}/obj/firewall/proxy-addrgrp`
}

export function buildProxyAddressGroupBody(spec: ProxyAddressGroupSpec): Record<string, unknown> {
  const body: Record<string, unknown> = { name: spec.name, type: spec.type, member: spec.members }
  if (spec.comment) body.comment = spec.comment
  return body
}

export function snapshotLive(live: LiveProxyAddressGroup): Record<string, unknown> {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const url = proxyAddrgrpUrl(settings.adom)
  const specs = extractProxyAddressGroupSpecs(ctx.canvas).filter((s) => s.name)
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const live = Array.isArray(listed.data) ? (listed.data as LiveProxyAddressGroup[]) : []
      const liveByName = new Map<string, LiveProxyAddressGroup>()
      ...
      for (const spec of specs) {
        ...
        const resp = await client.set(url, buildProxyAddressGroupBody(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some proxy address groups failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} explicit-proxy address group(s)`, rollbackData: { entries } }
}
