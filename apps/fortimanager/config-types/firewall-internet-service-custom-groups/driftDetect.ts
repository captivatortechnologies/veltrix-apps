...
import { extractInternetServiceCustomGroupSpecs, liveMemberNames, type LiveInternetServiceCustomGroup } from './validate'
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  const specs = extractInternetServiceCustomGroupSpecs(ctx.deployedConfig).filter((s) => s.name)
  const diffs: Diffs = []
  try {
