...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(recurringScheduleUrl(settings.adom))
  const latencyMs = Date.now() - start
  await client.logout()
  const passed = resp.ok

  checks.push({
    name: 'fmg-firewall-schedule-recurring',
    passed,
    message: passed ? `Reached the FortiManager recurring-schedule table on ADOM "${settings.adom}"` : `FortiManager error: ${fmgErrorMessage(resp)}`,
    latencyMs,
  })
