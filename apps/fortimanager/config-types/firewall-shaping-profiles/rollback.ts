...
import { shapingProfileUrl, type RollbackEntry } from './deploy'
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    for (const e of entries) {
      if (e.existed && e.prior) {
        ...
        const resp = await client.delete(url, ['profile-name', '==', e.name])
  ...
  return { success: true, message: `Rolled back firewall shaping profiles: ${deleted} deleted, ${restored} restored` }
}
