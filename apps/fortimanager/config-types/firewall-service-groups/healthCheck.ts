...
import { serviceGroupUrl } from './deploy'
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(serviceGroupUrl(settings.adom))
  const latencyMs = Date.now() - start
  await client.logout()
  const passed = resp.ok

  checks.push({
    name: 'fmg-firewall-service-group',
    passed,
    message: passed ? `Reached the FortiManager service-group table on ADOM "${settings.adom}"` : `FortiManager error: ${fmgErrorMessage(resp)}`,
    latencyMs,
  })
