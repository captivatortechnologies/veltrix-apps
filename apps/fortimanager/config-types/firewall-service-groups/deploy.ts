...
import { extractServiceGroupSpecs, liveMemberNames, type LiveServiceGroup, type ServiceGroupSpec } from './validate'
...
/** The ADOM-scoped service-group object path. */
export function serviceGroupUrl(adom: string): string {
  return `/pm/config/adom/${adom}/obj/firewall/service/group`
}
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const url = serviceGroupUrl(settings.adom)
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const live = Array.isArray(listed.data) ? (listed.data as LiveServiceGroup[]) : []
      const liveByName = new Map<string, LiveServiceGroup>()
      ...
      for (const spec of specs) {
        ...
        const resp = await client.set(url, buildServiceGroupBody(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some service groups failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} firewall service group(s)`, rollbackData: { entries } }
}
