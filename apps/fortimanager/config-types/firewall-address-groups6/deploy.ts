...
import { extractAddressGroup6Specs, liveMemberNames, type AddressGroup6Spec, type LiveAddressGroup6 } from './validate'
...
/** The ADOM-scoped IPv6 address-group object path. */
export function addrgrp6Url(adom: string): string {
  return `/pm/config/adom/${adom}/obj/firewall/addrgrp6`
}

export function buildAddressGroup6Body(spec: AddressGroup6Spec): Record<string, unknown> {
  const body: Record<string, unknown> = { name: spec.name, member: spec.members }
  if (spec.comment) body.comment = spec.comment
  return body
}

export function snapshotLive(live: LiveAddressGroup6): Record<string, unknown> {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractAddressGroup6Specs(ctx.canvas).filter((s) => s.name)
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const liveByName = new Map<string, LiveAddressGroup6>()
      ...
      for (const spec of specs) {
        ...
        const resp = await client.set(url, buildAddressGroup6Body(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some IPv6 address groups failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} firewall IPv6 address group(s)`, rollbackData: { entries } }
}
