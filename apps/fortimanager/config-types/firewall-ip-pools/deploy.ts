...
import { extractIpPoolSpecs, type IpPoolSpec, type LiveIpPool } from './validate'
...
/** The ADOM-scoped IP pool object path. */
export function ippoolUrl(adom: string): string {
  return `/pm/config/adom/${adom}/obj/firewall/ippool`
}

export function buildIpPoolBody(spec: IpPoolSpec): Record<string, unknown> {
  const body: Record<string, unknown> = { name: spec.name, type: spec.type, startip: spec.startIp, endip: spec.endIp }
  if (spec.comment) body.comments = spec.comment
  return body
}

export function snapshotLive(live: LiveIpPool): Record<string, unknown> {
  const body: Record<string, unknown> = { name: live.name }
  for (const k of ['type', 'startip', 'endip', 'comments'] as const) {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractIpPoolSpecs(ctx.canvas).filter((s) => s.name)
  ...
  try {
    ...
    if (!listed.ok) {
      ...
      const liveByName = new Map<string, LiveIpPool>()
      ...
      for (const spec of specs) {
        ...
        const resp = await client.set(url, buildIpPoolBody(spec))
        if (!resp.ok) {
  ...
  if (failures.length) {
    return { success: false, message: `Some IP pools failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} firewall IP pool(s)`, rollbackData: { entries } }
}
