...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(ldapServerUrl(settings.adom))
  ...
  checks.push({
    name: 'fmg-user-ldap',
    passed,
    message: passed ? `Reached the FortiManager LDAP server table on ADOM "${settings.adom}"` : `FortiManager error: ${fmgErrorMessage(resp)}`,
    latencyMs,
  })
