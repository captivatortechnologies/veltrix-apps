...
//
// The shared node knobs keep the SDK form's Splunk-shaped names; the MISP
// topology maps them to its own roles (see lib/byolTopology.ts):
//   • indexerCount    → MariaDB database nodes (database)
//   • searchHeadCount → MISP core web/API nodes (misp-core)
//
...
export function readByol(body: any): { data: Record<string, unknown>; error?: string } {
  ...
  if (isDistributed) {
    ...
    if (indexerErr) return { data: {}, error: `Database placement: ${indexerErr}` }
    const searchErr = validatePlacement(searchHeadPlacement, searchHeadCount)
    if (searchErr) return { data: {}, error: `MISP core placement: ${searchErr}` }
    ...
    for (const [label, p] of [
      ['Database', indexerPlacement],
      ['MISP core', searchHeadPlacement],
    ] as const) {
