...
//     it returns null for an infra. MISP has no built-in ingest metric, so the
//     poller is unused today — node-hours are the live dimension (the poller
//     stays wired so a future MISP-specific volume metric can plug straight in).
//
 ...
 * omit it to record node-hours only (the current default — MISP has no built-in
 * ingest metric).
 */
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM misp_byol_infrastructure',
  )
