...
// `misp_byol_infrastructure` table (and its region satellites).
//
 ...
 * `nodeTiers` (always populated by `readByol`, in [database, core] order);
 * when absent — a direct store caller that predates the generic topology —
 * derives the same two-tier shape from the legacy scalar fields, matching
 * migration 004's backfill, so the column is never written empty.
 */
function nodeTiersJson(input: ByolInput): string {
  const tiers: NodeTierDto[] =
    input.nodeTiers && input.nodeTiers.length > 0
      ? input.nodeTiers
      : [
          { key: 'database', count: input.indexerCount, placement: input.indexerPlacement ?? null },
          { key: 'core', count: input.searchHeadCount, placement: input.searchHeadPlacement ?? null },
        ]
...
export interface ByolInput {
  ...
  // Generic per-tier storage — the app-agnostic replacement for the
  // indexerCount/searchHeadCount pair (see lib/byolInput.ts).
...
async function attachRegions(db: PlatformDatabaseClient, infra: ByolDto): Promise<ByolDto> {
  const [indexer, searchHead] = await Promise.all([
    db.$queryRawUnsafe<Row[]>(
      'SELECT * FROM misp_byol_indexer_region WHERE infrastructure_id = $1::uuid',
      infra.id,
    ),
    db.$queryRawUnsafe<Row[]>(
      'SELECT * FROM misp_byol_search_head_region WHERE infrastructure_id = $1::uuid',
      infra.id,
    ),
  ])
...
export async function listByol(db: PlatformDatabaseClient, customerId: string): Promise<ByolDto[]> {
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM misp_byol_infrastructure WHERE customer_id = $1::uuid ORDER BY updated_at DESC',
    customerId,
  )
...
export async function getByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM misp_byol_infrastructure WHERE id = $1::uuid AND customer_id = $2::uuid',
    id,
    customerId,
  )
...
export async function createByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    ...
    `INSERT INTO misp_byol_infrastructure
...
export async function updateByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    `UPDATE misp_byol_infrastructure SET
...
export async function setByolStatus(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'UPDATE misp_byol_infrastructure SET status = $2, updated_at = now() WHERE id = $1::uuid RETURNING *',
    id,
    status,
  )
...
export async function deleteByol(db: PlatformDatabaseClient, id: string): Promise<void> {
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM misp_byol_infrastructure WHERE id = $1::uuid',
    id,
  )
  if (rows[0]) await emitStateEvent(db, mapByol(rows[0]), 'decommissioned')
  await db.$executeRawUnsafe('DELETE FROM misp_byol_infrastructure WHERE id = $1::uuid', id)
}
...
export async function getByolCore(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT id, name, customer_id FROM misp_byol_infrastructure WHERE id = $1::uuid',
    id,
  )
  const r = rows[0]
