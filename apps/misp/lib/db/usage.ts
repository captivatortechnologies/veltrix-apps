...
// `misp_byol_state_event` (lifecycle log) and `misp_byol_usage` (daily metered
...
export async function recordStateEvent(
  ...
  await db.$executeRawUnsafe(
    `INSERT INTO misp_byol_state_event (infrastructure_id, customer_id, status, node_count)
...
export async function listStateEventsForWindow(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    `(
       SELECT * FROM misp_byol_state_event
