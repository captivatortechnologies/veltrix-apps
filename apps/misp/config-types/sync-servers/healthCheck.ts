...
/**
 * Health for sync-servers config = MISP answers on its REST API with the configured
 * automation key. Read-only: GET /servers/getVersion. Any response below 500
 * counts as reachable (auth nuances surface at deploy time, not here). Verify
 * /servers/getVersion against a live MISP 2.4 instance.
 */
