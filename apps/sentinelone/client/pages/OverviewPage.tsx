...
/**
 * Shows what this app manages in a SentinelOne tenant, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/sentinelone/meta')
  ...
  if (loading) return <Spinner label="Loading SentinelOne app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages SentinelOne endpoint security configuration as code through the SentinelOne
          Management API. Create a configuration in the Configuration Canvas and deploy it through the
          pipeline — validate, deploy, health check, drift detection and rollback are all handled per
          configuration type. Objects are scoped across the Account / Site / Group hierarchy via the
          Scope and Scope ID settings.
        </p>
