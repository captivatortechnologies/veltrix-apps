...
/**
 * Health check for blocklist-hash configuration:
 *   1. SentinelOne API reachability + credential/scope validity (a scoped list)
 *   2. Every declared blocklist hash (sha1, osType) still exists at the scope
  ...
  if (reachable.passed && reachable.live) {
    ...
    for (const spec of specs) {
      ...
      checks.push({
        ...
        passed: present,
        message: present ? `Blocklist hash "${spec.sha1}" is present` : `Blocklist hash "${spec.sha1}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: import('./validate').LiveRestriction[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: import('./validate').LiveRestriction[] }> {
