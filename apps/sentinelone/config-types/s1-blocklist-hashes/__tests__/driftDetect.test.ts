...
describe('SentinelOne Blocklist Hashes Drift Detect Handler', () => {
  it('reports no drift and calls nothing without a credential', async () => {
    await withFetch([], async (calls) => {
      const result = await driftDetect(
        driftContext({ configTypeId: CONFIG_TYPE, sections: [malware], credential: null }),
      )
  ...
  it('reports no drift and calls nothing when the Scope ID setting is unset', async () => {
    await withFetch([], async (calls) => {
      const result = await driftDetect(ctx([malware], SCOPELESS_SETTINGS))
      expect(result.hasDrift).toBe(false)
      expect(calls).toHaveLength(0)
    })
  })

  it('reports a hash removed from the blocklist as critical drift', async () => {
    await withFetch([envelope([]), envelope([])], async () => {
      const result = await driftDetect(ctx([malware]))
  ...
  it('reports no drift while the hash is still blocklisted', async () => {
  ...
  it('attributes the removal to the console user who made it', async () => {
    const activity = {
      ...
      primaryDescription: 'Blocklist entry was removed by Alice Admin.',
      userId: 'u-alice',
  ...
  it('never attributes drift to the connection\'s own service user', async () => {
    const ourOwnDeploy = {
      id: 'act-1',
      createdAt: '2026-08-01T10:00:00.000Z',
      primaryDescription: 'Blocklist entry was removed by veltrix-svc.',
      userId: 'u-veltrix',
