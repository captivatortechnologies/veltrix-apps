...
import { extractDeviceRuleSpecs, ruleKey } from './validate'

/**
 * Health check for Device Control configuration:
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed && reachable.live) {
    const names = new Set(reachable.live.filter((r) => r.ruleName).map((r) => ruleKey(r.ruleName as string)))
    for (const spec of specs) {
      const present = names.has(ruleKey(spec.ruleName))
      checks.push({
        name: `rule:${spec.ruleName}`,
        passed: present,
        message: present ? `Device control rule "${spec.ruleName}" is present` : `Device control rule "${spec.ruleName}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: import('./validate').LiveDeviceRule[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: import('./validate').LiveDeviceRule[] }> {
