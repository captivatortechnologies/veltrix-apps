...
import { extractFirewallRuleSpecs, ruleKey } from './validate'

/**
 * Health check for Firewall Control configuration:
  ...
  if (reachable.passed && reachable.live) {
    ...
    for (const spec of specs) {
      ...
      checks.push({
        name: `rule:${spec.name}`,
        passed: present,
        message: present ? `Firewall rule "${spec.name}" is present` : `Firewall rule "${spec.name}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: import('./validate').LiveFirewallRule[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: import('./validate').LiveFirewallRule[] }> {
