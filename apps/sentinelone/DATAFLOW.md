...
**App:** `sentinelone` · **Category:** EDR · **Version:** 1.2.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **SentinelOne API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 10 config types · 8 API families.</sub>
...
sequenceDiagram
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + s1-exclusionswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + s1-exclusionsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: s1-exclusionswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
