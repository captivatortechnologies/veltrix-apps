-- Graylog App — BYOL stack infrastructure + end-to-end deployment tracking
-- (app-owned).
--
-- These make the "deploy all necessary resources" surface real: a BYOL Graylog
...
-- {"key","count","placement"} for the scalable tiers (graylog / opensearch).
--
...
-- `graylog_`-prefixed so it can never collide with a platform table.

-- --- BYOL infrastructure -----------------------------------------------------

CREATE TABLE IF NOT EXISTS graylog_byol_infrastructure (
  id                          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name                        TEXT NOT NULL,
  -- 'single' (all-in-one node) | 'distributed' (Graylog + OpenSearch + MongoDB)
...
CREATE INDEX IF NOT EXISTS graylog_byol_infrastructure_customer_idx
  ON graylog_byol_infrastructure(customer_id);

-- --- Resources: one row per thing that must exist for the stack ---------------

CREATE TABLE IF NOT EXISTS graylog_byol_resource (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  infrastructure_id UUID NOT NULL REFERENCES graylog_byol_infrastructure(id) ON DELETE CASCADE,
  -- foundation | data | app
  tier              TEXT NOT NULL,
  -- machine kind: network, load-balancer, graylog, opensearch, mongodb,
  -- standalone, ...
  kind              TEXT NOT NULL,
  name              TEXT NOT NULL,
  role              TEXT,
  ...
  zone              TEXT,
  -- machine roles a node runs (e.g. web/rest on a graylog node).
  roles             JSONB,
...
CREATE UNIQUE INDEX IF NOT EXISTS graylog_byol_resource_plan_key_idx
  ON graylog_byol_resource(infrastructure_id, plan_key);
CREATE INDEX IF NOT EXISTS graylog_byol_resource_infra_idx
  ON graylog_byol_resource(infrastructure_id);

-- --- Deployment runs: one row per deploy/destroy invocation -------------------

CREATE TABLE IF NOT EXISTS graylog_byol_deployment (
  id                   UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  infrastructure_id    UUID NOT NULL REFERENCES graylog_byol_infrastructure(id) ON DELETE CASCADE,
  -- deploy | destroy
...
CREATE INDEX IF NOT EXISTS graylog_byol_deployment_infra_idx
  ON graylog_byol_deployment(infrastructure_id, started_at DESC);

-- --- Deployment steps: the ordered Activity timeline within a run -------------

CREATE TABLE IF NOT EXISTS graylog_byol_deployment_step (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  deployment_id UUID NOT NULL REFERENCES graylog_byol_deployment(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS graylog_byol_deployment_step_key_idx
  ON graylog_byol_deployment_step(deployment_id, step_key);
CREATE INDEX IF NOT EXISTS graylog_byol_deployment_step_order_idx
  ON graylog_byol_deployment_step(deployment_id, step_order);
