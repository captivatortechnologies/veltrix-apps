...
// page. The app additionally OWNS BYOL stack provisioning: the /byol routes
// derive a resource plan from a stack's topology, persist it, and emit
...
async function emit(events: AppEventPublisher, topic: string, payload: unknown): Promise<void> {
  try {
    ...
    console.error(`[graylog] publish ${topic} failed:`, err)
  }
}
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  // service token holding `usage:write`. Node-hours only — Graylog has no simple
  // built-in daily-ingest metric (see collector.ts).
