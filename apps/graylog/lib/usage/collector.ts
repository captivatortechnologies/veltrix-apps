...
//     it returns null for an infra. Graylog exposes throughput metrics but no
//     single built-in daily-ingest counter, so the poller is unused today —
//     node-hours are the live dimension (the poller stays wired so a future
//     Graylog volume metric can plug straight in).
//
 ...
 * omit it to record node-hours only (the current default — Graylog has no simple
 * built-in daily-ingest metric).
 */
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM graylog_byol_infrastructure',
  )
