...
//   • tiers['graylog']    → Graylog server nodes (web + REST; ALB targets)
//   • tiers['opensearch'] → OpenSearch / Elasticsearch search + storage nodes
//
...
// `nodeTiers` in declared order [graylog, opensearch].
// =============================================================================

import { normalizeControlPlaneLayout, parsePlacement, validatePlacement, type ClusterPlacement } from './byolPlacement'

/** Graylog's two scalable BYOL node tiers, in persisted/display order. */
const TIER_KEYS = ['graylog', 'opensearch'] as const
type TierKey = (typeof TIER_KEYS)[number]

/** Error-message labels — must match client/pages/BYOLPage.tsx's `topology.tiers[].label`. */
const TIER_LABELS: Record<TierKey, string> = {
  graylog: 'Graylog nodes',
  opensearch: 'OpenSearch nodes',
}

/**
 * Minimum node count per tier. Graylog nodes stay permissive (≥1). A DISTRIBUTED
 * OpenSearch / Elasticsearch search cluster needs at least three nodes to form a
 * real quorum, so the opensearch minimum is 3 for distributed deployments (a
 * single-node / eval deployment collapses to one all-in-one box, so every tier
 * is effectively ≥1 there).
 */
function tierMinimum(key: TierKey, isDistributed: boolean): number {
  if (key === 'opensearch' && isDistributed) return 3
  return 1
}
...
export function readByol(body: any): { data: Record<string, unknown>; error?: string } {
  ...
  // "Distributed" is the multi-node Graylog stack (Graylog servers + OpenSearch +
  // MongoDB on separate nodes); single instance is the all-in-one box.
  ...
  // Carried on the record for SDK-form round-tripping; unused by Graylog's
  // resource plan (Graylog has no consolidated control plane / forwarder tier).
