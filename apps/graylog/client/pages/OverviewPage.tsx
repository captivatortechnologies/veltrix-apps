...
/**
 * What this app manages in a Graylog instance, rendered with the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/graylog/meta')
  ...
  if (loading) return <Spinner label="Loading Graylog app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Graylog (open-source SIEM / log management) configuration as code. Author a
          configuration in the Configuration Canvas and deploy it through the pipeline — validate, deploy,
          health check, drift detection and rollback are handled per configuration type. Streams are applied
          over the Graylog REST API (HTTP Basic auth, X-Requested-By CSRF guard, self-signed TLS tolerated).
        </p>
