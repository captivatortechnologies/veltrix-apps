...
import { buildLookupCacheBody } from './_shared'

/** Graylog lookup-table entity names must be lowercase word/dash/dot characters. */
const NAME_REGEX = /^[a-z0-9][a-z0-9_.-]*$/

/**
 * Validate lookup-cache items: a non-empty title, a non-empty name matching
 * Graylog's entity-name convention (the stable identity used for upsert and as
 * the reference lookup tables point to), and a well-formed `config` JSON object
 * with a `type` discriminator (e.g. "guava_cache", "none"). Static — no target
 * access, so per-type required config keys surface at deploy time.
 */
export default async function validate(ctx: PipelineContext): Promise<ValidationResult> {
  ...
  if (items.length === 0) {
    errors.push({ field: 'items', message: 'Add at least one lookup cache.', code: 'EMPTY' })
  }
  ...
  items.forEach((item, i) => {
    ...
    const { error } = buildLookupCacheBody(item.fields)
    if (error) {
