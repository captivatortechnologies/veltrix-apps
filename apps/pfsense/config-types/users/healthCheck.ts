...
/** Health for this config type: REST API package reachability + credential validity, then per-user presence. Read-only. */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (checks[0]?.passed) {
    ...
    try {
      ...
      const names = new Set(live.filter((u) => u.name).map((u) => userKey(u.name)))
      for (const spec of specs) {
        const present = names.has(userKey(spec.name))
        checks.push({ name: `user:${spec.name}`, passed: present, message: present ? `User "${spec.name}" is present.` : `User "${spec.name}" is missing.` })
      }
    } catch (error) {
      checks.push({ name: 'pfsense_users_list', passed: false, message: `Could not list users: ${error instanceof Error ? error.message : 'error'}` })
    }
  }
