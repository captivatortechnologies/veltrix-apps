 ...
 *   2. Every declared, previously-deployed rule (tracked by itemId) still exists live
 * Read-only.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (checks[0]?.passed) {
    ...
    try {
      const [live, prior] = await Promise.all([client.listFirewallRules(), loadPriorEntries(ctx.platform, canvas)])
      ...
      for (const spec of specs) {
        ...
        checks.push({ name: `rule:${label}`, passed: present, message: present ? `Rule "${label}" is present.` : `Rule "${label}" is missing.` })
      }
    } catch (error) {
      checks.push({ name: 'pfsense_rules_list', passed: false, message: `Could not list rules: ${error instanceof Error ? error.message : 'error'}` })
    }
  }
