...
/** Deployment status for a dns-resolver-domain-overrides configuration, from platform records only. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['pfsense'] })
