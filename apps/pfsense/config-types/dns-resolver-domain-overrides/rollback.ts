...
/** Undo a domain-overrides deploy from rollbackData.previous, then apply once via /api/v2/services/dns_resolver/apply. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      if (entry.prior) {
        await client.updateDnsResolverDomainOverride(entry.id, entry.prior)
        restored++
      } else {
        await client.deleteDnsResolverDomainOverride(entry.id)
        deleted++
      }
    }
    ...
    return {
      success: true,
      message: `Rolled back pfSense DNS Resolver domain overrides: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.`,
    }
  } catch (error) {
