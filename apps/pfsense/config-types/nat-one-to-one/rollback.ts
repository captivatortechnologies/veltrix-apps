...
/** Undo a nat-one-to-one deploy from rollbackData.previous, in reverse order, then apply once via /api/v2/firewall/apply. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    for (const entry of [...previous].reverse()) {
      if (entry.prior) {
        await client.updateOneToOneNatMapping(entry.id, entry.prior)
        restored++
      } else {
        await client.deleteOneToOneNatMapping(entry.id)
        deleted++
      }
    }
    ...
    return { success: true, message: `Rolled back pfSense 1:1 NAT mappings: ${restored} restored, ${deleted} deleted.` }
  } catch (error) {
