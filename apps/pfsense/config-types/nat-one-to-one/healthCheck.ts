...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (checks[0]?.passed) {
    ...
    try {
      const [live, prior] = await Promise.all([client.listOneToOneNatMappings(), loadPriorEntries(ctx.platform, canvas)])
      ...
      checks.push({ name: 'pfsense_mappings_list', passed: false, message: `Could not list 1:1 NAT mappings: ${error instanceof Error ? error.message : 'error'}` })
    }
  }
