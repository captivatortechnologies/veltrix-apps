...
import { extractSpecs, scheduleKey } from './_shared'

/** Health for this config type: REST API package reachability + credential validity, then per-schedule presence. Read-only. */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (checks[0]?.passed) {
    ...
    try {
      ...
      const names = new Set(live.filter((s) => s.name).map((s) => scheduleKey(s.name)))
      for (const spec of specs) {
        const present = names.has(scheduleKey(spec.name))
        checks.push({ name: `schedule:${spec.name}`, passed: present, message: present ? `Schedule "${spec.name}" is present.` : `Schedule "${spec.name}" is missing.` })
      }
    } catch (error) {
      checks.push({ name: 'pfsense_schedules_list', passed: false, message: `Could not list schedules: ${error instanceof Error ? error.message : 'error'}` })
    }
  }
