...
/** Undo a firewall-schedules deploy from rollbackData.previous, then apply once via /api/v2/firewall/apply. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      if (entry.prior) {
        await client.updateFirewallSchedule(entry.id, entry.prior)
        restored++
      } else {
        await client.deleteFirewallSchedule(entry.id)
        deleted++
      }
    }
    ...
    return {
      success: true,
      message: `Rolled back pfSense firewall schedules: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.`,
    }
  } catch (error) {
