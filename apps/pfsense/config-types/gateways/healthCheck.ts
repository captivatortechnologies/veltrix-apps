...
/** Health for this config type: REST API package reachability + credential validity, then per-gateway presence. Read-only. */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (checks[0]?.passed) {
    ...
    try {
      ...
      const names = new Set(live.filter((g) => g.name).map((g) => gatewayKey(g.name)))
      for (const spec of specs) {
        const present = names.has(gatewayKey(spec.name))
        checks.push({ name: `gateway:${spec.name}`, passed: present, message: present ? `Gateway "${spec.name}" is present.` : `Gateway "${spec.name}" is missing.` })
      }
    } catch (error) {
      checks.push({ name: 'pfsense_gateways_list', passed: false, message: `Could not list gateways: ${error instanceof Error ? error.message : 'error'}` })
    }
  }
