...
/** Undo a gateways deploy from rollbackData.previous, then apply once via /api/v2/routing/apply. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      if (entry.prior) {
        await client.updateRoutingGateway(entry.id, entry.prior)
        restored++
      } else {
        await client.deleteRoutingGateway(entry.id)
        deleted++
      }
    }
    ...
    return {
      success: true,
      message: `Rolled back pfSense gateways: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.`,
    }
  } catch (error) {
