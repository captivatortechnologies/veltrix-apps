...
/** Undo a user-groups deploy from rollbackData.previous. No apply step — writes apply immediately (see deploy.ts's module doc). */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      if (entry.prior) {
        await client.updateUserGroup(entry.id, entry.prior)
        restored++
      } else {
        await client.deleteUserGroup(entry.id)
        deleted++
      }
    }
    ...
    return {
      success: true,
      message: `Rolled back pfSense user groups: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.`,
    }
  } catch (error) {
