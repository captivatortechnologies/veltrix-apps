...
/**
 * Undo a firewall-aliases deploy from rollbackData.previous (written by
 * deploy()), in reverse deploy order:
 *   - prior === null   -> this deploy CREATED the alias -> DELETE it
 *   - prior !== null   -> this deploy UPDATED an existing alias -> PATCH its
 *                         prior type/descr/address/detail back
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    for (const entry of [...previous].reverse()) {
      ...
      if (entry.prior) {
        await client.updateAlias(entry.id, entry.prior)
        restored++
      } else {
        await client.deleteAlias(entry.id)
        deleted++
      }
    }
    ...
    return {
      success: true,
      message: `Rolled back pfSense firewall aliases: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.`,
    }
  } catch (error) {
