 ...
 *   2. Every declared, previously-deployed port forward (tracked by itemId) still exists live
 * Read-only.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (checks[0]?.passed) {
    ...
    try {
      const [live, prior] = await Promise.all([client.listPortForwards(), loadPriorEntries(ctx.platform, canvas)])
      const liveIds = new Set(live.filter((pf) => pf.id !== undefined).map((pf) => String(pf.id)))
      ...
      for (const spec of specs) {
        ...
        checks.push({ name: `port-forward:${label}`, passed: present, message: present ? `Port forward "${label}" is present.` : `Port forward "${label}" is missing.` })
      }
    } catch (error) {
      checks.push({ name: 'pfsense_port_forwards_list', passed: false, message: `Could not list port forwards: ${error instanceof Error ? error.message : 'error'}` })
    }
  }
