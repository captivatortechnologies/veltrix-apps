...
/** Health for this config type: REST API package reachability + credential validity, then per-route presence. Read-only. */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (checks[0]?.passed) {
    ...
    try {
      const [live, prior] = await Promise.all([client.listStaticRoutes(), loadPriorEntries(ctx.platform, canvas)])
      const liveIds = new Set(live.filter((r) => r.id !== undefined).map((r) => String(r.id)))
      ...
      for (const spec of specs) {
        ...
        checks.push({ name: `route:${label}`, passed: present, message: present ? `Route "${label}" is present.` : `Route "${label}" is missing.` })
      }
    } catch (error) {
      checks.push({ name: 'pfsense_routes_list', passed: false, message: `Could not list routes: ${error instanceof Error ? error.message : 'error'}` })
    }
  }
