...
/** Undo a static-routes deploy from rollbackData.previous, in reverse order, then apply once via /api/v2/routing/apply. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    for (const entry of [...previous].reverse()) {
      if (entry.prior) {
        await client.updateStaticRoute(entry.id, entry.prior)
        restored++
      } else {
        await client.deleteStaticRoute(entry.id)
        deleted++
      }
    }
    ...
    return { success: true, message: `Rolled back pfSense static routes: ${restored} restored, ${deleted} deleted.` }
  } catch (error) {
