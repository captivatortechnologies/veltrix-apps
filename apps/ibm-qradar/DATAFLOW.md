...
**App:** `ibm-qradar` · **Category:** SIEM · **Version:** 0.5.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **IBM QRadar API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as IBM QRadar API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + reference-setswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + reference-setsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: reference-setswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
