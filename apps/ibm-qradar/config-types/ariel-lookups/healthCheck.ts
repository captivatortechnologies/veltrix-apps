...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', '/ariel/lookups', { range: 'items=0-0' })
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'qradar-ariel-lookups',
    passed,
    message: passed ? 'Reached the QRadar Ariel lookups endpoint' : `QRadar error: ${qradarErrorMessage(resp)}`,
    latencyMs,
  })
