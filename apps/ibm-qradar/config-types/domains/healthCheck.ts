...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', '/config/domain_management/domains', { range: 'items=0-0' })
  ...
  checks.push({
    name: 'qradar-domains',
    passed,
    message: passed ? 'Reached the QRadar domain-management endpoint' : `QRadar error: ${qradarErrorMessage(resp)}`,
    latencyMs,
  })
