...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', '/config/access/tenant_management/tenants', { range: 'items=0-0' })
  ...
  checks.push({
    name: 'qradar-tenants',
    passed,
    message: passed ? 'Reached the QRadar tenant-management endpoint' : `QRadar error: ${qradarErrorMessage(resp)}`,
    latencyMs,
  })
