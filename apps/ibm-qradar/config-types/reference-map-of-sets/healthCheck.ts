...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', '/reference_data/map_of_sets', { range: 'items=0-0' })
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'qradar-map-of-sets',
    passed,
    message: passed ? 'Reached the QRadar map-of-sets endpoint' : `QRadar error: ${qradarErrorMessage(resp)}`,
    latencyMs,
  })
