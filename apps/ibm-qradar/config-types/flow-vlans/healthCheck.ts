...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', '/ariel/flow_vlans', { range: 'items=0-0' })
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'qradar-flow-vlans',
    passed,
    message: passed ? 'Reached the QRadar flow-VLANs endpoint' : `QRadar error: ${qradarErrorMessage(resp)}`,
    latencyMs,
  })
