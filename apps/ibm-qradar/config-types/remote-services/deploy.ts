...
import { extractRemoteServiceSpecs, type LiveRemoteService, type RemoteServiceSpec } from './validate'
...
export interface RollbackEntry {
  ...
  name: string
  existed: boolean
  /** the QRadar remote-service id, for rename-safe matching and delete/restore. */
  id?: number
  prior?: RemoteServiceState
}

/**
 * The live remote services, or null when the console could not be read.
 *
 ...
 */
export async function listRemoteServices(client: QRadarClient): Promise<LiveRemoteService[] | null> {
  const res = await client.request('GET', '/staged_config/remote_services', { range: 'items=0-9999' })
  if (!res.ok) return null
  const parsed = parseJson<LiveRemoteService[]>(res.body)
...
function bodyOf(spec: RemoteServiceSpec): RemoteServiceState {
  return { name: spec.name, description: spec.description, group: spec.group, cidrs: spec.cidrs }
}

function stateOf(live: LiveRemoteService): RemoteServiceState {
  return { name: live.name ?? '', description: live.description ?? '', group: live.group ?? '', cidrs: live.cidrs ?? [] }
}

function differs(a: RemoteServiceState, b: RemoteServiceState): boolean {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractRemoteServiceSpecs(ctx.canvas).filter((s) => s.name)
  ...
  if (live === null) {
    return {
      success: false,
      message:
        'Could not read the existing remote services, so nothing was written. ' +
  ...
  for (const spec of specs) {
    ...
    if (existing && typeof existing.id === 'number') {
      ...
      if (differs(priorState, bodyOf(spec))) {
        const resp = await client.request('POST', `/staged_config/remote_services/${existing.id}`, { body: bodyOf(spec) })
        if (!resp.ok) {
      ...
      const resp = await client.request('POST', '/staged_config/remote_services', { body: bodyOf(spec) })
      if (!resp.ok) {
        failures.push(`${spec.name}: ${qradarErrorMessage(resp)}`)
        continue
      }
      const created = parseJson<LiveRemoteService>(resp.body)
      changed = true
      entries.push({ itemId: spec.itemId, name: spec.name, existed: false, id: created?.id })
    }
  }

  // Reconcile: delete remote services THIS app created previously but no longer declares.
  ...
  for (const p of prior) {
    if (!p.existed && typeof p.id === 'number' && !(p.itemId && declaredItemIds.has(p.itemId)) && !declaredNames.has(p.name.toLowerCase())) {
      const resp = await client.request('DELETE', `/staged_config/remote_services/${p.id}`)
  ...
  if (failures.length) {
    return { success: false, message: `Some remote services failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} remote service(s)`, rollbackData: { entries } }
}
