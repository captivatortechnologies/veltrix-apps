...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (!e.existed) {
      ...
      const resp = await client.request('POST', `${PATH}/${e.id}`, { body: { name: e.prior.name } })
  ...
  return { success: true, message: `Rolled back tagged field categories: ${deleted} deleted, ${restored} restored` }
}
