...
/**
 * The live remote networks, or null when the console could not be read.
 *
 ...
 */
export async function listRemoteNetworks(client: QRadarClient): Promise<LiveRemoteNetwork[] | null> {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  if (live === null) {
    return {
      success: false,
      message:
        'Could not read the existing remote networks, so nothing was written. ' +
