...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (!e.existed) {
      const resp = await client.request('DELETE', `/staged_config/remote_networks/${e.id}`)
      if (!resp.ok && resp.status !== 202 && resp.status !== 404) failures.push(`delete ${e.name}: ${qradarErrorMessage(resp)}`)
      else { deleted++; changed = true }
    } else if (e.prior) {
      const resp = await client.request('POST', `/staged_config/remote_networks/${e.id}`, { body: e.prior })
  ...
  return { success: true, message: `Rolled back remote networks: ${deleted} deleted, ${restored} restored` }
}
