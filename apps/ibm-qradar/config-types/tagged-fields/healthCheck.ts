...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', '/ariel/taggedfields', { range: 'items=0-0' })
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'qradar-tagged-fields',
    passed,
    message: passed ? 'Reached the QRadar tagged-fields endpoint' : `QRadar error: ${qradarErrorMessage(resp)}`,
    latencyMs,
  })
