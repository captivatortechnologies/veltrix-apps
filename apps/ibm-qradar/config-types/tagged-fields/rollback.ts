...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (!e.existed) {
      ...
      const resp = await client.request('POST', `${PATH}/${e.id}`, { body: { category_id: e.prior.categoryId, description: e.prior.description } })
  ...
  return { success: true, message: `Rolled back tagged fields: ${deleted} deleted, ${restored} restored` }
}
