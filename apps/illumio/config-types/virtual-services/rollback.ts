...
/** Roll back a virtual services deploy, then re-provision the undo so it takes effect. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  if (changedHrefs.length > 0) {
    try {
      await provisionChanges(base, settings, headers, `Veltrix: rollback virtual services (${changedHrefs.length} change(s))`, {
  ...
  return { success: true, message: `Rolled back virtual services: ${deleted} deleted, ${restored} restored${provisionNote}` }
}
