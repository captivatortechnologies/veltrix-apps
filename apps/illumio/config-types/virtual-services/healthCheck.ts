...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await illumioRequest(`${base}${secPolicyDraftPath(settings, 'virtual_services')}?max_results=1`, {
      headers,
    ...
    const passed = res.ok
    checks.push({
      name: 'illumio-virtual-services',
      passed,
      message: passed
        ? `Reached the PCE draft virtual services endpoint for org ${settings.orgId}`
