...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await illumioRequest(`${base}${orgPath(settings, 'pairing_profiles')}?max_results=1`, {
      headers,
    ...
    const passed = res.ok
    checks.push({
      name: 'illumio-pairing-profiles',
      passed,
      message: passed
        ? `Reached the PCE pairing profiles endpoint for org ${settings.orgId}`
