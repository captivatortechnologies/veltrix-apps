...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await illumioRequest(`${base}${secPolicyDraftPath(settings, 'label_groups')}?max_results=1`, {
      headers,
    ...
    const passed = res.ok
    checks.push({
      name: 'illumio-label-groups',
      passed,
      message: passed
        ? `Reached the PCE draft label groups endpoint for org ${settings.orgId}`
