...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await illumioRequest(`${base}${secPolicyDraftPath(settings, 'rule_sets')}?max_results=1`, {
      headers,
    ...
    const passed = res.ok
    checks.push({
      name: 'illumio-rulesets',
      passed,
      message: passed
        ? `Reached the PCE draft rule sets endpoint for org ${settings.orgId}`
