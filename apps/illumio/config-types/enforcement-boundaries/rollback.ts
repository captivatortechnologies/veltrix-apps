...
/** Roll back an enforcement boundaries deploy, then re-provision the undo so it takes effect. */
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  if (changedHrefs.length > 0) {
    try {
      await provisionChanges(base, settings, headers, `Veltrix: rollback enforcement boundaries (${changedHrefs.length} change(s))`, {
  ...
  return { success: true, message: `Rolled back enforcement boundaries: ${deleted} deleted, ${restored} restored${provisionNote}` }
}
