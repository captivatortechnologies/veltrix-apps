...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await illumioRequest(`${base}${secPolicyDraftPath(settings, 'enforcement_boundaries')}?max_results=1`, {
      headers,
    ...
    const passed = res.ok
    checks.push({
      name: 'illumio-enforcement-boundaries',
      passed,
      message: passed
        ? `Reached the PCE draft enforcement boundaries endpoint for org ${settings.orgId}`
