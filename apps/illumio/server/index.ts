// =============================================================================
// Illumio App — Server Entry Module
//
// Registers Illumio-specific API routes as a Fastify plugin, prefixed with
// /api/apps/illumio/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration
// Canvas, and every write to the PCE goes through the pipeline handlers
// (Illumio REST API v2).
// =============================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext): Promise<void> {
  const { hasPermission, db, manifest } = ctx

  // Metadata for the Overview page: what this app manages, from the manifest.
  fastify.get('/meta', {
    preHandler: [hasPermission('illumio-labels', 'read')],
  ...
  // The tenant's saved app settings (e.g. host, port, org_id, verify_tls).
