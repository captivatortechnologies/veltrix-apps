...
/**
 * Health for entity tags = Vectra answers on its Detect REST API with the configured
 * API token. Tagging has no list-all endpoint, so this probes the same generic
 * Detect API reachability endpoint the rest of this app uses: GET /rules?page_size=1.
 * Any response below 500 counts as reachable (auth nuances surface at deploy time,
 * not here).
 */
