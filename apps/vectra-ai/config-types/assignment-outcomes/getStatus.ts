...
/** Deployment status for an assignment-outcomes configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['vectra-brain', 'standalone'] })
