...
/**
 * Health for assignment outcomes = Vectra answers on its Detect REST API with the
 * configured API token. Read-only: GET /assignment_outcomes. Any response below
 * 500 counts as reachable (auth nuances surface at deploy time, not here).
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await vectraRequest(`${base}/assignment_outcomes`, { headers, timeoutMs: 8000 })
