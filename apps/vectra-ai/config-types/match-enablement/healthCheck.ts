...
/**
 * Health for match enablement = Vectra answers on its Detect REST API with the
 * configured API token. Read-only: GET /vectra-match/available-devices. Any
 * response below 500 counts as reachable (auth/licensing nuances surface at
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await vectraRequest(`${base}/vectra-match/available-devices`, { headers, timeoutMs: 8000 })
