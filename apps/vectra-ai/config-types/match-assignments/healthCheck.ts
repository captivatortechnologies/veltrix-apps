...
/**
 * Health for match assignments = Vectra answers on its Detect REST API with the
 * configured API token. Read-only: GET /vectra-match/assignment. Any response
 * below 500 counts as reachable (auth/licensing nuances surface at deploy time,
 * not here).
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await vectraRequest(`${base}/vectra-match/assignment`, { headers, timeoutMs: 8000 })
