...
/**
 * Health for groups = Vectra answers on its Detect REST API with the configured API
 * token. Read-only: GET /groups?page_size=1. Any response below 500 counts as
 * reachable (auth nuances surface at deploy time, not here). Verify against a live
 * Vectra brain.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await vectraRequest(`${base}/groups?page_size=1`, { headers, timeoutMs: 8000 })
