...
/**
 * Health for internal networks = Vectra answers on its Detect REST API with the
 * configured API token. Read-only: GET /settings/internal_network. Any response
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await vectraRequest(`${base}/settings/internal_network`, { headers, timeoutMs: 8000 })
