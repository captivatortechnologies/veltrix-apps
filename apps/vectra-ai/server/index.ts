// ========================================================================
// Vectra AI App — Server Entry Module
//
// Registers Vectra-specific API routes as a Fastify plugin, prefixed with
// /api/apps/vectra-ai/ and protected by app-level auth + permission middleware.
// Triage-rule CONFIGURATION authoring happens in the Configuration Canvas and
// every config write goes through the pipeline handlers (Vectra Detect REST API).
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('triage-rules', 'read')],
