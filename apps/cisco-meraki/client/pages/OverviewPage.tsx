...
/**
 * Shows what this app manages in a Meraki organization, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/cisco-meraki/meta')
  ...
  if (loading) return <Spinner label="Loading Cisco Meraki app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Cisco Meraki Dashboard network security configuration as code through the Meraki
          Dashboard API v1. Create a configuration in the Configuration Canvas and deploy it through the
          pipeline — validate, deploy, health check, drift detection and rollback are all handled per
          configuration type. The app authenticates with a Meraki Dashboard API key against the fixed
          api.meraki.com base — there is no per-organization API host to configure.
        </p>
