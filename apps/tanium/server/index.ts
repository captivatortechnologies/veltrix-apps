// ========================================================================
// Tanium App — Server Entry Module
//
// Registers Tanium-specific API routes as a Fastify plugin, prefixed with
// /api/apps/tanium/ and protected by app-level auth + permission middleware.
// Computer-group authoring happens in the Configuration Canvas and every config
// write goes through the pipeline handlers (Tanium REST v2 API). This foundation
// exposes read-only /meta + /settings routes that back the Overview page.
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('computer-groups', 'read')],
