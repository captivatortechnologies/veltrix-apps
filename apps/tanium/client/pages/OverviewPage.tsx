...
/**
 * What this app manages in a Tanium deployment, rendered with the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/tanium/meta')
  ...
  if (loading) return <Spinner label="Loading Tanium app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Tanium endpoint-management configuration as code. Author a configuration in the
          Configuration Canvas and deploy it through the pipeline — validate, deploy, health check, drift
          detection and rollback are handled per configuration type. Computer groups are applied over the
          Tanium REST v2 API (HTTPS 443, self-signed tolerated).
        </p>
