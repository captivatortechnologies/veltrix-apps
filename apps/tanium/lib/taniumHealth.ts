// Shared health check for every Tanium config type: the connection credential
// authenticates AND the Tanium REST v2 API answers. Read-only — resolve a session
// (API token verbatim, or username/password via /api/v2/session/login) then GET
...
import { buildTaniumBaseUrl, resolveTaniumSession, taniumRequest, sessionHeader } from './taniumApi'

export async function taniumReachableHealth(ctx: HealthCheckContext): Promise<HealthCheckResult> {
