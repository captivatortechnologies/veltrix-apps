...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const ops = [
        ...
        { op: 'replace', path: '/deadlineDuration', value: e.prior.deadlineDuration },
      ]
  ...
  return { success: true, message: `Rolled back campaign templates: ${deleted} deleted, ${restored} restored` }
}
