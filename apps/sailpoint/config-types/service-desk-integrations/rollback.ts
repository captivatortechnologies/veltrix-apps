...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      // Secret attributes are never read back, so only the scalar fields are restored.
      const ops = [
  ...
  return { success: true, message: `Rolled back service desk integrations: ${deleted} deleted, ${restored} restored` }
}
