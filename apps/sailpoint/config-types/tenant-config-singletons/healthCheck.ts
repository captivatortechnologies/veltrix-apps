...
// Reachability is checked against a stable tenant singleton.
const BASE = '/v3/password-org-config'
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  checks.push({
    name: 'isc-tenant-config',
    passed,
    message: passed ? 'Reached the ISC tenant config endpoints' : `ISC error: ${iscErrorMessage(resp)}`,
    latencyMs,
  })
