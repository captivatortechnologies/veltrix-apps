...
// Provisioning policies are nested; reachability is checked via the parent list.
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  checks.push({
    name: 'isc-provisioning-policies',
    passed,
    message: passed ? 'Reached the ISC sources endpoint' : `ISC error: ${iscErrorMessage(resp)}`,
    latencyMs,
  })
