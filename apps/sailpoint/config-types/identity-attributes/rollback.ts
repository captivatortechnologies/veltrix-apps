...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    if (e.existed && e.prior) {
      const resp = await client.put(`${BASE}/${encodeURIComponent(e.name)}`, e.prior)
  ...
  return { success: true, message: `Rolled back identity attributes: ${deleted} deleted, ${restored} restored` }
}
