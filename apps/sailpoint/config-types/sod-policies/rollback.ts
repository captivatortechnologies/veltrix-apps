...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const ops = [
        ...
        { op: 'replace', path: '/state', value: e.prior.state },
        { op: 'replace', path: '/ownerRef', value: { type: e.prior.ownerType, id: e.prior.ownerId } },
      ]
  ...
  return { success: true, message: `Rolled back SOD policies: ${deleted} deleted, ${restored} restored` }
}
