...
// Reachability is checked against a stable MFA method config endpoint.
const BASE = '/v3/mfa/okta-verify/config'
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  checks.push({
    name: 'isc-mfa-config',
    passed,
    message: passed ? 'Reached the ISC MFA configuration endpoint' : `ISC error: ${iscErrorMessage(resp)}`,
    latencyMs,
  })
