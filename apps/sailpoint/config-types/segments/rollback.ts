...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      // We updated this one — restore its prior name/description/active.
      const ops = [
        { op: 'replace', path: '/name', value: e.prior.name },
        { op: 'replace', path: '/description', value: e.prior.description },
        { op: 'replace', path: '/active', value: e.prior.active },
      ]
  ...
  return { success: true, message: `Rolled back segments: ${deleted} deleted, ${restored} restored` }
}
