...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const ops = [
        ...
        { op: 'replace', path: '/owner', value: { type: 'IDENTITY', id: e.prior.ownerId } },
        { op: 'replace', path: '/deleteThreshold', value: e.prior.deleteThreshold },
      ]
  ...
  return { success: true, message: `Rolled back sources: ${deleted} deleted, ${restored} restored` }
}
