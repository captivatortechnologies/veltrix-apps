...
// Lifecycle states are nested; reachability is checked via the parent list.
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  checks.push({
    name: 'isc-lifecycle-states',
    passed,
    message: passed ? 'Reached the ISC identity-profiles endpoint' : `ISC error: ${iscErrorMessage(resp)}`,
    latencyMs,
  })
