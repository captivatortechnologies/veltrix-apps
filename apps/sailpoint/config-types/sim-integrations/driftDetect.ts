...
import { extractSimIntegrationSpecs, type LiveSimIntegration } from './validate'
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  for (const spec of specs) {
    ...
    if (idSet(live.sources ?? []) !== idSet(spec.sources)) {
      diffs.push({ field: `${spec.name}.sources`, expected: idSet(spec.sources), actual: idSet(live.sources ?? []), severity: 'warning' })
    }
  }
