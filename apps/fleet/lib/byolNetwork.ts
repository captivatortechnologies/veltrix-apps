...
// the POLICY here — which provider/region a Fleet stack targets and what subnet
// size it asks for — while the platform owns the shared Network, the global CIDR
...
/** The subnet a Fleet stack carries (superset of the plan's `network` block). */
...
/**
 * The subnet prefix (mask length) a Fleet stack requests. The resolved network
 * model gives every stack a /24 within its customer's block; kept as a function
 * so the policy can later widen for larger distributed stacks.
 */
...
export async function resolvePlanNetwork(
  ...
  try {
    ...
    console.error('[fleet] network allocation peek failed:', err)
...
export async function reserveDeployNetwork(
  ...
  try {
    ...
    console.error('[fleet] network allocation reserve failed:', err)
    return { tags }
  }
}
