...
//   * ingest_gb: from an INJECTED poller (e.g. the Fleet server's host/metrics
//     API). Skipped when no poller is supplied or it returns null for an infra —
//     because reaching a stack's Fleet server requires the component/
//     connectivity/credential context that BYOL records do not carry yet
//     (follow-up: link a BYOL Fleet stack to its server component so ingest can
//     be polled).
//
 ...
 * omit it to record node-hours only (the current default until the Fleet
 * server's ingest poller is wired).
 */
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM fleet_byol_infrastructure',
  )
