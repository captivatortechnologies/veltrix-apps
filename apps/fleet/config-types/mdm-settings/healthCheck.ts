...
/**
 * Health for the mdm-settings config type: the Fleet server answers on its
 * REST API with the configured credential, AND MDM is turned on for at least
 * one platform — MDM settings have no effect otherwise. Verify against a live
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    ...
    checks.push({
      name: 'mdm_enabled',
      passed: mdmOn,
      message: mdmOn ? 'MDM is turned on for at least one platform.' : 'MDM is not turned on — MDM settings will not take effect.',
    })
  } catch {
