...
/** Deployment status for a configuration-profiles configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['fleet-server', 'standalone'] })
