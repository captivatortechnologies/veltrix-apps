...
/**
 * Health for the global-settings config type = the Fleet server answers on its
 * REST API with the configured credential. Read-only: GET /api/v1/fleet/
 * version. Any response below 500 counts as reachable (auth/permission
 * nuances surface at deploy time, not here). Verify against a live Fleet
 * (fleetdm) instance.
 */
