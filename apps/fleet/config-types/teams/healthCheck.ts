...
/**
 * Health for team config = the Fleet server answers on its REST API with the
 * configured credential. Read-only: GET /api/v1/fleet/version. Any response below
 * 500 counts as reachable (auth/permission and Premium-tier nuances surface at
 * deploy time, not here). Verify the endpoint against a live Fleet (fleetdm)
 * instance.
 */
