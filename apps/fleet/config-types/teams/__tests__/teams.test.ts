 ...
 * and network-free. (Teams is a Fleet Premium feature; the tier check happens at
 * deploy time, not in validate.)
 */
...
const good = {
  name: 'Workstations',
  description: 'All employee laptops',
  comment: 'initial rollout',
}

test('validate rejects an unsafe team name', async () => {
  const res = await validate(ctxOf([{ ...good, name: 'bad name/../x' }]))
  assert.equal(res.valid, false)
  assert.ok(res.errors.some((e) => e.code === 'INVALID_NAME'))
})

test('validate rejects an empty team name', async () => {
  const res = await validate(ctxOf([{ ...good, name: '' }]))
  assert.equal(res.valid, false)
  assert.ok(res.errors.some((e) => e.code === 'EMPTY_NAME'))
})

test('validate accepts a good team (description optional)', async () => {
