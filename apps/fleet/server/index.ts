...
// (/api/v1/fleet). The app additionally OWNS BYOL Fleet-stack provisioning: the
// /byol routes derive a resource plan from a stack's topology (server / MySQL /
// Redis), persist it, and emit provisioning events for downstream workers.
// ========================================================================
...
const DESTROY_STEPS: Array<{ key: string; title: string; detail: string }> = [
  ...
  { key: 'drain', title: 'Draining & decommissioning', detail: 'Stopping the Fleet server and removing compute, database, cache and network.' },
  { key: 'done', title: 'Resources destroyed', detail: 'All resources for this Fleet stack have been removed.' },
]
...
async function emit(events: AppEventPublisher, topic: string, payload: unknown): Promise<void> {
  try {
    ...
    console.error(`[fleet] publish ${topic} failed:`, err)
  }
}
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  // service token holding `usage:write`. Node-hours only until the Fleet server's
  // ingest poller is wired (see collector.ts).
