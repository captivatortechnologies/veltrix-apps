...
/**
 * Shows what this app manages in an Akeyless account, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/akeyless/meta')
  ...
  if (loading) return <Spinner label="Loading Akeyless app details..." />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Akeyless configuration as code through the Akeyless REST API. Create a configuration
          in the Configuration Canvas and deploy it through the pipeline - validate, deploy, health
          check, drift detection, and rollback are all handled per configuration type. Static, dynamic
          and rotated secret <em>values</em> are never read, written or diffed by this app - only the
          declarative configuration objects that produce or govern them.
        </p>
