...
/**
 * Shows what this app manages in a Sophos Central tenant, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/sophos-central/meta')
  ...
  if (loading) return <Spinner label="Loading Sophos Central app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Sophos Central endpoint configuration as code through the Sophos Central public APIs. Create a
          configuration in the Configuration Canvas and deploy it through the pipeline — validate, deploy, health
          check, drift detection and rollback are all handled per configuration type. The app authenticates with an
          OAuth2 client-credentials service principal and resolves your tenant's data-region API host automatically
          via the Who-Am-I API — there is no data region to configure by hand.
        </p>
