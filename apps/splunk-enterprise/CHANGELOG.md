...
### Added
- **Two new config types — License Pools and API Access Tokens — after a full research pass against Splunk's REST API Reference.** This app had only 2 config types (`hec-tokens`, `apps`) and no documented rationale for why. Audited every genuinely declarative Splunk REST write surface this app did not yet manage (indexes, saved searches, data inputs, roles, authentication/LDAP, outputs, props/transforms, lookups, event types, tags, macros, workload pools, license pools, API access tokens, distributed search peers) against the official REST API Reference Manual. Result: everything expressible as a `.conf` file (indexes, roles, saved searches, props/transforms, inputs, outputs, macros, event types, tags, workload management, ...) was **already** covered — deliberately, by design — by authoring it inline in the `apps` config type's App Contents (the same reasoning that retired the old `config-files` type in v1.12.0: a generic REST config write lands in an app's `local/`, the user's override layer, while packaging the identical `.conf` content ships it in `default/`). Two genuinely REST-only, non-`.conf`-representable surfaces were found missing and are added as full config types:
  - **License Pools** (`license-pools`, target: License Server) — `/services/licenser/pools`: daily indexing quota carved from a licensing stack (`Enterprise`/`download-trial`/`Forwarder`/`Free`/`Lite`/`Lite_Free`) and which indexer peers may draw from it. Deploy creates/edits pools, recreating (delete + create) one whose stack changed since Splunk has no "move pool" operation — surfaced as a clear error when the stack's fixed default pool can't be deleted. Health check verifies existence/stack/usable quota; drift detection compares precise byte quota (with a small rounding tolerance), stack, peers, and description, with the same `_audit`-backed who/when attribution `hec-tokens` and `apps` already have.
  - **API Access Tokens** (`auth-tokens`, target: search-head/indexer/heavy-forwarder) — `/services/authorization/tokens` + `/services/admin/token-auth/tokens_auth`: Splunk's Token Authentication feature. Deploy idempotently enables Token Authentication (mirroring how `hec-tokens` enables the global HTTP input first), then creates a token per (username, audience) pair — Splunk gives a token no name of its own — and surfaces the Splunk-generated secret value **once**, at creation, as a masked reveal-once deploy resource exactly like `hec-tokens`. Splunk tokens are immutable after creation (type/expiration/not-before can never be edited, only enabled/disabled status can); a deploy that finds an immutable field drifted deletes the old token and creates a fresh one rather than silently leaving the mismatch.
- **README "Coverage" section** documenting every managed config type + Splunk REST surface, the full list of `.conf` files intentionally folded into `apps` instead of separate REST types (with the REST surface each would otherwise use), and every intentionally excluded surface (license installation, search peer management, license-group activation, one-shot/read-only endpoints, system-namespace `.conf`, secret material) with a sourced one-line reason each.
