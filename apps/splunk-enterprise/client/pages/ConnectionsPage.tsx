...
// Splunk server (component) types this app connects to. A component's `type` is
 ...
 * is matched to the server by convention — `credential.name === hostname` — so
 * a single "connection" row shows both without a schema-level join.
 */
...
/**
 * Connections — the Splunk servers this app talks to (indexers, search heads,
 * cluster managers, forwarders …) together with the credential used to reach
 * each. Full CRUD over the platform's components (server) and credentials
 * (auth) APIs. Secrets are write-only: they can be set here but are never read
 * back — the table only shows whether a secret is stored.
 */
export default function ConnectionsPage() {
  ...
  const openEdit = (row: Connection) => {
    ...
    setForm({
      ...
      port: row.component.port ?? '8089',
      type: row.component.type?.[0] ?? 'indexer',
  ...
  return (
    <Card variant="bordered">
        ...
        {error ? (
          ...
          <DataTable
            ...
            emptyState={{
              ...
              description:
                'Register a Splunk server (indexer, search head, cluster manager, forwarder …) and the credential used to reach it.',
            }}
          />
        )}
      </CardBody>
      ...
      <FormDialog
        ...
        description="A Splunk server the pipeline connects to, plus the credential used to authenticate."
          ...
          <div style={{ borderTop: '1px solid var(--vx-border, #e5e7eb)', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              ...
              <p style={{ margin: '4px 0 0', fontSize: 12, opacity: 0.7 }}>
                How the pipeline authenticates to this server. Leave blank to register the server without a credential.
              </p>
            </div>
            <Select
              label="Auth method"
