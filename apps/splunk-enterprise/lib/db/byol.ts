 ...
 * Falls back to deriving an [indexer, searchHead] pair from the legacy
...
function nodeTiersJson(input: ByolInput): string {
  const tiers: ByolTierDto[] =
    ...
    [
      ...
      { key: 'searchHead', count: input.searchHeadCount, placement: input.searchHeadPlacement ?? null },
    ]
...
export interface ByolInput {
  ...
  /** Generic per-tier node counts + placement, in display order [indexer, searchHead]. Derived from indexerCount/searchHeadCount + placements when omitted. */
