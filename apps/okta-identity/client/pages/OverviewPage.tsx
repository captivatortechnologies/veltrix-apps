...
/**
 * Shows what this app manages in an Okta org, using the platform design-system
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/okta-identity/meta')
  ...
  if (loading) return <Spinner label="Loading Okta app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Okta configuration as code through the Okta Management API. Create a configuration
          in the Configuration Canvas and deploy it through the pipeline — validate, deploy, health
          check, drift detection, and rollback are all handled per configuration type.
        </p>
