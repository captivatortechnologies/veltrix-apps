...
/**
 * Health check for custom-domain configuration:
 ...
 *   2. Every declared custom domain still exists in the org (re-found by domain)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared custom domain still exists (re-found by domain).
  if (reachable.passed) {
    const specs = extractCustomDomainSpecs(ctx.canvas).filter((s) => s.domain)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`custom-domain:${spec.domain}`, async () => {
          const live = await findCustomDomain(client, spec.domain)
          if (!live) throw new Error(`Custom domain "${spec.domain}" does not exist in the Okta org`)
          return `Custom domain "${spec.domain}" is present (validationStatus=${live.validationStatus ?? 'unknown'})`
        }),
      )
    }
  }
