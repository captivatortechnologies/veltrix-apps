// =============================================================================
// custom-domains — getStatus. It reports custom-domain deployment state from the
...
describe('custom-domains getStatus', () => {
  ...
  it('reports the last successful deployment per component without calling Okta', async () => {
    await withFetch([], async (calls) => {
      const result = await getStatus(
        statusContext({ latestDeployment: priorDeployment({ 'item-1': 'dom-live' }) }),
      )
  ...
  it('reports a status per registered component', async () => {
    const result = await getStatus(
      statusContext({
  ...
  it('marks a component healthy exactly at the 80 threshold', async () => {
    const result = await getStatus(
