...
import { mappingLabel, resolveMapping } from './deploy'
import { extractMappingSpecs } from './validate'

/**
 * Health check for profile-mapping configuration:
 ...
 *   2. Every declared (source, target) mapping still resolves to exactly one mapping
 *      (a zero/ambiguous resolve is a real fault — mappings are update-only)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared mapping resolves to exactly one mapping.
  if (reachable.passed) {
    const specs = extractMappingSpecs(ctx.canvas).filter((s) => s.sourceId && s.targetId)
    for (const spec of specs) {
      const label = mappingLabel(spec.sourceId, spec.targetId)
      checks.push(
        await timedCheck(`mapping:${spec.sourceId}:${spec.targetId}`, async () => {
          const mapping = await resolveMapping(client, spec.sourceId, spec.targetId)
          return `Profile mapping resolves for ${label} (id ${mapping.id ?? 'unknown'})`
        }),
      )
    }
  }
