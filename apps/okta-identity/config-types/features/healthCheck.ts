...
/**
 * Health check for feature-toggle configuration:
 ...
 *   2. Every declared feature still exists in the org (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared feature still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractFeatureSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`feature:${spec.name}`, async () => {
          const live = await findFeature(client, spec.name)
          if (!live) throw new Error(`Feature "${spec.name}" does not exist in the Okta org`)
          return `Feature "${spec.name}" is present (status=${live.status ?? 'unknown'})`
        }),
      )
    }
  }
