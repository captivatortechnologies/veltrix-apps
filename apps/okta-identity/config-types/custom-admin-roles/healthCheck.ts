...
import { findRoleByLabel, listRoles } from './deploy'
import { extractRoleSpecs } from './validate'

/**
 * Health check for custom-admin-role configuration:
 ...
 *   2. Every declared role still exists in the org (re-found by label)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared role still exists (re-found by label).
  if (reachable.passed) {
    const specs = extractRoleSpecs(ctx.canvas).filter((s) => s.label)
    if (specs.length > 0) {
      let liveRoles: Awaited<ReturnType<typeof listRoles>> = []
      let listError: string | null = null
      try {
        liveRoles = await listRoles(client)
      } catch (error) {
        listError = error instanceof Error ? error.message : 'Failed to list roles'
      }

      for (const spec of specs) {
        checks.push(
          await timedCheck(`role:${spec.label}`, async () => {
            if (listError) throw new Error(listError)
            const live = findRoleByLabel(liveRoles, spec.label)
            if (!live) throw new Error(`Custom admin role "${spec.label}" does not exist in the Okta org`)
            return `Custom admin role "${spec.label}" is present`
          }),
        )
      }
    }
