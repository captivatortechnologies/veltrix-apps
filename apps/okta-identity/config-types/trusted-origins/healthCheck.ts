...
import { extractTrustedOriginSpecs } from './validate'

/**
 * Health check for trusted-origin configuration:
 ...
 *   2. Every declared trusted origin still exists in the org (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared trusted origin still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractTrustedOriginSpecs(ctx.canvas).filter((s) => s.name && s.origin && s.scopes.length > 0)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`trustedOrigin:${spec.name}`, async () => {
          const live = await findTrustedOrigin(client, spec.name)
          if (!live) throw new Error(`Trusted origin "${spec.name}" does not exist in the Okta org`)
          return `Trusted origin "${spec.name}" is present`
        }),
      )
    }
  }
