// =============================================================================
// users — getStatus. It reports deployment state from the platform data API and
// must never reach the Okta org to do it.
// =============================================================================

import getStatus from '../getStatus'
import { FIXTURE_COMPONENT, priorDeployment, statusContext, withFetch } from '../../../lib/__tests__/fakeOkta'
...
describe('users getStatus', () => {
  ...
  it('reports the last successful deployment per component without calling Okta', async () => {
    await withFetch([], async (calls) => {
      const result = await getStatus(
        statusContext({ latestDeployment: priorDeployment({ 'item-1': '00u1' }) }),
      )
  ...
  it('marks a component unhealthy when the recorded health score is below 80', async () => {
    const result = await getStatus(
      statusContext({
        latestDeployment: priorDeployment({}, { healthScore: 40 }),
      }),
    )
