...
/**
 * Health check for custom email-domain configuration:
 ...
 *   2. Every declared email domain still exists in the org (re-found by domain)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared email domain still exists (re-found by domain).
  if (reachable.passed) {
    const specs = extractEmailDomainSpecs(ctx.canvas).filter((s) => s.domain)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`email-domain:${spec.domain}`, async () => {
          const live = await findEmailDomain(client, spec.domain)
          if (!live) throw new Error(`Email domain "${spec.domain}" does not exist in the Okta org`)
          return `Email domain "${spec.domain}" is present (validationStatus=${live.validationStatus ?? 'unknown'})`
        }),
      )
    }
  }
