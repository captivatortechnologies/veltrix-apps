...
import { findEventHook } from './deploy'
import { extractEventHookSpecs } from './validate'

/**
 * Health check for event-hook configuration:
 ...
 *   2. Every declared hook still exists in the org (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared hook still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractEventHookSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`event-hook:${spec.name}`, async () => {
          const live = await findEventHook(client, spec.name)
          if (!live) throw new Error(`Event hook "${spec.name}" does not exist in the Okta org`)
          return `Event hook "${spec.name}" is present`
        }),
      )
    }
  }
