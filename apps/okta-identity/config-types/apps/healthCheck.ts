...
import { extractAppSpecs } from './validate'

/**
 * Health check for application-instance configuration:
 ...
 *   2. Every declared app still exists in the org (re-found by label + signOnMode;
 *      an ambiguous label match fails the check with a clear message)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared app still exists (re-found by label + signOnMode).
  if (reachable.passed) {
    const specs = extractAppSpecs(ctx.canvas).filter((s) => s.label && s.signOnMode)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`app:${spec.label} (${spec.signOnMode})`, async () => {
          const live = await findApp(client, spec.label, spec.signOnMode)
          if (!live) throw new Error(`App "${spec.label}" (${spec.signOnMode}) does not exist in the Okta org`)
          return `App "${spec.label}" (${spec.signOnMode}) is present`
        }),
      )
    }
  }
