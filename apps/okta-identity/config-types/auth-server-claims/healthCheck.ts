...
import { extractClaimSpecs } from './validate'

/**
 * Health check for authorization-server claim configuration:
 ...
 *   2. Every declared claim still exists under its parent authorization server
 *      (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared claim still exists under its auth server.
  if (reachable.passed) {
    const specs = extractClaimSpecs(ctx.canvas).filter((s) => s.authServerId && s.name)
    for (const spec of specs) {
      const label = `${spec.authServerId}:${spec.name}`
      checks.push(
        await timedCheck(`claim:${label}`, async () => {
          const live = await findClaim(client, spec.authServerId, spec.name)
          if (!live) throw new Error(`Claim "${spec.name}" does not exist under authorization server "${spec.authServerId}"`)
          return `Claim "${label}" is present`
        }),
      )
    }
  }
