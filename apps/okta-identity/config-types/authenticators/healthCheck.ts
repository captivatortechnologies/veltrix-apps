...
import { authenticatorIdentity, extractAuthenticatorSpecs } from './validate'

/**
 * Health check for authenticator configuration:
 ...
 *   2. Every declared authenticator still exists in the org (re-found by key,
 *      or key+name for a multi-instance key)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('okta_reachable', async () => {
    ...
    if (res.status === 401 || res.status === 403) {
      throw new Error(
        'Okta rejected the API token (SSWS) — check it is valid and belongs to an admin with read access',
      )
    }
  ...
  // Check 2..n: each declared authenticator still exists.
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      ...
      checks.push(
        await timedCheck(`authenticator:${identity}`, async () => {
          const live = await findAuthenticator(client, spec)
          if (!live) throw new Error(`Authenticator "${identity}" does not exist in the Okta org`)
          return `Authenticator "${identity}" is present (status ${live.status ?? 'unknown'})`
        }),
      )
    }
  }
