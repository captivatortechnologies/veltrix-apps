 ...
 *   2. Every declared brand still exists in the org (re-found by name)
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`brand:${spec.name}`, async () => {
          ...
          if (!live) throw new Error(`Brand "${spec.name}" does not exist in the Okta org`)
