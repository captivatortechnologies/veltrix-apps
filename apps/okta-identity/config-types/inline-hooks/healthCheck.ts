...
import { extractInlineHookSpecs } from './validate'

/**
 * Health check for inline-hook configuration:
 ...
 *   2. Every declared hook still exists in the org (re-found by (name, type))
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared hook still exists (re-found by (name, type)).
  if (reachable.passed) {
    const specs = extractInlineHookSpecs(ctx.canvas).filter((s) => s.name && s.type)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`hook:${spec.type}:${spec.name}`, async () => {
          const live = await findInlineHook(client, spec.name, spec.type)
          if (!live) throw new Error(`Inline hook "${spec.type}:${spec.name}" does not exist in the Okta org`)
          return `Inline hook "${spec.type}:${spec.name}" is present`
        }),
      )
    }
  }
