// =============================================================================
// inline-hooks — healthCheck, driven against the fake Okta org.
//
// A hook that vanished does not fail loudly: Okta simply stops calling it and
// the token/sign-in customisation silently disappears. The check must degrade to
// a FAILED check rather than throwing, and must never print the SSWS token or a
// hook secret into a check message.
// =============================================================================
...
function hook(name: string, type = HOOK_TYPE): CanvasItemInput {
  return {
    name,
    fields: {
      name,
...
const live = (name: string, type = HOOK_TYPE): Record<string, unknown> => ({
  id: 'ih1',
  name,
  type,
  status: 'ACTIVE',
  channel: { type: 'HTTP', version: '1.0.0', config: { uri: 'https://hooks.example.com/token' } },
})

describe('inline-hooks healthCheck', () => {
  ...
  it('passes a check per declared hook and scores 100 when all are present', async () => {
    ...
    expect(result.checks[1].name).toBe(`hook:${HOOK_TYPE}:A`)
    expect(result.checks[1].passed).toBe(true)
  })

  it('looks each hook up filtered by its own type', async () => {
    await withFetch([ok({ id: 'org1' }), ok([live('A')])], async (calls) => {
      await healthCheck(healthContext({ sections: [hook('A')] }))
      expect(calls[1].query.type).toBe(HOOK_TYPE)
    })
  })
  ...
  it('fails the check for a declared hook that no longer exists in the org', async () => {
    ...
    const missing = result.checks.find((c) => c.name === `hook:${HOOK_TYPE}:Gone`)
    expect(missing).toBeDefined()
    expect(missing?.passed).toBe(false)
    expect(missing?.message).toMatch(/does not exist/)
  })

  it('fails the check when only a same-named hook of a different type is present', async () => {
    const result = await withFetch(
      [ok({ id: 'org1' }), ok([live('A', 'com.okta.saml.tokens.transform')])],
      async () => healthCheck(healthContext({ sections: [hook('A')] })),
    )

    expect(result.checks[1].passed).toBe(false)
    expect(result.checks[1].message).toMatch(/does not exist/)
  })
  ...
  it('turns a failed hook listing into a failed check instead of throwing', async () => {
    ...
    expect(result.healthy).toBe(false)
    expect(result.checks[1].passed).toBe(false)
    expect(result.checks[1].message).toMatch(/Failed to list inline hooks/)
    expect(leaksToken(result)).toBe(false)
