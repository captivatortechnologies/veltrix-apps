// =============================================================================
// rate-limit-settings — getStatus. It reports deployment state from the platform data
...
describe('rate-limit-settings getStatus', () => {
  ...
  it('reports the last successful deployment per component without calling Okta', async () => {
    await withFetch([], async (calls) => {
      const result = await getStatus(
        statusContext({ latestDeployment: priorDeployment({ 'item-1': 'rate-limits' }) }),
      )
