// =============================================================================
// authorization-servers — getStatus. It reports deployment state from the
// platform data API and must never reach the Okta org to do it.
// =============================================================================
...
describe('authorization-servers getStatus', () => {
  ...
  it('reports the last successful deployment per component without calling Okta', async () => {
    await withFetch([], async (calls) => {
      const result = await getStatus(
        statusContext({ latestDeployment: priorDeployment({ 'item-1': 'aus1' }) }),
      )
  ...
  it('falls back to startedAt when the deployment never recorded a completion time', async () => {
    ...
    // The per-component row does NOT share that fallback — it stays empty.
    expect(result.componentStatuses[0].lastDeployedAt).toBe('')
  })

  it('reports one status row per registered okta-org component', async () => {
    const result = await getStatus(
      statusContext({
  ...
  it('reports no component rows when no okta-org component is registered', async () => {
    const result = await getStatus(
