...
/**
 * Health check for authorization-server configuration:
 ...
 *   2. Every declared authorization server still exists (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared authorization server still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractAuthServerSpecs(ctx.canvas).filter((s) => s.name && s.audiences.length === 1)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`authServer:${spec.name}`, async () => {
          const live = await findAuthServer(client, spec.name)
          if (!live) throw new Error(`Authorization server "${spec.name}" does not exist in the Okta org`)
          return `Authorization server "${spec.name}" is present`
        }),
      )
    }
  }
