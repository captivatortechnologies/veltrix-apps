// =============================================================================
// resource-set-bindings — getStatus. It reports deployment state from the platform
...
describe('resource-set-bindings getStatus', () => {
  ...
  it('reports the last successful deployment per component without calling Okta', async () => {
    await withFetch([], async (calls) => {
      const result = await getStatus(
        statusContext({ latestDeployment: priorDeployment({ 'item-1': 'Helpdesk scope:cr0HELPDESK' }) }),
      )
  ...
  it('treats exactly 80 as healthy', async () => {
    const result = await getStatus(
  ...
  it('reports one status per registered org component', async () => {
    const result = await getStatus(
      statusContext({
  ...
  it('reports no component statuses when no org component is registered', async () => {
    const result = await getStatus(
