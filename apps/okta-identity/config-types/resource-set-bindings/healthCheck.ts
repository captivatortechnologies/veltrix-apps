...
import { getBinding } from './deploy'
import { extractBindingSpecs } from './validate'

/**
 * Health check for resource-set-binding configuration:
 ...
 *   2. Every declared (resourceSet, role) binding still exists (GET the binding;
 *      a 404 means the grant is gone — e.g. its last member was unassigned)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared binding still exists.
  if (reachable.passed) {
    const specs = extractBindingSpecs(ctx.canvas).filter((s) => s.resourceSet && s.role)
    for (const spec of specs) {
      const label = `${spec.resourceSet}:${spec.role}`
      checks.push(
        await timedCheck(`binding:${label}`, async () => {
          const live = await getBinding(client, spec.resourceSet, spec.role)
          if (!live) {
            throw new Error(`Binding of role "${spec.role}" in resource set "${spec.resourceSet}" does not exist`)
          }
          return `Binding "${label}" is present`
        }),
      )
    }
  }
