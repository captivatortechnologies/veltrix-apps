...
import { extractZoneSpecs } from './validate'

/**
 * Health check for network-zone configuration:
 ...
 *   2. Every declared zone still exists in the org (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared zone still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractZoneSpecs(ctx.canvas).filter((s) => s.name && s.type)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`zone:${spec.name}`, async () => {
          const live = await findZone(client, spec.name)
          if (!live) throw new Error(`Zone "${spec.name}" does not exist in the Okta org`)
          return `Zone "${spec.name}" is present`
        }),
      )
    }
  }
