...
import { extractSmsTemplateSpecs } from './validate'

/**
 * Health check for custom SMS-template configuration:
 ...
 *   2. Every declared template still exists in the org (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared template still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractSmsTemplateSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`sms-template:${spec.name}`, async () => {
          const live = await findSmsTemplate(client, spec.name)
          if (!live) throw new Error(`SMS template "${spec.name}" does not exist in the Okta org`)
          return `SMS template "${spec.name}" is present`
        }),
      )
    }
  }
