...
import { extractDeviceAssuranceSpecs } from './validate'

/**
 * Health check for device-assurance-policy configuration:
 ...
 *   2. Every declared policy still exists in the org (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    const specs = extractDeviceAssuranceSpecs(ctx.canvas).filter((s) => s.name && s.platform)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`device-assurance:${spec.name}`, async () => {
          const live = await findDeviceAssurance(client, spec.name)
          if (!live) throw new Error(`Device assurance policy "${spec.name}" does not exist in the Okta org`)
          return `Device assurance policy "${spec.name}" is present`
        }),
      )
    }
  }
