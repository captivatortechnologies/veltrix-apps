...
import { extractIdpSpecs } from './validate'
 ...
 *   2. Every declared IdP still exists in the org (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared IdP still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractIdpSpecs(ctx.canvas).filter((s) => s.name && s.type)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`idp:${spec.name}`, async () => {
          const live = await findIdp(client, spec.name)
          if (!live) throw new Error(`IdP "${spec.name}" does not exist in the Okta org`)
          return `IdP "${spec.name}" is present`
        }),
      )
    }
  }
