...
import { extractLogStreamSpecs } from './validate'

/**
 * Health check for log-stream configuration:
 ...
 *   2. Every declared stream still exists in the org (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared stream still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractLogStreamSpecs(ctx.canvas).filter((s) => s.name && s.type)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`log-stream:${spec.name}`, async () => {
          const live = await findLogStream(client, spec.name)
          if (!live) throw new Error(`Log stream "${spec.name}" does not exist in the Okta org`)
          return `Log stream "${spec.name}" is present`
        }),
      )
    }
  }
