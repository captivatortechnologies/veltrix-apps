...
import { EXCEPTION_ENDPOINTS } from './_shared'

/**
 * Health for the exception-list config = Trend Vision One answers on its public
 * API with the configured API key. Read-only: GET
 * /threatintel/suspiciousObjectExceptions (top=1). A response below 500 counts as
 * reachable; 401/403 mean the key is bad. VERIFY the probe endpoint against a live
 * Vision One tenant.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.get(`${EXCEPTION_ENDPOINTS.list}?top=1`)
