...
import { ACCOUNT_ENDPOINTS } from './_shared'

/**
 * Health for the user-accounts config = Trend Vision One answers on its public
 * API with the configured API key. Read-only: GET /iam/accounts (top=1). A
 * response below 500 counts as reachable; 401/403 mean the key is bad (or lacks
 * IAM scope). VERIFY the probe endpoint against a live Vision One tenant.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.get(`${ACCOUNT_ENDPOINTS.list}?top=1`)
