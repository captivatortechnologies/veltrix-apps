...
import { CUSTOM_SCRIPT_LIST } from './_shared'

/**
 * Health for the custom-scripts config = Trend Vision One answers on its public
 * API with the configured API key. Read-only: GET /response/customScripts (top=1).
 * A response below 500 counts as reachable; 401/403 mean the key is bad (or lacks
 * Response Management scope). VERIFY the probe endpoint against a live Vision One
 * tenant.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await client.get(`${CUSTOM_SCRIPT_LIST}?top=1`)
