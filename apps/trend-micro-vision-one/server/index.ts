// ========================================================================
// Trend Micro Vision One App — Server Entry Module
//
// Registers Vision One-specific API routes as a Fastify plugin, prefixed with
// /api/apps/trend-micro-vision-one/ and protected by app-level auth + permission
// middleware. Read-only on the platform side: authoring happens in the
// Configuration Canvas, and every write goes through the pipeline handlers (Trend
// Vision One public REST API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('suspicious-objects', 'read')],
