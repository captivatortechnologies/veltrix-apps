# 🛡️ Trend Micro Vision One — Request Flows
...
**App:** `trend-micro-vision-one` · **Category:** EDR · **Version:** 0.2.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Trend Micro Vision One API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
<sub>Applies to: 3 config types · 2 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Trend Micro Vision One API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + suspicious-objectswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + suspicious-objectsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: suspicious-objectswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
