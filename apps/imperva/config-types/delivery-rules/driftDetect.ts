...
/**
 * Drift for delivery rules: compare the action-specific parameters, filter and
 * enabled state we declare against the live rule in Imperva (matched by name
 * within its site). Best-effort — a rule that can't be matched is skipped rather
 * than raising false drift. Read-only: POST /sites/incapRules/list per distinct
 * site.
 */
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  for (const item of items) {
    ...
    if (liveEnabled !== fields.enabled) {
      diffs.push({ field: `${label}.enabled`, expected: String(fields.enabled), actual: String(liveEnabled), severity: 'warning' })
    }

    const declared = declaredDeliveryValues(fields)
    const live = liveDeliveryValues(match, kind)
    for (const key of Object.keys({ ...declared, ...live })) {
      if (key === 'name' || key === 'action' || key === 'enabled' || key === 'filter') continue
      const expected = declared[key] ?? ''
      const actual = live[key] ?? ''
      if (expected !== actual) diffs.push({ field: `${label}.${key}`, expected, actual, severity: 'warning' })
    }
  }
