...
/**
 * Undo a delivery rules deploy from rollbackData.previous (written by deploy()):
 ...
 *
 * Applied over the same v1 IncapRules endpoints as ACL Rules. A rule whose id we
 * never learned is skipped (nothing safe to undo).
 */
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    ...
    return {
      success: true,
      message: `Rolled back delivery rules: ${restored} restored, ${deleted} deleted${skipped ? `, ${skipped} skipped` : ''}.`,
    }
  } catch (error) {
