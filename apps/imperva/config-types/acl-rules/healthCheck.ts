...
/**
 * Health for acl-rules config = the Cloud WAF (Incapsula) API v1 answers to an
 * authenticated request. Read-only: POST /account (account details). A `res === 0`
 * envelope confirms the endpoint resolves AND the API ID / API key authenticate.
 */
