...
**App:** `imperva` · **Category:** NETWORK · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Imperva API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Imperva API
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + acl-ruleswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + acl-rulesread
  rbac-->>pipeline: permitted ✓ / denied
