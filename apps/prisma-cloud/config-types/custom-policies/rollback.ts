...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      // Restore the prior policy object verbatim.
      const resp = await client.put(`${SINGLE}/${e.id}`, e.prior)
      if (!resp.ok && resp.status !== 404) failures.push(`restore ${e.name}: ${pcErrorMessage(resp)}`)
      else restored++
    } else if (!e.existed) {
      const resp = await client.delete(`${SINGLE}/${e.id}`)
  ...
  return { success: true, message: `Rolled back custom policies: ${deleted} deleted, ${restored} restored` }
}
