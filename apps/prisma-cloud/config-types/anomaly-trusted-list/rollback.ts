...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const resp = await client.put(`${BASE}/${e.id}`, {
        ...
        trustedListType: e.prior.trustedListType,
        accountId: e.prior.accountId,
        vpc: e.prior.vpc,
        applicablePolicies: e.prior.applicablePolicies,
        trustedListEntries: e.prior.trustedListEntries,
      })
  ...
  return { success: true, message: `Rolled back anomaly trusted lists: ${deleted} deleted, ${restored} restored` }
}
