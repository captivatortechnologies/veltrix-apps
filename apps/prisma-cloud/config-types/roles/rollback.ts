...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      ...
      const resp = await client.put(`${BASE}/${e.id}`, {
        name: e.prior.name,
        roleType: e.prior.roleType,
        description: e.prior.description,
        accountGroupIds: e.prior.accountGroupIds,
        resourceListIds: e.prior.resourceListIds,
        restrictDismissalAccess: e.prior.restrictDismissalAccess,
      })
  ...
  return { success: true, message: `Rolled back roles: ${deleted} deleted, ${restored} restored` }
}
