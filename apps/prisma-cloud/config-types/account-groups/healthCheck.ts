...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(`${BASE}/name`)
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'prisma-account-groups',
    passed,
    message: passed ? 'Reached the Prisma Cloud account-group endpoint' : `Prisma Cloud error: ${pcErrorMessage(resp)}`,
    latencyMs,
  })
