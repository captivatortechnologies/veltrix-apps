...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const resp = await client.put(`${BASE}/${e.id}`, { name: e.prior.name, description: e.prior.description, accountIds: e.prior.accountIds })
  ...
  return { success: true, message: `Rolled back account groups: ${deleted} deleted, ${restored} restored` }
}
