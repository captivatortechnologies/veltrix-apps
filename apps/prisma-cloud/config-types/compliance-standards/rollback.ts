...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      // We updated this one — restore its prior name/description.
      const resp = await client.put(`${BASE}/${e.id}`, { name: e.prior.name, description: e.prior.description })
  ...
  return { success: true, message: `Rolled back compliance standards: ${deleted} deleted, ${restored} restored` }
}
