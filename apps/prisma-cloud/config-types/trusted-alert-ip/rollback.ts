...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const resp = await client.put(`${BASE}/${e.id}`, {
        name: e.prior.name,
        cidrs: e.prior.cidrs.map((c) => (c.description ? { cidr: c.cidr, description: c.description } : { cidr: c.cidr })),
      })
  ...
  return { success: true, message: `Rolled back trusted alert IPs: ${deleted} deleted, ${restored} restored` }
}
