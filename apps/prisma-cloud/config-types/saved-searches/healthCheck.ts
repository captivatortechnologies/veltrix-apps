...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.get(`${BASE}?filter=saved`)
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'prisma-saved-searches',
    passed,
    message: passed ? 'Reached the Prisma Cloud saved search endpoint' : `Prisma Cloud error: ${pcErrorMessage(resp)}`,
    latencyMs,
  })
