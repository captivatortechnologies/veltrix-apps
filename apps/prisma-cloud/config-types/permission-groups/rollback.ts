...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const resp = await client.put(`${BASE}/${e.id}`, {
        ...
        permissionGroupType: e.prior.permissionGroupType,
        acceptAccountGroups: e.prior.acceptAccountGroups,
        acceptResourceLists: e.prior.acceptResourceLists,
        acceptCodeRepositories: e.prior.acceptCodeRepositories,
        features: e.prior.features,
      })
  ...
  return { success: true, message: `Rolled back permission groups: ${deleted} deleted, ${restored} restored` }
}
