...
**App:** `prisma-cloud` · **Category:** CLOUD · **Version:** 0.5.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Prisma Cloud API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 17 config types · 1 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Prisma Cloud API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + compliance-standardswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + compliance-standardsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: compliance-standardswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
