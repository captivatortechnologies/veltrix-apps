...
/** Deployment status for a Conditional Access Policies configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['jumpcloud-org'] })
