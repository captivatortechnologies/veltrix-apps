...
import { listUserGroups, findGroupByName } from './deploy'
import { extractMembershipSpecs } from './_shared'

/**
 * Health check for Group Membership configuration:
 *   1. JumpCloud API reachability + key validity (GET /usergroups — a 401/403
 *      means the API key was rejected).
 *   2. Every declared target group still exists in the org (matched by name).
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  let groups: Awaited<ReturnType<typeof listUserGroups>> = []
  let reachable = false
  ...
  if (reachable) {
    for (const spec of extractMembershipSpecs(ctx.canvas).filter((s) => s.groupName)) {
      const live = findGroupByName(groups, spec.groupName)
      checks.push({
        name: `group:${spec.groupName}`,
        passed: Boolean(live),
        message: live ? `User Group "${spec.groupName}" is present.` : `User Group "${spec.groupName}" was not found in the org.`,
      })
    }
  }
