// ========================================================================
// JumpCloud App — Server Entry Module
//
// Registers JumpCloud-specific API routes as a Fastify plugin, prefixed with
// /api/apps/jumpcloud/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration Canvas,
// and every write to JumpCloud goes through the pipeline handlers (JumpCloud
// REST API). These routes back the Overview page.
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('user-groups', 'read')],
