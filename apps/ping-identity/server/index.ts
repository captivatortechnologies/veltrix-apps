// ========================================================================
// Ping Identity App - Server Entry Module
//
// Registers app-specific API routes as a Fastify plugin, prefixed with
// /api/apps/ping-identity/ and protected by app-level auth + permission
// middleware. Read-only on the platform side: authoring happens in the
// Configuration Canvas, and every write to PingOne goes through the pipeline
// handlers (PingOne Platform API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('sign-on-policies', 'read')],
