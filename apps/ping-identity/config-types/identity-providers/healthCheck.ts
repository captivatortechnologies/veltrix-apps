...
import { extractIdentityProviderSpecs } from './validate'

/**
 * Health check for identity-provider configuration:
 *   1. PingOne environment reachability + worker credential validity
 *      (GET /environments/{id} - proves the token is accepted and the
 *      environment is readable; 401/403 means the worker credential was rejected)
 *   2. Every declared identity provider still exists in the environment
 *      (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared identity provider still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractIdentityProviderSpecs(ctx.canvas).filter((s) => s.name && s.type)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`identity-provider:${spec.name}`, async () => {
          const live = await findIdentityProvider(client, spec.name)
          if (!live) throw new Error(`Identity provider "${spec.name}" does not exist in the PingOne environment`)
          return `Identity provider "${spec.name}" is present`
        }),
      )
    }
  }
