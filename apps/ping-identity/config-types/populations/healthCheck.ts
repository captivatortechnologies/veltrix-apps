...
/**
 * Health check for population configuration:
 ...
 *   2. Every declared population still exists in the environment (re-found by name)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 2..n: each declared population still exists (re-found by name).
  if (reachable.passed) {
    const specs = extractPopulationSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`population:${spec.name}`, async () => {
          const live = await findPopulation(client, spec.name)
          if (!live) throw new Error(`Population "${spec.name}" does not exist in the PingOne environment`)
          return `Population "${spec.name}" is present`
        }),
      )
    }
  }
