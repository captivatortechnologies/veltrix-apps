// ========================================================================
// Keycloak App — Server Entry Module
//
// Registers Keycloak-specific API routes as a Fastify plugin, prefixed with
// /api/apps/keycloak/ and protected by app-level auth + permission middleware.
// Client CONFIGURATION authoring happens in the Configuration Canvas and every
// config write goes through the pipeline handlers (Keycloak Admin REST API).
// This foundation exposes read-only /meta + /settings routes that back the
// Overview page. The app additionally OWNS BYOL stack provisioning: the /byol
...
async function emit(events: AppEventPublisher, topic: string, payload: unknown): Promise<void> {
  try {
    ...
    console.error(`[keycloak] publish ${topic} failed:`, err)
  }
}
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('clients', 'read')],
  ...
  // service token holding `usage:write`. Node-hours only — Keycloak has no
  // built-in ingest-volume metric (see collector.ts).
