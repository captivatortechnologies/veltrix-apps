...
//     it returns null for an infra. Keycloak has no ingest-volume metric, so the
//     poller is unused today — node-hours are the live dimension (the poller
//     stays wired so a future Keycloak volume metric can plug straight in).
//
 ...
 * omit it to record node-hours only (the current default — Keycloak has no
...
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM keycloak_byol_infrastructure',
  )
