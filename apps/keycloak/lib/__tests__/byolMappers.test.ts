...
test('mapByol parses an already-decoded node_tiers array (the normal pg jsonb shape)', () => {
  const dto = mapByol({
    ...baseRow,
    node_tiers: [{ key: 'server', count: 2, placement: null }],
  })
  assert.deepEqual(dto.tiers, [{ key: 'server', count: 2, placement: null }])
})

test('mapByol parses a raw JSON string node_tiers value', () => {
  const dto = mapByol({
    ...baseRow,
    node_tiers: JSON.stringify([{ key: 'server', count: 1, placement: null }]),
  })
  assert.deepEqual(dto.tiers, [{ key: 'server', count: 1, placement: null }])
})
...
test('mapByol carries a tier placement through', () => {
  ...
  const dto = mapByol({
    ...baseRow,
    node_tiers: [{ key: 'server', count: 3, placement }],
  })
