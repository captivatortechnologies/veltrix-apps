...
test('createByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  await createByol(db, 'cust-1', {
    ...baseInput,
    nodeTiers: [{ key: 'server', count: 3, placement: null }],
  })

  const insert = calls.find((c) => c.sql.includes('INSERT INTO keycloak_byol_infrastructure'))
  assert.ok(insert)
  assert.match(insert!.sql, /node_tiers/)
  const nodeTiersArg = insert!.args[insert!.args.length - 1] as string
  assert.deepEqual(JSON.parse(nodeTiersArg), [{ key: 'server', count: 3, placement: null }])
})
...
test('updateByol writes the caller-supplied nodeTiers verbatim', async () => {
  ...
  await updateByol(db, 'i1', {
    ...baseInput,
    nodeTiers: [{ key: 'server', count: 5, placement: null }],
  })

  const update = calls.find((c) => c.sql.includes('UPDATE keycloak_byol_infrastructure'))
  assert.ok(update)
  assert.match(update!.sql, /node_tiers\s*=/)
  const nodeTiersArg = update!.args[update!.args.length - 1] as string
  assert.deepEqual(JSON.parse(nodeTiersArg), [{ key: 'server', count: 5, placement: null }])
})
