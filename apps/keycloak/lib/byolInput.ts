...
// placement }]` array. This app declares ONE scalable tier (see
// client/pages/BYOLPage.tsx's `topology` prop and lib/byolTopology.ts):
//   • tiers['server'] → Keycloak IAM server nodes (ALB targets, Infinispan-clustered)
//
...
// `nodeTiers` in declared order [server].
// =============================================================================

import { normalizeControlPlaneLayout, parsePlacement, validatePlacement, type ClusterPlacement } from './byolPlacement'

/** Keycloak's scalable BYOL node tiers, in persisted/display order. */
const TIER_KEYS = ['server'] as const
type TierKey = (typeof TIER_KEYS)[number]

/** Error-message labels — must match client/pages/BYOLPage.tsx's `topology.tiers[].label`. */
const TIER_LABELS: Record<TierKey, string> = {
  server: 'Keycloak nodes',
}

/**
 * Minimum node count per tier. Keycloak clusters from a single node upward, so
 * the `server` tier stays permissive (≥1) for both single and distributed
 * deployments.
 */
function tierMinimum(_key: TierKey, _isDistributed: boolean): number {
  return 1
}
...
export function readByol(body: any): { data: Record<string, unknown>; error?: string } {
  ...
  // "Distributed" is the multi-node Keycloak stack (clustered Keycloak servers +
  // a dedicated PostgreSQL datastore); single instance is the all-in-one box.
  ...
  // Carried on the record for SDK-form round-tripping; unused by Keycloak's
  // resource plan (Keycloak has no consolidated control plane / forwarder tier).
