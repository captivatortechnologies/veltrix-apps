...
// `keycloak_byol_infrastructure` table.
//
...
export interface ByolInput {
  ...
  // populated by lib/byolInput.ts's readByol, in [server] order.
  nodeTiers?: NodeTierDto[]
}

export async function listByol(db: PlatformDatabaseClient, customerId: string): Promise<ByolDto[]> {
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM keycloak_byol_infrastructure WHERE customer_id = $1::uuid ORDER BY updated_at DESC',
    customerId,
  )
...
export async function getByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM keycloak_byol_infrastructure WHERE id = $1::uuid AND customer_id = $2::uuid',
    id,
    customerId,
  )
...
export async function createByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    ...
    `INSERT INTO keycloak_byol_infrastructure
...
export async function updateByol(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    `UPDATE keycloak_byol_infrastructure SET
...
export async function setByolStatus(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'UPDATE keycloak_byol_infrastructure SET status = $2, updated_at = now() WHERE id = $1::uuid RETURNING *',
    id,
    status,
  )
...
export async function deleteByol(db: PlatformDatabaseClient, id: string): Promise<void> {
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM keycloak_byol_infrastructure WHERE id = $1::uuid',
    id,
  )
  if (rows[0]) await emitStateEvent(db, mapByol(rows[0]), 'decommissioned')
  await db.$executeRawUnsafe('DELETE FROM keycloak_byol_infrastructure WHERE id = $1::uuid', id)
}
...
export async function getByolCore(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT id, name, customer_id FROM keycloak_byol_infrastructure WHERE id = $1::uuid',
    id,
  )
  const r = rows[0]
