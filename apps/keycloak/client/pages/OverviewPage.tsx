...
/**
 * What this app manages in a Keycloak realm, rendered with the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/keycloak/meta')
  ...
  if (loading) return <Spinner label="Loading Keycloak app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Keycloak (open-source Identity and Access Management) configuration as code. Author a
          configuration in the Configuration Canvas and deploy it through the pipeline — validate, deploy,
          health check, drift detection and rollback are handled per configuration type. Clients are applied
          over the Keycloak Admin REST API (HTTPS, self-signed tolerated) using an admin service-account token.
        </p>
