...
import { extractMtlsCertificateSpecs, mtlsCertificateKey } from './validate'

/**
 * Health check for Access mTLS certificate configuration:
 ...
 *   3. Every declared certificate (by name) still exists in the account
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('cloudflare_reachable', async () => {
    const res = await client.account('GET', '/access/certificates', { query: { per_page: 1 } })
  ...
  if (reachable.passed) {
    ...
    if (specs.length > 0) {
      ...
      const keys = new Set(live.filter((c) => c.name).map((c) => mtlsCertificateKey(c.name as string)))
      for (const spec of specs) {
        const present = keys.has(mtlsCertificateKey(spec.name))
        checks.push({
          name: `certificate:${spec.name}`,
          passed: present,
          message: present
            ? `mTLS certificate "${spec.name}" is present`
            : `mTLS certificate "${spec.name}" is missing`,
        })
      }
    }
  }
