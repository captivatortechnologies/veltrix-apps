...
import { extractGatewayPolicySpecs } from './validate'

/**
 * Health check for Gateway policy configuration:
 *   1. An account id is available (account-scoped objects need one)
 *   2. Cloudflare API reachability + zone resolution (the token works, zone found)
 *   3. Every declared policy still exists in the account (matched by name)
