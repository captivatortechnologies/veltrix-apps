...
import { extractRateLimitRuleSpecs } from './validate'

/**
 * Health check for rate limiting rule configuration:
 *   1. Cloudflare API reachability + zone resolution
 *   2. Every declared rule (by ref) is present in the phase entrypoint
    ...
    if (specs.length > 0) {
      ...
      for (const spec of specs) {
        ...
        checks.push({
          name: `rule:${spec.name}`,
          passed: present,
          message: present ? `Rule "${spec.name}" is present` : `Rule "${spec.name}" (ref ${spec.ref}) is missing`,
        })
      }
    }
  }
