...
**App:** `runzero` · **Category:** COMPLIANCE · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **runZero API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 10 config types · 4 API families.</sub>
...
sequenceDiagram
  ...
  participant api as runZero API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + siteswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + sitesread
  rbac-->>pipeline: permitted ✓ / denied
