...
/**
 * Health for the sites config = runZero answers on its console API with the
 * configured Organization API key. Read-only: GET /org/sites. A 2xx confirms the
 * key authenticates and the org is reachable; a 401/403 flags the key; anything
 * ≥ 500 is a runZero-side failure.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await runzeroRequest(`${base}/org/sites`, { headers, timeoutMs })
    const authOk = res.status !== 401 && res.status !== 403
    const passed = res.ok
    checks.push({
      name: 'runzero_reachable',
      passed,
      message: passed
