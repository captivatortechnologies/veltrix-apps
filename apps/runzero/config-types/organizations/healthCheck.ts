...
/**
 * Health for the organizations config = runZero answers on the ACCOUNT orgs endpoint with the
 * configured API key. Read-only: GET /account/orgs. A 2xx confirms the key authenticates AND is
 * account-scoped; a 401/403 flags the key — most often an Organization key used where an
 * account-scoped key is required (see the config type's _shared header).
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await runzeroRequest(`${base}/account/orgs`, { headers, timeoutMs })
    const authOk = res.status !== 401 && res.status !== 403
    const passed = res.ok
    checks.push({
      name: 'runzero_orgs_reachable',
      passed,
      message: passed
        ? `runZero account organizations reachable and authenticated (HTTP ${res.status}).`
        : authOk
