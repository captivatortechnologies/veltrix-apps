...
/**
 * Health for the scan-templates config = runZero answers on the ACCOUNT templates endpoint with
 * the configured API key. Read-only: GET /account/tasks/templates. A 2xx confirms the key
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await runzeroRequest(`${base}/account/tasks/templates`, { headers, timeoutMs })
    const authOk = res.status !== 401 && res.status !== 403
    const passed = res.ok
    checks.push({
      name: 'runzero_templates_reachable',
      passed,
      message: passed
        ? `runZero account templates reachable and authenticated (HTTP ${res.status}).`
        : authOk
