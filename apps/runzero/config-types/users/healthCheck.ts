...
/**
 * Health for the users config = runZero answers on the ACCOUNT users endpoint with the configured
 * API key. Read-only: GET /account/users. A 2xx confirms the key authenticates AND is
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await runzeroRequest(`${base}/account/users`, { headers, timeoutMs })
    const authOk = res.status !== 401 && res.status !== 403
    const passed = res.ok
    checks.push({
      name: 'runzero_users_reachable',
      passed,
      message: passed
        ? `runZero account users reachable and authenticated (HTTP ${res.status}).`
        : authOk
