...
/**
 * Health for the groups config = runZero answers on the ACCOUNT groups endpoint with the
 * configured API key. Read-only: GET /account/groups. A 2xx confirms the key authenticates AND is
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    const res = await runzeroRequest(`${base}/account/groups`, { headers, timeoutMs })
    const authOk = res.status !== 401 && res.status !== 403
    const passed = res.ok
    checks.push({
      name: 'runzero_groups_reachable',
      passed,
      message: passed
        ? `runZero account groups reachable and authenticated (HTTP ${res.status}).`
        : authOk
