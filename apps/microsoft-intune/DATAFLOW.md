# 🖥️ Microsoft Intune — Request Flows
...
**App:** `microsoft-intune` · **Category:** EDR · **Version:** 1.5.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Microsoft Intune API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Microsoft Intune API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + intune-asr-ruleswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + intune-asr-rulesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: intune-asr-ruleswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
