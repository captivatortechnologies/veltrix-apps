...
import { listDriverUpdateProfiles } from './deploy'
import { extractProfileSpecs, profileKey } from './validate'

/**
 * Health check for driver update profiles:
 *   1. Graph reachability + token/permission validity (a windowsDriverUpdateProfiles list)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  let live: Awaited<ReturnType<typeof listDriverUpdateProfiles>> | null = null
  try {
    live = await listDriverUpdateProfiles(client)
  ...
  if (live) {
    ...
    for (const spec of extractProfileSpecs(ctx.canvas).filter((s) => s.name)) {
      ...
      checks.push({
        ...
        passed: present,
        message: present ? `Driver update profile "${spec.name}" is present` : `Driver update profile "${spec.name}" is missing`,
      })
    }
  }
