...
import { listAppConfigs } from './deploy'
import { extractAppConfigSpecs, policyKey } from './validate'

/**
 * Health check for app configuration policies:
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  let live: Awaited<ReturnType<typeof listAppConfigs>> | null = null
  try {
    live = await listAppConfigs(client)
  ...
  if (live) {
    ...
    for (const spec of extractAppConfigSpecs(ctx.canvas).filter((s) => s.name)) {
      const present = names.has(policyKey(spec.name))
      checks.push({
        name: `policy:${spec.name}`,
        passed: present,
        message: present ? `App configuration policy "${spec.name}" is present` : `App configuration policy "${spec.name}" is missing`,
      })
    }
  }
