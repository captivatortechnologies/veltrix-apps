...
import { listProtections } from './deploy'
import { extractProtectionSpecs, policyKey } from './validate'

/**
 * Health check for Android app protection policies:
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  let live: Awaited<ReturnType<typeof listProtections>> | null = null
  try {
    live = await listProtections(client)
  ...
  if (live) {
    ...
    for (const spec of extractProtectionSpecs(ctx.canvas).filter((s) => s.name)) {
