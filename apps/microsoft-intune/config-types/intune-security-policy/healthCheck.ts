...
import { listConfigurationPolicies } from '../../lib/policy'
import { extractPolicySpecs, policyKey } from './validate'

/**
 * Health check for imported policies:
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  let live: Awaited<ReturnType<typeof listConfigurationPolicies>> | null = null
  try {
    live = await listConfigurationPolicies(client)
