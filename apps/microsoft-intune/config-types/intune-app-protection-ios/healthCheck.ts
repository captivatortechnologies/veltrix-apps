...
import { listIosMamPolicies } from './deploy'
import { extractIosMamSpecs, policyKey } from './validate'

/**
 * Health check for iOS app protection policies:
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  let live: Awaited<ReturnType<typeof listIosMamPolicies>> | null = null
  try {
    live = await listIosMamPolicies(client)
  ...
  if (live) {
    ...
    for (const spec of extractIosMamSpecs(ctx.canvas).filter((s) => s.name)) {
      const present = names.has(policyKey(spec.name))
      checks.push({
        name: `policy:${spec.name}`,
        passed: present,
        message: present ? `App protection policy "${spec.name}" is present` : `App protection policy "${spec.name}" is missing`,
      })
    }
  }
