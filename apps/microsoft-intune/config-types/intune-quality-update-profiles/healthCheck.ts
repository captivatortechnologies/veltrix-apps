...
import { listQualityUpdateProfiles } from './deploy'
import { extractProfileSpecs, profileKey } from './validate'

/**
 * Health check for quality update profiles:
 *   1. Graph reachability + token/permission validity (a windowsQualityUpdateProfiles list)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  let live: Awaited<ReturnType<typeof listQualityUpdateProfiles>> | null = null
  try {
    live = await listQualityUpdateProfiles(client)
  ...
  if (live) {
    ...
    for (const spec of extractProfileSpecs(ctx.canvas).filter((s) => s.name)) {
      ...
      checks.push({
        ...
        passed: present,
        message: present ? `Quality update profile "${spec.name}" is present` : `Quality update profile "${spec.name}" is missing`,
      })
    }
  }
