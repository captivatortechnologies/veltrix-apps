...
/**
 * Report the current deployment status of OPNsense unbound-domain-overrides
 * configurations. Reads deployment and component records through the
 * platform data API only — makes no calls to the OPNsense API itself.
 */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['opnsense-firewall'] })
