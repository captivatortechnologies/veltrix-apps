...
import { searchOneToOneRules } from '../../lib/oneToOneNatApi'
import { buildOpnsenseClient } from '../../lib/opnsenseApi'
import { extractOneToOneRuleSpecs } from './_shared'

/**
 * Health check for OPNsense one-to-one-nat configuration: API reachability +
 ...
 * deployment's rollbackData — no name to check by). Read-only.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const specs = extractOneToOneRuleSpecs(ctx.canvas)
