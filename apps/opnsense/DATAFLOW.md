...
**App:** `opnsense` · **Category:** NETWORK · **Version:** 0.1.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **OPNsense API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as OPNsense API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + firewall-aliaseswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + firewall-aliasesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: firewall-aliaseswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
