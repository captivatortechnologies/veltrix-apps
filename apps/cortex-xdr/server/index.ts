// ========================================================================
// Cortex XDR App — Server Entry Module
//
// Registers Cortex XDR-specific API routes as a Fastify plugin, prefixed with
// /api/apps/cortex-xdr/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration Canvas,
// and every write goes through the pipeline handlers (Cortex XDR public REST API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('iocs', 'read')],
