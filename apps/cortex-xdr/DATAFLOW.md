...
**App:** `cortex-xdr` · **Category:** EDR · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Cortex XDR API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
<sub>Applies to: 12 config types · 6 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Cortex XDR API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + iocswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + iocsread
  rbac-->>pipeline: permitted ✓ / denied
