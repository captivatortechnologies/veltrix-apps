...
/**
 * Health for BIOCs config = Cortex XDR answers on its public API with the
 * configured API Key. Read-only: POST /endpoints/get_endpoint_groups/ with an
 * empty request. A 2xx (or any response below 500) counts as reachable; 401/403
 * mean the key is bad. VERIFY the probe endpoint against a live Cortex XDR tenant.
 */
