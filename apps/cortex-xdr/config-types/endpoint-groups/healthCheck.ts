...
/**
 * Health for endpoint groups = Cortex XDR answers on its public API with the
 * configured API Key. Read-only reachability probe (client.health() —
 * POST /endpoints/get_endpoint_groups/, which is exactly this config type's list
 * endpoint). A response below 500 counts as reachable; 401/403 mean the key is
 * bad. VERIFY the probe endpoint against a live Cortex XDR tenant.
 */
