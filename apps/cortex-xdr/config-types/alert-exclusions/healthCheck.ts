...
/**
 * Health for alert exclusions = Cortex XDR answers on its public API with the
 * configured API Key. Read-only reachability probe (client.health() —
 * POST /endpoints/get_endpoint_groups/, a documented endpoint used because the
 * exclusion endpoints themselves are not public). A response below 500 counts as
 * reachable; 401/403 mean the key is bad. VERIFY the probe endpoint against a
