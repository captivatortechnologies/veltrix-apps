// ========================================================================
// Zscaler App — Server Entry Module
//
// Registers Zscaler-specific API routes as a Fastify plugin, prefixed with
// /api/apps/zscaler/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration
// Canvas, and every write goes through the pipeline handlers (Zscaler OneAPI).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('zia-url-filtering-rules', 'read')],
