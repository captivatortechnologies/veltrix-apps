...
**App:** `zscaler` · **Category:** NETWORK · **Version:** 1.3.0  
...
<sub>Applies to: 32 config types · 7 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Zscaler API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + zia-url-filtering-ruleswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + zia-url-filtering-rulesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: zia-url-filtering-ruleswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
