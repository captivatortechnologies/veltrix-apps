...
import { extractWebDlpRuleSpecs } from './validate'

/**
 * Health check for web DLP rule configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared web DLP rule still exists in the tenant
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `Web DLP rule "${spec.name}" is present`
              : `Web DLP rule "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-web-dlp-rules this canvas declares: ${
