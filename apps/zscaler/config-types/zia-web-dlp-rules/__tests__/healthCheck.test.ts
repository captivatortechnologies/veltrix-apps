// healthCheck for zia-web-dlp-rules.
//
...
// second half of the check: every declared web DLP rule must still exist in the
...
test('zia-web-dlp-rules healthCheck: passes when every declared rule is present', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
    ziaList([{ id: 7331, name: 'Block PII uploads' }]),
  ])
  try {
    ...
    const check = result.checks.find((c) => c.name === 'rule:Block PII uploads')
...
test('zia-web-dlp-rules healthCheck: fails when a declared rule has been deleted in the tenant', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
...
test('zia-web-dlp-rules healthCheck: does not look for rules when the tenant is unreachable', async () => {
  ...
  try {
    ...
    assert.equal(calls.filter((c) => c.url.includes('/webDlpRules')).length, 0)
  } finally {
    restore()
  }
})
