// rollback for zia-web-dlp-rules.
//
...
// WHOLE prior rule PUT back verbatim — the DLP engine references included — so a
// rule that was only confirming uploads before the deploy goes back to that; a
// rule deploy created is deleted; a 404 is not an error; and the revert is
...
const UPDATED_ENTRY = { name: 'Block PII uploads', existed: true, id: 7331, prior: PRIOR }
const CREATED_ENTRY = { name: 'Block source code', existed: false, id: 7401 }

test('zia-web-dlp-rules rollback: restores the prior rule deploy overwrote, verbatim', async () => {
  const { calls, restore } = recordFetch([TOKEN, ok({ id: 7331 }), ACTIVATED])
  try {
    ...
    assert.match(tenant[0].url, /\/zia\/api\/v1\/webDlpRules\/7331$/)
...
test('zia-web-dlp-rules rollback: deletes a rule deploy created', async () => {
  ...
  try {
    ...
    assert.match(tenant[0].url, /\/zia\/api\/v1\/webDlpRules\/7401$/)
    assert.equal(result.success, true)
  } finally {
    restore()
  }
...
test('zia-web-dlp-rules rollback: undoes the newest change first', async () => {
...
test('zia-web-dlp-rules rollback: a rule already gone is not an error', async () => {
...
test('zia-web-dlp-rules rollback: a rejected restore fails rather than throwing', async () => {
  const { calls, restore } = recordFetch([TOKEN, ziaError(400, 'Referenced DLP engine no longer exists')])
  try {
    const result = await rollback(rollbackContext({ previousState: [UPDATED_ENTRY] }))

    assert.equal(result.success, false)
    assert.match(String(result.message), /Referenced DLP engine no longer exists/)
...
test('zia-web-dlp-rules rollback: a failed activation is reported as still-staged', async () => {
