// healthCheck for zia-firewall-ips-rules.
//
...
// second half: every declared IPS rule must still exist in the tenant, matched
// by name and reported as an `ips-rule:<name>` check.
...
test('zia-firewall-ips-rules healthCheck: passes when every declared rule is present', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
    ziaList([{ id: 812, name: 'Inspect Inbound Web', order: 15 }]),
  ])
  try {
    ...
    const check = result.checks.find((c) => c.name === 'ips-rule:Inspect Inbound Web')
...
test('zia-firewall-ips-rules healthCheck: fails when a declared rule has been deleted in the tenant', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
...
test('zia-firewall-ips-rules healthCheck: does not look for rules when the tenant is unreachable', async () => {
  ...
  try {
    ...
    assert.equal(result.healthy, false)
    assert.equal(
      result.checks.some((c) => c.name.startsWith('ips-rule:')),
      false,
      'an unreadable tenant must not be reported as the rule being absent',
    )
    assert.equal(calls.filter((c) => c.url.includes('/firewallIpsRules')).length, 0)
  } finally {
    restore()
  }
})
