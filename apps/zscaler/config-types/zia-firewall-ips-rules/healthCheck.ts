...
import { extractIpsRuleSpecs } from './validate'

/**
 * Health check for firewall IPS rule configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared firewall IPS rule still exists in the tenant
    ...
    if (specs.length > 0) {
      ...
      try {
        ...
        for (const spec of specs) {
          checks.push({
            name: `ips-rule:${spec.name}`,
            passed: names.has(spec.name),
            message: names.has(spec.name)
              ? `Firewall IPS rule "${spec.name}" is present`
              : `Firewall IPS rule "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-firewall-ips-rules this canvas declares: ${
