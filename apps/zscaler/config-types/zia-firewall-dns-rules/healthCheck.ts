...
/**
 * Health check for DNS rule configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared DNS rule still exists in the tenant
    ...
    if (specs.length > 0) {
      ...
      try {
        ...
        for (const spec of specs) {
          checks.push({
            name: `dns-rule:${spec.name}`,
            passed: names.has(spec.name),
            message: names.has(spec.name)
              ? `DNS rule "${spec.name}" is present`
              : `DNS rule "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-firewall-dns-rules this canvas declares: ${
