// healthCheck for zia-firewall-dns-rules.
//
...
// second half: every declared DNS rule must still exist in the tenant, matched
// by name and reported as a `dns-rule:<name>` check.
...
test('zia-firewall-dns-rules healthCheck: passes when every declared rule is present', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
    ziaList([{ id: 331, name: 'Block Rogue Resolvers', order: 12 }]),
  ])
  try {
    ...
    const check = result.checks.find((c) => c.name === 'dns-rule:Block Rogue Resolvers')
...
test('zia-firewall-dns-rules healthCheck: fails when a declared rule has been deleted in the tenant', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
...
test('zia-firewall-dns-rules healthCheck: does not look for rules when the tenant is unreachable', async () => {
  ...
  try {
    ...
    assert.equal(result.healthy, false)
    assert.equal(
      result.checks.some((c) => c.name.startsWith('dns-rule:')),
      false,
      'an unreadable tenant must not be reported as the rule being absent',
    )
    assert.equal(calls.filter((c) => c.url.includes('/firewallDnsRules')).length, 0)
  } finally {
    restore()
  }
})
