...
/**
 * Detect drift between the deployed service edge group configuration and the
 * live tenant. Re-finds each declared group by name and diffs the managed
 * fields (description, enabled, location); a missing group is critical drift.
 */
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  const specs = extractServiceEdgeGroupSpecs(ctx.deployedConfig).filter((s) => s.name)
