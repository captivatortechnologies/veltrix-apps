...
import { listServiceEdgeGroups } from './deploy'
import { extractServiceEdgeGroupSpecs } from './validate'

/**
 * Health check for service edge group configuration:
 *   1. ZPA API reachability + credential/customerId validity
 *   2. Every declared service edge group still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('zpa_reachable', async () => {
    const res = await client.zpa('GET', '/serviceEdgeGroup', { query: { page: 1, pagesize: 1 } })
  ...
  if (reachable.passed) {
    ...
    if (specs.length > 0) {
      ...
      try {
        const live = await listServiceEdgeGroups(client)
        const names = new Set(live.map((g) => g.name))
        for (const spec of specs) {
          checks.push({
            name: `serviceEdgeGroup:${spec.name}`,
            passed: names.has(spec.name),
            message: names.has(spec.name)
              ? `Service edge group "${spec.name}" is present`
              : `Service edge group "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zpa-service-edge-groups this canvas declares: ${
