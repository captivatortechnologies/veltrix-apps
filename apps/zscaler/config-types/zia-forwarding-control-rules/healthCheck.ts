...
/**
 * Health check for forwarding control rule configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared forwarding rule still exists in the tenant
    ...
    if (specs.length > 0) {
      ...
      try {
        ...
        for (const spec of specs) {
          checks.push({
            name: `forwarding-rule:${spec.name}`,
            passed: names.has(spec.name),
            message: names.has(spec.name)
              ? `Forwarding rule "${spec.name}" is present`
              : `Forwarding rule "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-forwarding-control-rules this canvas declares: ${
