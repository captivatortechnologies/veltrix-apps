// healthCheck for zia-forwarding-control-rules.
//
...
// second half: every declared forwarding rule must still exist in the tenant,
// matched by name and reported as a `forwarding-rule:<name>` check.
...
test('zia-forwarding-control-rules healthCheck: passes when every declared rule is present', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
    ziaList([{ id: 907, name: 'Route Branch To ZPA', order: 8 }]),
  ])
  try {
    ...
    const check = result.checks.find((c) => c.name === 'forwarding-rule:Route Branch To ZPA')
...
test('zia-forwarding-control-rules healthCheck: fails when a declared rule has been deleted in the tenant', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
...
test('zia-forwarding-control-rules healthCheck: does not look for rules when the tenant is unreachable', async () => {
  ...
  try {
    ...
    assert.equal(result.healthy, false)
    assert.equal(
      result.checks.some((c) => c.name.startsWith('forwarding-rule:')),
      false,
      'an unreadable tenant must not be reported as the rule being absent',
    )
    assert.equal(calls.filter((c) => c.url.includes('/forwardingRules')).length, 0)
  } finally {
    restore()
  }
})
