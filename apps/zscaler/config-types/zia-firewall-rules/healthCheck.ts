...
/**
 * Health check for firewall rule configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared firewall rule still exists in the tenant
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `Firewall rule "${spec.name}" is present`
              : `Firewall rule "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-firewall-rules this canvas declares: ${
