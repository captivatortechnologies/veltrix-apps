...
/**
 * Health check for segment group configuration:
 *   1. ZPA API reachability + credential/customerId validity
 *   2. Every declared segment group still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('zpa_reachable', async () => {
    const res = await client.zpa('GET', '/segmentGroup', { query: { page: 1, pagesize: 1 } })
    ...
    if (specs.length > 0) {
      ...
      try {
        ...
        for (const spec of specs) {
          checks.push({
            name: `segmentGroup:${spec.name}`,
            passed: names.has(spec.name),
            message: names.has(spec.name)
              ? `Segment group "${spec.name}" is present`
              : `Segment group "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zpa-segment-groups this canvas declares: ${
