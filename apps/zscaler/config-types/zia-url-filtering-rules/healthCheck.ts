...
import { extractUrlFilteringRuleSpecs } from './validate'

/**
 * Health check for URL filtering rule configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared URL filtering rule still exists in the tenant
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `URL filtering rule "${spec.name}" is present`
              : `URL filtering rule "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-url-filtering-rules this canvas declares: ${
