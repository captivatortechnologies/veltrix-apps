...
/**
 * Health check for network service group configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared network service group still exists in the tenant
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `Network service group "${spec.name}" is present`
              : `Network service group "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-network-service-groups this canvas declares: ${
