 ...
 *   2. Every declared server group still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('zpa_reachable', async () => {
    const res = await client.zpa('GET', '/serverGroup', { query: { page: 1, pagesize: 1 } })
    ...
    if (specs.length > 0) {
      ...
      try {
        ...
        for (const spec of specs) {
          checks.push({
            name: `serverGroup:${spec.name}`,
            passed: names.has(spec.name),
            message: names.has(spec.name)
              ? `Server group "${spec.name}" is present`
              : `Server group "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zpa-server-groups this canvas declares: ${
