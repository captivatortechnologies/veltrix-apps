...
import { extractServerGroupSpecs } from './validate'

/**
 * Detect drift between the deployed server group configuration and the live
 * tenant. Re-finds each declared group by name and diffs the managed scalar
 * fields (description, enabled, dynamicDiscovery); a missing group is critical
 * drift. Member connector groups / servers are referenced by name and resolved +
 * normalized server-side, so they are not diffed here.
 */
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  try {
    const live = await listServerGroups(client)
    ...
    for (const spec of specs) {
      ...
      const liveDynamicDiscovery = found.dynamicDiscovery ?? true
      if (spec.dynamicDiscovery !== liveDynamicDiscovery) {
        diffs.push({
          field: `${spec.name}.dynamicDiscovery`,
          expected: String(spec.dynamicDiscovery),
          actual: String(liveDynamicDiscovery),
          severity: 'warning',
        })
      }
