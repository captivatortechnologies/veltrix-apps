...
/**
 * Health check for DLP engine configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared DLP engine still exists in the tenant
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `DLP engine "${spec.name}" is present`
              : `DLP engine "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-dlp-engines this canvas declares: ${
