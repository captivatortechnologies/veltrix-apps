...
// carries the prior engine EXPRESSION deploy captured, not a rebuilt one — a
// reverted engine that keeps the new expression catches the wrong things; an
// engine deploy created is DELETEd; an engine already gone (404) is not an error;
...
const CREATED_ENTRY = { name: 'New Engine', existed: false, id: 8810 }

test('zia-dlp-engines rollback: restores the prior expression of an engine deploy overwrote', async () => {
  const { calls, restore } = recordFetch([TOKEN, ok({ id: 8801 }), ACTIVATED])
  try {
    ...
    assert.match(tenant[0].url, /\/zia\/api\/v1\/dlpEngines\/8801$/)
    const body = bodyOf(tenant[0])
    assert.equal(body?.engineExpression, '((D101.S > 5))', 'the restored rule is the prior one, not a default')
...
test('zia-dlp-engines rollback: deletes an engine deploy created', async () => {
  ...
  try {
    ...
    assert.match(tenant[0].url, /\/zia\/api\/v1\/dlpEngines\/8810$/)
    assert.equal(result.success, true)
  } finally {
    restore()
  }
...
test('zia-dlp-engines rollback: undoes the newest change first', async () => {
...
test('zia-dlp-engines rollback: an engine already gone is not an error', async () => {
  // 404 is a known answer — the engine we would delete is already absent, which
...
test('zia-dlp-engines rollback: a rejected restore fails rather than throwing', async () => {
  const { calls, restore } = recordFetch([TOKEN, ziaError(400, 'Engine is referenced by a DLP rule')])
  try {
    const result = await rollback(rollbackContext({ previousState: [UPDATED_ENTRY] }))

    assert.equal(result.success, false)
    assert.match(String(result.message), /Engine is referenced by a DLP rule/)
...
test('zia-dlp-engines rollback: a failed activation is reported as still-staged', async () => {
