...
/**
 * Health check for DLP notification template configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared template still exists in the tenant
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `DLP notification template "${spec.name}" is present`
              : `DLP notification template "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-dlp-notification-templates this canvas declares: ${
