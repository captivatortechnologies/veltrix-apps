...
/**
 * Health check for custom URL category configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared URL category still exists in the tenant (match by
 *      configuredName)
  ...
  if (reachable.passed) {
    ...
    if (specs.length > 0) {
      ...
      try {
        ...
        const names = new Set(live.map((c) => c.configuredName))
        for (const spec of specs) {
          checks.push({
            name: `category:${spec.configuredName}`,
            passed: names.has(spec.configuredName),
            message: names.has(spec.configuredName)
              ? `URL category "${spec.configuredName}" is present`
              : `URL category "${spec.configuredName}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-url-categories this canvas declares: ${
