...
/**
 * Health check for sandbox rule configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared sandbox rule still exists in the tenant
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `Sandbox rule "${spec.name}" is present`
              : `Sandbox rule "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-sandbox-rules this canvas declares: ${
