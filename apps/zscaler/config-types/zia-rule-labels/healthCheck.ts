...
import { listLabels } from './deploy'
import { extractLabelSpecs } from './validate'

/**
 * Health check for rule label configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared rule label still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed) {
    ...
    if (specs.length > 0) {
      ...
      try {
        const live = await listLabels(client)
        const names = new Set(live.map((l) => l.name))
        for (const spec of specs) {
          checks.push({
            name: `label:${spec.name}`,
            passed: names.has(spec.name),
            message: names.has(spec.name)
              ? `Rule label "${spec.name}" is present`
              : `Rule label "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-rule-labels this canvas declares: ${
