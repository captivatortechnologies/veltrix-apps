...
/**
 * Health check for IP destination group configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared destination group still exists in the tenant
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `Destination group "${spec.name}" is present`
              : `Destination group "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-ip-destination-groups this canvas declares: ${
