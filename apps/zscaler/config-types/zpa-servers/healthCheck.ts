...
/**
 * Health check for server configuration:
 *   1. ZPA API reachability + credential/customerId validity
 *   2. Every declared server still exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('zpa_reachable', async () => {
    const res = await client.zpa('GET', '/server', { query: { page: 1, pagesize: 1 } })
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `Server "${spec.name}" is present`
              : `Server "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zpa-servers this canvas declares: ${
