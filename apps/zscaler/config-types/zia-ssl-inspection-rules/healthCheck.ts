...
/**
 * Health check for SSL inspection rule configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared SSL inspection rule still exists in the tenant
    ...
    if (specs.length > 0) {
      ...
      try {
        ...
        for (const spec of specs) {
          checks.push({
            name: `ssl-rule:${spec.name}`,
            passed: names.has(spec.name),
            message: names.has(spec.name)
              ? `SSL inspection rule "${spec.name}" is present`
              : `SSL inspection rule "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-ssl-inspection-rules this canvas declares: ${
