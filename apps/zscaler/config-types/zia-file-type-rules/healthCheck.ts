...
/**
 * Health check for file type control rule configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared file type rule still exists in the tenant
        ...
        for (const spec of specs) {
          checks.push({
            ...
            message: names.has(spec.name)
              ? `File type rule "${spec.name}" is present`
              : `File type rule "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-file-type-rules this canvas declares: ${
