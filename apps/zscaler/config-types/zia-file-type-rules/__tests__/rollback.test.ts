// rollback for zia-file-type-rules.
//
// The shared contract covers the refusals (no credential, nothing recorded, an
// entry with no id or no prior body). What is specific here: the file types a
// rule covers live in the captured body, so the restore has to PUT that body
// back verbatim — restoring only the scalars would leave the rule covering
// whatever the deploy pointed it at. A rule deploy created is deleted, a rule
// already gone (404) is not an error, and the revert only takes effect on
// activation.
...
const CREATED_ENTRY = { name: 'New File Type Rule', existed: false, id: 5001 }

test('zia-file-type-rules rollback: restores the recorded prior body, not a default', async () => {
  const { calls, restore } = recordFetch([TOKEN, ok({ id: 505 }), ACTIVATED])
  try {
    ...
    assert.match(tenant[0].url, /\/zia\/api\/v1\/fileTypeRules\/505$/)
    const body = bodyOf(tenant[0])
    assert.equal(body?.action, 'ALLOW')
    assert.equal(body?.state, 'ENABLED')
    assert.equal(body?.order, 3)
    assert.deepEqual(body?.fileTypes, ['FTCATEGORY_ENCRYPT'])
    assert.deepEqual(body?.labels, [{ id: 99, name: 'Hand-made legacy label' }])
...
test('zia-file-type-rules rollback: deletes a rule deploy created', async () => {
  ...
  try {
    ...
    assert.match(tenant[0].url, /\/zia\/api\/v1\/fileTypeRules\/5001$/)
    assert.equal(result.success, true)
  } finally {
    restore()
  }
...
test('zia-file-type-rules rollback: undoes the newest change first, and activates once', async () => {
...
test('zia-file-type-rules rollback: a rule already gone is not an error', async () => {
...
test('zia-file-type-rules rollback: a rejected restore fails rather than throwing', async () => {
  const { calls, restore } = recordFetch([TOKEN, ziaError(400, 'Referenced label no longer exists')])
  try {
    const result = await rollback(rollbackContext({ previousState: [UPDATED_ENTRY] }))

    assert.equal(result.success, false)
    assert.match(String(result.message), /Referenced label no longer exists/)
...
test('zia-file-type-rules rollback: a failed activation is reported as still-staged', async () => {
