...
/**
 * Health check for network application group configuration:
 *   1. ZIA API reachability + credential validity (GET /status)
 *   2. Every declared network application group still exists in the tenant
    ...
    if (specs.length > 0) {
      ...
      try {
        ...
        for (const spec of specs) {
          checks.push({
            name: `network_app_group:${spec.name}`,
            passed: names.has(spec.name),
            message: names.has(spec.name)
              ? `Network application group "${spec.name}" is present`
              : `Network application group "${spec.name}" does not exist in the tenant`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'presence',
          passed: false,
          message: `Could not list the zia-network-app-groups this canvas declares: ${
