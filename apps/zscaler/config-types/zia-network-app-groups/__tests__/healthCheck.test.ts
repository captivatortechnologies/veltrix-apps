// healthCheck for zia-network-app-groups.
//
...
// matched by name, reported as `network_app_group:<name>`.
...
test('zia-network-app-groups healthCheck: passes when every declared group is present', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
    ziaList([{ id: 6611, name: 'Collab apps', networkApplications: ['APNS', 'WEBEX'] }]),
  ])
  try {
    ...
    const check = result.checks.find((c) => c.name === 'network_app_group:Collab apps')
...
test('zia-network-app-groups healthCheck: fails when a declared group has been deleted in the tenant', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    activationStatus('ACTIVE'),
...
test('zia-network-app-groups healthCheck: does not look for groups when the tenant is unreachable', async () => {
  ...
  try {
    ...
    assert.equal(result.healthy, false)
    assert.equal(
      result.checks.some((c) => c.name.startsWith('network_app_group:')),
      false,
      'an unreadable tenant must not be reported as the group being absent',
    )
    assert.equal(calls.filter((c) => c.url.includes('/networkApplicationGroups')).length, 0)
  } finally {
    restore()
  }
})
