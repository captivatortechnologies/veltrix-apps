...
/**
 * Health check for Secrets settings:
 *   1. Snyk API reachability + token/org validity (a settings GET)
 *   2. The live secrets_enabled matches the declared value
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const matches = (live?.secrets_enabled ?? false) === spec.secretsEnabled
    checks.push({
      name: 'secrets_enabled',
      passed: matches,
      message: matches
        ? `Snyk Secrets scanning is ${spec.secretsEnabled ? 'enabled' : 'disabled'} as configured`
        : `Snyk Secrets scanning is ${live?.secrets_enabled ? 'enabled' : 'disabled'} but configuration expects ${spec.secretsEnabled ? 'enabled' : 'disabled'}`,
    })
  } catch (error) {
