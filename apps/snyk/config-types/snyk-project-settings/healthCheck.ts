...
/**
 * Health check for project settings:
 *   1. Snyk API reachability + token/org validity (the first project's settings)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  for (let i = 0; i < specs.length; i++) {
    ...
    try {
      await readProjectSettings(client, spec.projectId)
      if (!reachable) {
