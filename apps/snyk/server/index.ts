// ========================================================================
// Snyk App — Server Entry Module
//
// Registers Snyk-specific API routes as a Fastify plugin, prefixed with
// /api/apps/snyk/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration
// Canvas, and every write goes through the pipeline handlers (Snyk REST + v1).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('snyk-sast-settings', 'read')],
