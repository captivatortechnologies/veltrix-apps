...
/** Deployment status for an ip-address-restrictions configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['delinea-secret-server'] })
