# 🔑 Delinea Secret Server — Request Flows
...
**App:** `delinea-secret-server` · **Category:** IAM · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Delinea Secret Server API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Delinea Secret Server API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + folderswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + foldersread
  rbac-->>pipeline: permitted ✓ / denied
