// ========================================================================
// Delinea Secret Server App — Server Entry Module
//
// Registers the app's API routes as a Fastify plugin, prefixed with
// /api/apps/delinea-secret-server/ and protected by app-level auth + permission
// middleware. Read-only on the platform side: folder authoring happens in the
// Configuration Canvas, and every write goes through the pipeline handlers
// (Secret Server REST API). This foundation exposes read-only /meta + /settings
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('folders', 'read')],
