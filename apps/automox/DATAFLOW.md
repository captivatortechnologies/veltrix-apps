...
**App:** `automox` · **Category:** Endpoint Management · **Version:** 0.2.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Automox API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Automox API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + policieswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + policiesread
  rbac-->>pipeline: permitted ✓ / denied
