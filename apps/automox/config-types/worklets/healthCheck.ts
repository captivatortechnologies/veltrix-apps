...
import { extractWorkletSpecs } from './_shared'

/**
 * Health check for Worklet configuration:
 ...
 *      Organization ID).
 *   2. Every declared item still exists in the org (matched by name + type).
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable) {
    for (const spec of extractWorkletSpecs(ctx.canvas).filter((s) => s.name)) {
      const live = findPolicyByName(livePolicies, spec.name, spec.workletType)
      checks.push({
        name: `worklet:${spec.name}`,
        passed: Boolean(live),
        message: live ? `Worklet "${spec.name}" is present.` : `Worklet "${spec.name}" was not found in the org.`,
      })
    }
  }
