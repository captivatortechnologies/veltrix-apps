...
/**
 * Health check for analytics rules:
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (live) {
    ...
    for (const spec of extractRuleSpecs(ctx.canvas).filter((s) => s.ruleName)) {
      const present = names.has(spec.ruleId.toLowerCase())
      checks.push({
        name: `rule:${spec.ruleName}`,
        passed: present,
        message: present ? `Analytics rule "${spec.ruleName}" is present` : `Analytics rule "${spec.ruleName}" is missing`,
      })
    }
  }
