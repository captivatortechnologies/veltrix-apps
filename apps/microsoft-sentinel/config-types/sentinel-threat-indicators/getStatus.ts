...
/**
 * Report the current deployment status of the threat intelligence indicators
 * across the workspace components this config type targets. Reads deployment +
 * component records through the platform data API.
 */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['sentinel-workspace'] })
