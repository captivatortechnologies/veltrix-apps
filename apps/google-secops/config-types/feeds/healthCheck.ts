...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', `${client.parent()}/feeds?pageSize=1`)
  ...
  checks.push({
    name: 'secops-feeds',
    passed,
    message: passed ? 'Reached the Google SecOps feeds endpoint' : `Google SecOps error: ${secopsErrorMessage(resp)}`,
    latencyMs,
  })
