...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Extensions are nested under a log type — reachability is checked against the
  ...
  checks.push({
    name: 'secops-parser-extensions',
    passed,
    message: passed ? 'Reached the Google SecOps log types endpoint (parser extensions are nested under it)' : `Google SecOps error: ${secopsErrorMessage(resp)}`,
    latencyMs,
  })
