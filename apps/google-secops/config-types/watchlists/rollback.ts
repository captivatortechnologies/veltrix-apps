...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (!e.existed) {
      // We created this watchlist — remove it.
      const del = await client.request('DELETE', `${parent}/watchlists/${enc(e.watchlistId)}?force=true`)
      if (!del.ok && del.status !== 404) failures.push(`delete ${e.displayName}: ${secopsErrorMessage(del)}`)
      else deleted++
    } else if (e.prior) {
      // We updated this watchlist — restore its prior definition.
      const resp = await client.request('PATCH', `${parent}/watchlists/${enc(e.watchlistId)}?updateMask=${UPDATE_MASK}`, {
        displayName: e.prior.displayName,
        description: e.prior.description,
        multiplyingFactor: e.prior.multiplyingFactor,
        entityPopulationMechanism: { manual: {} },
        watchlistUserPreferences: { pinned: e.prior.pinned },
      })
