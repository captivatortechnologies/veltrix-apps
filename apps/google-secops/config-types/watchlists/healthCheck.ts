...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', `${client.parent()}/watchlists?pageSize=1`)
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'secops-watchlists',
    passed,
    message: passed ? 'Reached the Google SecOps watchlists endpoint' : `Google SecOps error: ${secopsErrorMessage(resp)}`,
    latencyMs,
  })
