...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', `${client.parent()}/rules?pageSize=1`)
  ...
  checks.push({
    name: 'secops-rules',
    passed,
    message: passed ? 'Reached the Google SecOps rules endpoint' : `Google SecOps error: ${secopsErrorMessage(resp)}`,
    latencyMs,
  })
