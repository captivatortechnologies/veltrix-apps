...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', `${client.parent()}/nativeDashboards?pageSize=1`)
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'secops-native-dashboards',
    passed,
    message: passed ? 'Reached the Google SecOps native-dashboards endpoint' : `Google SecOps error: ${secopsErrorMessage(resp)}`,
    latencyMs,
  })
