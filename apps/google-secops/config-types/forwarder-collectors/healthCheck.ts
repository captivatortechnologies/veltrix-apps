...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Collectors are nested under a forwarder — reachability is checked against the
  // parent forwarders endpoint (the collector type's dependency).
  const resp = await client.request('GET', `${client.parent()}/forwarders?pageSize=1`)
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'secops-forwarder-collectors',
    passed,
    message: passed ? 'Reached the Google SecOps forwarders endpoint (collectors are nested under it)' : `Google SecOps error: ${secopsErrorMessage(resp)}`,
    latencyMs,
  })
