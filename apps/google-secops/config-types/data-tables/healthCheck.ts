...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const resp = await client.request('GET', `${client.parent()}/dataTables?pageSize=1`)
  const latencyMs = Date.now() - start
  const passed = resp.ok

  checks.push({
    name: 'secops-data-tables',
    passed,
    message: passed ? 'Reached the Google SecOps data-tables endpoint' : `Google SecOps error: ${secopsErrorMessage(resp)}`,
    latencyMs,
  })
