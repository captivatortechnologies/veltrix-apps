...
**App:** `google-secops` · **Category:** SIEM · **Version:** 0.5.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Google SecOps API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 16 config types · 1 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Google SecOps API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + reference-listswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + reference-listsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: reference-listswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
