...
**App:** `greenbone` · **Category:** COMPLIANCE · **Version:** 0.4.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Greenbone API** · credentials via the Credential Vault (`ctx.resolveConnection`) · reachable over `ctx.remote`.
...
<sub>Applies to: 15 config types · 7 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Greenbone API
  ...
  Note over rbac: configuration-canvasread · configuration-canvaswrite · componentread · credentialread + targetswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + targetsread
  rbac-->>pipeline: permitted ✓ / denied
