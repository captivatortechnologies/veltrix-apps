...
/**
 * Deployment status for a Greenbone configuration, derived from platform records
 * (the latest SUCCEEDED deployment for the canvas + the Greenbone components).
 * Shared by every Greenbone config type — the shape is identical; only the canvas
 * differs. Read-only; touches no gvmd socket.
 */
export async function configDeploymentStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['greenbone', 'standalone'] })
