...
// (`greenbone_byol_resource`), deployment runs (`greenbone_byol_deployment`) and
// their ordered steps (`greenbone_byol_deployment_step`).
//
...
export async function listResources(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM greenbone_byol_resource WHERE infrastructure_id = $1::uuid ORDER BY sort_order ASC',
    infrastructureId,
  )
...
export async function seedResources(
  ...
  await db.$executeRawUnsafe(
    'DELETE FROM greenbone_byol_resource WHERE infrastructure_id = $1::uuid AND NOT (plan_key = ANY($2::text[]))',
    infrastructureId,
    keys,
  )
  for (const item of plan) {
    await db.$executeRawUnsafe(
      `INSERT INTO greenbone_byol_resource
...
export async function setResourceStatus(
  ...
  const affected = await db.$executeRawUnsafe(
    `UPDATE greenbone_byol_resource
       SET status = $3,
...
export async function setAllResourceStatuses(
  ...
  await db.$executeRawUnsafe(
    'UPDATE greenbone_byol_resource SET status = $2, updated_at = now() WHERE infrastructure_id = $1::uuid',
    infrastructureId,
    status,
  )
}
...
export async function createDeployment(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    `INSERT INTO greenbone_byol_deployment (infrastructure_id, action, status, initiated_by_user_id)
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    await db.$executeRawUnsafe(
      `INSERT INTO greenbone_byol_deployment_step
...
async function attachSteps(db: PlatformDatabaseClient, deployment: ByolDeploymentDto): Promise<ByolDeploymentDto> {
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM greenbone_byol_deployment_step WHERE deployment_id = $1::uuid ORDER BY step_order ASC',
    deployment.id,
  )
...
export async function listDeployments(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM greenbone_byol_deployment WHERE infrastructure_id = $1::uuid ORDER BY started_at DESC',
    infrastructureId,
  )
...
export async function getLatestDeployment(
  ...
  const rows = await db.$queryRawUnsafe<Row[]>(
    'SELECT * FROM greenbone_byol_deployment WHERE infrastructure_id = $1::uuid ORDER BY started_at DESC LIMIT 1',
    infrastructureId,
  )
...
export async function advanceStep(
  ...
  const affected = await db.$executeRawUnsafe(
    `UPDATE greenbone_byol_deployment_step
...
export async function setDeploymentStatus(
  ...
  await db.$executeRawUnsafe(
    `UPDATE greenbone_byol_deployment SET status = $2, message = COALESCE($3, message), updated_at = now()${setCompleted}
     WHERE id = $1::uuid`,
    deploymentId,
    status,
    message ?? null,
...
export async function reconcileTerminal(
  ...
  if (outcome === 'succeeded') {
    ...
    await db.$executeRawUnsafe(
      "UPDATE greenbone_byol_resource SET status = 'failed', updated_at = now() WHERE infrastructure_id = $1::uuid AND status <> 'ready'",
      infrastructureId,
    )
    if (latest) {
