...
import { withGmpSession, resolveGmpHost, resolveGmpPort, buildGetVersionCommand, parseGmpStatus, parseRootAttributes } from './greenboneApi'

/**
 * Shared gvmd reachability health check for every Greenbone config type: gvmd
 * accepts a GMP connection, authenticates the configured username/password and
 * answers <get_version/>. withGmpSession() authenticates before the callback, so
 * reaching it already proves reachability + auth. Read-only. Applied over GMP
 * (XML over TLS, 9390).
 */
export async function checkGvmdReachable(ctx: HealthCheckContext): Promise<HealthCheckResult> {
