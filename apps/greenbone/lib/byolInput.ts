...
//   • tiers['manager'] → gvmd + GSA web manager nodes (ALB targets)
//   • tiers['scanner'] → openvas-scanner nodes
//
...
// `nodeTiers` in declared order [manager, scanner].
// =============================================================================

import { normalizeControlPlaneLayout, parsePlacement, validatePlacement, type ClusterPlacement } from './byolPlacement'

/** Greenbone's two scalable BYOL node tiers, in persisted/display order. */
const TIER_KEYS = ['manager', 'scanner'] as const
...
/**
 * Minimum node count per tier. Both the manager (gvmd + GSA) and the scanner
 * tiers stay permissive (≥1): a single manager is a complete Greenbone control
 * plane and one scanner is enough to run scans. A single-node / eval deployment
 * collapses to one all-in-one box, so every tier is effectively ≥1 there too.
 */
...
export function readByol(body: any): { data: Record<string, unknown>; error?: string } {
  ...
  // "Distributed" is the multi-node Greenbone stack (manager + scanners +
  // PostgreSQL + Redis on separate nodes); single instance is the all-in-one box.
  ...
  // Carried on the record for SDK-form round-tripping; unused by Greenbone's
  // resource plan (Greenbone has no consolidated control plane / forwarder tier).
