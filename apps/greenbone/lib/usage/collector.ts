...
//     it returns null for an infra. Greenbone has no ingest-volume metric (it is
//     a scanner, not an ingest pipeline), so the poller is unused today —
//     node-hours are the live dimension (the poller stays wired so a future
//     Greenbone volume metric could plug straight in).
//
 ...
 * omit it to record node-hours only (the current default — Greenbone has no
...
export async function collectForDate(
  ...
  const infra = await db.$queryRawUnsafe<Array<{ id: string; customer_id: string; name: string }>>(
    'SELECT id, customer_id, name FROM greenbone_byol_infrastructure',
  )
