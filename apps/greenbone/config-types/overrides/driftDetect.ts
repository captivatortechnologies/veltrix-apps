...
import { buildGetOverridesCommand, parseOverrides } from '../../lib/gmp/overrides'
import { extractSpecs, loadPriorEntries } from './_shared'

/**
 * Detect drift between the last-deployed override set and live gvmd state,
 * tracked by canvas-item id. Compares text/nvtOid/newSeverity/hosts/port —
 * NOT the "active" days-count (see lib/gmp/overrides.ts's FLAG: the read-side
 * representation is not independently verified to correspond to the
 * write-side day count, so it is always re-applied rather than diffed).
 * Read-only.
 */
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  try {
    live = await withGmpSession(
      ...
      async (session) => parseOverrides(await session.send(buildGetOverridesCommand())),
    )
  } catch {
  ...
  for (const spec of specs) {
    const label = `override (${spec.itemId})`
    ...
    const found = liveById.get(tracked.overrideId)
    if (!found) {
    ...
    if (Number(found.newSeverity) !== spec.newSeverity) diffs.push({ field: `${label}.newSeverity`, expected: String(spec.newSeverity), actual: found.newSeverity, severity: 'critical' })
