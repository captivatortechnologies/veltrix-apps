...
import { buildDeleteGroupCommand, buildModifyGroupCommand } from '../../lib/gmp/groups'

/**
 * Undo a groups deploy from rollbackData.previous (written by deploy()): for
 * each entry, either DELETE the group we created (<delete_group
 * ultimate="1"/>) or RESTORE the prior name/comment/users on one we modified
 * (<modify_group>). Best-effort — a failed step is counted and reported.
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    await withGmpSession(
      ...
      async (session) => {
        for (const entry of previous) {
          try {
            if (entry.created) {
              const st = parseGmpStatus(await session.send(buildDeleteGroupCommand(entry.groupId, true)))
              st.ok ? deleted++ : failed++
            } else if (entry.restore) {
              const st = parseGmpStatus(
                await session.send(buildModifyGroupCommand(entry.groupId, { name: entry.restore.name, comment: entry.restore.comment, users: entry.restore.users })),
              )
    ...
    return {
      success: failed === 0,
      message: `Rolled back groups: ${deleted} deleted, ${restored} restored${failed ? `, ${failed} failed` : ''}.`,
    }
  } catch (error) {
