...
import { buildDeleteRoleCommand, buildModifyRoleCommand } from '../../lib/gmp/roles'

/**
 * Undo a roles deploy from rollbackData.previous (written by deploy()): for
 * each entry, either DELETE the role we created (<delete_role
 * ultimate="1"/>) or RESTORE the prior name/comment/users on one we modified
 * (<modify_role>). Best-effort — a failed step is counted and reported.
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  try {
    await withGmpSession(
      ...
      async (session) => {
        for (const entry of previous) {
          try {
            if (entry.created) {
              const st = parseGmpStatus(await session.send(buildDeleteRoleCommand(entry.roleId, true)))
              st.ok ? deleted++ : failed++
            } else if (entry.restore) {
              const st = parseGmpStatus(
                await session.send(buildModifyRoleCommand(entry.roleId, { name: entry.restore.name, comment: entry.restore.comment, users: entry.restore.users })),
              )
    ...
    return {
      success: failed === 0,
      message: `Rolled back roles: ${deleted} deleted, ${restored} restored${failed ? `, ${failed} failed` : ''}.`,
    }
  } catch (error) {
