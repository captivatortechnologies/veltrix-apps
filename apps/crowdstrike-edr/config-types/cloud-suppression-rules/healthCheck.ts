...
import { SUPPRESSION_ENDPOINTS, findSuppressionRule } from './deploy'
import { extractSuppressionSpecs } from './validate'

/**
 * Health check for suppression rule configuration:
 *   1. Falcon API reachability + credential validity (Cloud Security scope)
 *   2. Every declared suppression rule exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', SUPPRESSION_ENDPOINTS.queries, { query: { limit: 1 } })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "Cloud security suppression rules: Read" scope (403)')
    }
  ...
  // Check 2..n: each declared suppression rule exists
  if (reachable.passed) {
    const specs = extractSuppressionSpecs(ctx.canvas).filter((s) => s.name)
    for (const spec of specs) {
      checks.push(
        await timedCheck(`suppression-rule:${spec.name}`, async () => {
          const live = await findSuppressionRule(client, spec.name)
          if (!live) {
            throw new Error(`Suppression rule "${spec.name}" does not exist in the tenant`)
          }
          return `Suppression rule "${spec.name}" is present`
        }),
      )
    }
  }
