...
import { extractContentUpdatePolicySpecs } from './validate'

/**
 * Health check for content update policy configuration:
 *   1. Falcon API reachability + credential validity (Content update policies scope)
 ...
 *      its hosts fall back to another content update policy.
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 1: API reachable and the client has the Content update policies scope
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', '/policy/queries/content-update/v1', {
      query: { limit: 1 },
    })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "Content update policies: Read" scope (403)')
    }
