...
import { CONTENT_UPDATE_ENDPOINTS } from './validate'
import type { ContentUpdatePolicyRollbackEntry } from './deploy'

/**
 * Roll back content update policies using the state captured during deploy:
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  const previousState = (ctx.rollbackData as { previousState?: ContentUpdatePolicyRollbackEntry[] })
    ?.previousState
  ...
  try {
    for (const entry of previousState) {
      if (!entry.existed) {
        ...
        if (entry.id) {
          try {
            await policyAction(client, CONTENT_UPDATE_ENDPOINTS, entry.id, 'disable')
          } catch {
            // Best effort — the policy may already be disabled or missing.
          }
          const res = await client.request('DELETE', CONTENT_UPDATE_ENDPOINTS.entity, {
            query: { ids: entry.id },
          })
        ...
        for (const groupId of entry.prior.groupsAdded ?? []) {
          await policyAction(client, CONTENT_UPDATE_ENDPOINTS, entry.id, 'remove-host-group', groupId)
        }
        for (const groupId of entry.prior.groupsRemoved ?? []) {
          await policyAction(client, CONTENT_UPDATE_ENDPOINTS, entry.id, 'add-host-group', groupId)
        }
      }
