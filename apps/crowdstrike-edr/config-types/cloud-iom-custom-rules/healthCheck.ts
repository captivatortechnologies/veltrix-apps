...
import { CLOUD_IOM_RULE_ENDPOINTS } from './deploy'
import { extractCloudIomRuleSpecs } from './validate'

/**
 * Health check for IOM custom-rule configuration:
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 1: API reachable and the client has the Cloud Security rules scope
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', CLOUD_IOM_RULE_ENDPOINTS.queries, { query: { limit: 1 } })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "Cloud Security: Read" scope (403)')
    }
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`iom-rule:${spec.name}`, async () => {
          const live = await findEntityByIdentity(client, CLOUD_IOM_RULE_ENDPOINTS, spec.name)
          if (!live) {
            throw new Error(`IOM custom rule "${spec.name}" does not exist in the tenant`)
          }
          return `IOM custom rule "${spec.name}" is present`
        }),
      )
    }
  }
