...
import { timedCheck } from '../ml-exclusions/exclusionShared'
import { SV_EXCLUSION_ENDPOINTS } from './deploy'
import { extractSvExclusionSpecs } from './validate'

/**
 * Health check for sensor visibility exclusion configuration:
 *   1. Falcon API reachability + credential validity (SV Exclusions scope)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 1: API reachable and the client has the Sensor Visibility Exclusions scope
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', SV_EXCLUSION_ENDPOINTS.queries, { query: { limit: 1 } })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "Sensor Visibility Exclusions: Read" scope (403)')
    }
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`sv-exclusion:${spec.value}`, async () => {
          const live = await findExclusion(client, SV_EXCLUSION_ENDPOINTS, spec.value)
          if (!live) {
            throw new Error(`Sensor visibility exclusion "${spec.value}" does not exist in the tenant`)
          }
          return `Sensor visibility exclusion "${spec.value}" is present`
        }),
      )
    }
  }
