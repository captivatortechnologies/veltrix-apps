import validate, { extractSvExclusionSpecs } from '../validate'
...
describe('CrowdStrike Sensor Visibility Exclusions Validate Handler', () => {
  ...
  it('warns (but stays valid) when applied globally', async () => {
    const result = await validate(
      makeCtx([{ name: 'sec1', fields: validFields({ appliedGlobally: true, hostGroups: '' }) }]),
    )
    expect(result.valid).toBe(true)
    expect(result.warnings.some((w) => w.code === 'global_telemetry_suppression')).toBe(true)
  })
...
describe('extractSvExclusionSpecs', () => {
  it('parses host groups and defaults appliedGlobally to false', () => {
    const sections = [{ name: 'sec1', fields: { value: '/tmp/x', hostGroups: 'g1, g2' } }]
    const specs = extractSvExclusionSpecs({
      id: 's',
      canvasId: 'c',
      version: 1,
      name: 'n',
