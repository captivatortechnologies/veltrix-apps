// healthCheck for response-policies.
//
...
// through the Response Policy combined query AND carry the declared enablement —
// a policy that exists but is switched off is not granting the RTR capability
// tier it was deployed for.
...
registerHealthCheckContract({
  ...
  handler: healthCheck,
  probePath: '/policy/queries/response/v1',
  scopePattern: /Response policies: Read/,
})
...
const LIVE = { id: 'pol-live-1', name: 'Corp Windows RTR', platform_name: 'Windows', enabled: true }

test('response-policies healthCheck: passes when every declared policy is present and enabled', async () => {
  ...
  try {
    ...
    const check = result.checks.find((c) => c.name === 'policy:Corp Windows RTR')
...
test('response-policies healthCheck: fails when a declared policy has been deleted in the tenant', async () => {
...
test('response-policies healthCheck: fails a policy that exists but is switched off', async () => {
...
test('response-policies healthCheck: does not look for policies when the tenant is unreachable', async () => {
  ...
  try {
    ...
    assert.equal(
      calls.filter((c) => c.url.includes('/policy/combined/response/v1')).length,
      0,
...
test('response-policies healthCheck: reports a failed per-policy lookup as failed, not as absent', async () => {
