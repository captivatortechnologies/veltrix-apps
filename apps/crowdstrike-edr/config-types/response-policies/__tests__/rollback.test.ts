// rollback for response-policies.
//
...
// what this deploy created, patch the prior capability toggles back and reverse
...
registerRollbackGuardContract({ label: 'response-policies', handler: rollback, entry: CREATED_ENTRY })

test('response-policies rollback: disables then deletes a policy this deploy created', async () => {
...
test('response-policies rollback: treats a 404 on delete as the policy already being gone', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
...
test('response-policies rollback: still deletes when the pre-delete disable is refused', async () => {
  const { calls, restore } = routeFetch([
...
test('response-policies rollback: restores the recorded prior values of a policy it overwrote', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
...
test('response-policies rollback: restores the prior enablement through an actions call', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
  ])
...
test('response-policies rollback: reverses exactly the host-group deltas the deploy applied', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
  ])
...
test('response-policies rollback: writes nothing for an updated entry whose prior state was never captured', async () => {
  ...
  try {
    await rollback(
      rollbackContext({
        previousState: [{ name: 'Corp Windows RTR', platform: 'Windows', existed: true, id: 'pol-live-1' }],
      }),
    )
...
test('response-policies rollback: writes nothing for an updated entry whose id was never captured', async () => {
  const { calls, restore } = routeFetch([], EMPTY)
  try {
    await rollback(
      rollbackContext({
        previousState: [
          { name: 'Corp Windows RTR', platform: 'Windows', existed: true, prior: UPDATED_ENTRY.prior },
        ],
      }),
    )
...
test('response-policies rollback: writes nothing for a created entry whose id was never captured', async () => {
  ...
  try {
    await rollback(
      rollbackContext({ previousState: [{ name: 'Corp Windows RTR', platform: 'Windows', existed: false }] }),
    )
...
test('response-policies rollback: reports a rejected delete rather than throwing', async () => {
...
test('response-policies rollback: treats HTTP 200 with a populated errors[] as a failed restore', async () => {
...
test('response-policies rollback: reports how far it got when a later entry fails', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: [ok(), forbidden('access denied, authorization failed')] },
  ])
  try {
    const second = { ...UPDATED_ENTRY, name: 'Corp Linux RTR', id: 'pol-live-2' }
