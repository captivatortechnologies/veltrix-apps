...
// `/policy/entities/response/v1`, but enablement and host-group assignment are
// NOT fields on it — each is a separate `response-actions` call.
//
...
const COMBINED = /\/policy\/combined\/response\/v1/
const ACTIONS = /\/policy\/entities\/response-actions\/v1/
const ENTITY = /\/policy\/entities\/response\/v1/
...
function actionCalls(calls: RecordedCall[], name: string): RecordedCall[] {
  return calls.filter((c) => c.url.includes('response-actions') && c.url.includes(`action_name=${name}`))
}

/**
 * One declared response policy. `extractPolicySpecs` reads a FLAT `fields`
 * record — `name`, `platform`, `description`, `enabled`, `hostGroups`,
 * `settings` (a JSON string of capability toggles, not ML sliders).
 */
 ...
 * toggle inverted, plus one toggle the canvas never declared.
 */
const LIVE_POLICY = {
  id: 'pol-live-1',
  name: 'Corp Windows RTR',
...
registerDeployGuardContract({ label: 'response-policies', handler: deploy, items: [POLICY] })

test('response-policies deploy: creates a policy that does not exist yet', async () => {
  ...
  try {
    ...
    const creates = callsOfMethod(calls, 'POST').filter((c) => !c.url.includes('response-actions'))
    assert.equal(creates.length, 1, `expected exactly one create, got ${describeCalls(creates)}`)
    const body = resource(creates[0])
    assert.equal(body?.name, 'Corp Windows RTR')
    assert.equal(body?.platform_name, 'Windows')
    assert.equal(body?.description, 'Tier 2 responder capabilities')
...
test('response-policies deploy: creates the policy DISABLED and enables it through an actions call', async () => {
  // The API ignores `enabled` on create — every new policy starts off. Without
  // the separate actions call the declared RTR tier is never granted.
  const { calls, restore } = routeFetch([
    { url: COMBINED, respond: EMPTY },
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'POST', respond: created({ id: 'pol-new-1' }) },
...
test('response-policies deploy: attaches the declared host groups through actions calls', async () => {
  const { calls, restore } = routeFetch([
    { url: COMBINED, respond: EMPTY },
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'POST', respond: created({ id: 'pol-new-1' }) },
...
test('response-policies deploy: records the created policy so rollback can delete it', async () => {
  ...
  try {
    ...
    assert.equal(state[0].name, 'Corp Windows RTR')
...
test('response-policies deploy: updates a policy that already exists, carrying its id', async () => {
  const { calls, restore } = routeFetch([
  ...
  try {
    ...
    assert.equal(result.success, true)
    assert.equal(
      callsOfMethod(calls, 'POST').filter((c) => !c.url.includes('response-actions')).length,
      0,
...
test('response-policies deploy: records the LIVE prior values of a policy it overwrote', async () => {
  // The canvas asks for enabled/RealTimeFunctionality on; the tenant holds the
  // inverse. Rollback restores what was there, not what was wanted.
  const { restore } = routeFetch([
    { url: COMBINED, respond: entityPage([LIVE_POLICY]) },
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
  ...
  try {
    ...
    assert.deepEqual(
      ...
      'only the toggles this deploy overwrote are recorded, at their live values',
    )
  } finally {
    restore()
  }
...
test('response-policies deploy: converges host groups and records the deltas it applied', async () => {
  const { calls, restore } = routeFetch([
    { url: COMBINED, respond: entityPage([LIVE_POLICY]) },
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
...
test('response-policies deploy: converges enablement on an existing policy it found disabled', async () => {
  const { calls, restore } = routeFetch([
    { url: COMBINED, respond: entityPage([LIVE_POLICY]) },
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
...
test('response-policies deploy: reports failure rather than throwing when the vendor rejects', async () => {
  const { restore } = routeFetch([
    { url: COMBINED, respond: EMPTY },
...
test('response-policies deploy: treats HTTP 200 with a populated errors[] as a failure', async () => {
...
test('response-policies deploy: treats a 200-with-errors on the enable action as a failure', async () => {
  const { restore } = routeFetch([
    { url: COMBINED, respond: EMPTY },
...
test('response-policies deploy: refuses a policy whose settings are not toggles, before any write', async () => {
  // ML slider keys belong to prevention policies. A mis-pasted setting must be
  // caught locally rather than sent to the tenant.
  const MIXED = item('Windows responders', {
    name: 'Corp Windows RTR',
    platform: 'Windows',
    enabled: true,
    settings: JSON.stringify([{ id: 'CloudAntiMalware', value: { detection: 'AGGRESSIVE' } }]),
  })
  const { calls, restore } = routeFetch([], EMPTY)
  try {
    const result = await deploy(deployContext([MIXED]))

    assert.equal(result.success, false)
    assert.match(String(result.message), /capability toggles/)
    assert.equal(
...
test('response-policies deploy: keeps the rollback record of what it wrote when a later policy fails', async () => {
  const SECOND = item('Linux responders', {
    name: 'Corp Linux RTR',
    platform: 'Linux',
    enabled: false,
  })
...
test('response-policies deploy: a create that returns no id is not reported as a success', async () => {
...
test('response-policies deploy: never puts the token or the client secret in its result', async () => {
  const { restore } = routeFetch([
    { url: COMBINED, respond: entityPage([LIVE_POLICY]) },
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
...
test('response-policies deploy: an empty canvas deploys nothing and touches nothing', async () => {
