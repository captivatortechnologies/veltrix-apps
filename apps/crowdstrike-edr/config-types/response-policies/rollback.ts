...
import { RESPONSE_POLICY_ENDPOINTS, type PolicyRollbackEntry } from './deploy'

/**
 * Roll back response policies using the state captured during deploy:
  ...
  try {
    for (const entry of previousState) {
      ...
      if (!entry.existed) {
        ...
        if (entry.id) {
          try {
            await policyAction(client, RESPONSE_POLICY_ENDPOINTS, entry.id, 'disable')
          } catch {
            // Best effort — the policy may already be disabled or missing.
          }
          const res = await client.request('DELETE', RESPONSE_POLICY_ENDPOINTS.entity, {
            query: { ids: entry.id },
          })
        ...
        for (const groupId of entry.prior.groupsAdded ?? []) {
          await policyAction(client, RESPONSE_POLICY_ENDPOINTS, entry.id, 'remove-host-group', groupId)
        }
        for (const groupId of entry.prior.groupsRemoved ?? []) {
          await policyAction(client, RESPONSE_POLICY_ENDPOINTS, entry.id, 'add-host-group', groupId)
        }
      }
    ...
    return {
      ...
      message: `Rolled back ${reverted.length} response policy(ies): ${reverted.join(', ')}${skipNote}`,
    }
  } catch (error) {
    return {
      success: false,
