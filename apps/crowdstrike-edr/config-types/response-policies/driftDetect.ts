...
import { RESPONSE_POLICY_ENDPOINTS } from './deploy'
import { extractPolicySpecs, flattenLiveSettings, parseResponseSettings, type PolicySetting } from './validate'

/**
 * Detect drift between the deployed response policy configuration and the
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  for (const spec of specs) {
    ...
    try {
      const live = await findPolicyByName(client, RESPONSE_POLICY_ENDPOINTS, spec.name, spec.platform)
      ...
      // Enablement decides whether the policy's RTR tier is in effect
      ...
      const { settings: declared } = parseResponseSettings(spec.settingsRaw)
      ...
      for (const setting of declared) {
        ...
        if (!settingMatches(setting, liveValue)) {
          // A capability toggle that should be on but is off means the tier is not granted
