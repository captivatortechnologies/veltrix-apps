...
/**
 * Health check for response policy configuration:
 *   1. Falcon API reachability + credential validity (Response policies scope)
 ...
 *      the intended RTR capability tier is not in effect.
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 1: API reachable and the client has the Response policies scope
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', '/policy/queries/response/v1', {
      query: { limit: 1 },
    })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "Response policies: Read" scope (403)')
    }
