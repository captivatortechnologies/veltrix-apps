// healthCheck for firewall-policies.
//
...
// through the Firewall Policy combined query AND carry the declared enablement —
// a policy that exists but is switched off leaves its hosts on whatever firewall
// policy ranks next.
//
// Note this check reads the policy SHELL only. The fwmgr container — rule
// groups, default in/out actions, enforce — is not probed here; drift is what
// covers it.
...
registerHealthCheckContract({
  ...
  handler: healthCheck,
  probePath: '/policy/queries/firewall/v1',
...
const LIVE = { id: 'pol-live-1', name: 'Corp Windows Firewall', platform_name: 'Windows', enabled: true }

test('firewall-policies healthCheck: passes when every declared policy is present and enabled', async () => {
  ...
  try {
    ...
    const check = result.checks.find((c) => c.name === 'policy:Corp Windows Firewall')
...
test('firewall-policies healthCheck: fails when a declared policy has been deleted in the tenant', async () => {
...
test('firewall-policies healthCheck: fails a policy that exists but is switched off', async () => {
...
test('firewall-policies healthCheck: does not look for policies when the tenant is unreachable', async () => {
  ...
  try {
    ...
    assert.equal(
      calls.filter((c) => c.url.includes('/policy/combined/firewall/v1')).length,
      0,
...
test('firewall-policies healthCheck: reports a failed per-policy lookup as failed, not as absent', async () => {
