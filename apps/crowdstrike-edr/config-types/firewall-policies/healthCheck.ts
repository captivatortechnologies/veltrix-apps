...
import { findFirewallPolicy } from './deploy'
import { extractFirewallPolicySpecs } from './validate'

/**
 * Health check for firewall policy configuration:
 ...
 *      state — a policy that should be enabled but is not leaves its hosts on
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', '/policy/queries/firewall/v1', { query: { limit: 1 } })
