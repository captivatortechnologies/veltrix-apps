...
import { extractIoaExclusionSpecs } from './validate'

/**
 * Health check for IOA exclusion configuration:
 *   1. Falcon API reachability + credential validity (IOA Exclusions scope)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 1: API reachable and the client has the IOA Exclusions scope
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', IOA_EXCLUSION_ENDPOINTS.queries, { query: { limit: 1 } })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "IOA Exclusions: Read" scope (403)')
    }
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`ioa-exclusion:${spec.name}`, async () => {
          const live = await findExclusion(client, IOA_EXCLUSION_ENDPOINTS, spec.name)
          if (!live) {
            throw new Error(`IOA exclusion "${spec.name}" does not exist in the tenant`)
          }
          return `IOA exclusion "${spec.name}" is present`
        }),
      )
    }
  }
