// healthCheck for firewall-rule-groups.
//
// The shared contract covers the refusals, the token-first probe and the error
// handling every crowdstrike-edr config type has in common. What is specific
// here is that presence alone is not health: a rule group that exists but is
// disabled enforces nothing, so the declared enablement is part of the check.

import test from 'node:test'
import assert from 'node:assert/strict'
import healthCheck from '../healthCheck'
import {
  EMPTY,
  TOKEN,
  entityPage,
  healthContext,
  idsPage,
  item,
  recordFetch,
  routeFetch,
  serverError,
  writeCalls,
} from '../../../lib/__tests__/fakeFalcon'
import { registerHealthCheckContract } from '../../../lib/__tests__/falconContracts'

registerHealthCheckContract({
  label: 'firewall-rule-groups',
  handler: healthCheck,
  probePath: '/fwmgr/queries/rule-groups/v1',
  scopePattern: /Firewall management: Read/,
})

const GROUP = item('Windows egress controls', {
  name: 'veltrix-fw-windows',
  platform: 'windows',
  enabled: true,
  rules: JSON.stringify([
    { name: 'Block outbound SMB', action: 'DENY', direction: 'OUT', protocol: 'TCP', remotePorts: [445] },
  ]),
})

const LIVE_GROUP = {
  id: 'frg-live-1',
  name: 'veltrix-fw-windows',
  platform: 'windows',
  enabled: true,
  tracking: 'tracking-token-abc',
  rules: [],
}

test('firewall-rule-groups healthCheck: passes when the group is present and enabled', async () => {
  const { calls, restore } = recordFetch([
    TOKEN,
    EMPTY,
    idsPage(['frg-live-1']),
    entityPage([LIVE_GROUP]),
  ])
  try {
    const result = await healthCheck(healthContext([GROUP]))

    assert.equal(result.healthy, true)
    assert.equal(result.score, 100)
    const check = result.checks.find((c) => c.name === 'rule-group:veltrix-fw-windows')
    assert.ok(check, `expected a per-group check, got ${result.checks.map((c) => c.name).join(', ')}`)
    assert.equal(check.passed, true)
    assert.equal(writeCalls(calls).length, 0, 'a health check must never write')
  } finally {
    restore()
  }
})

test('firewall-rule-groups healthCheck: fails when the group has been deleted in the tenant', async () => {
  const { restore } = recordFetch([TOKEN, EMPTY, EMPTY])
  try {
    const result = await healthCheck(healthContext([GROUP]))

    assert.equal(result.healthy, false)
    assert.equal(result.score, 50, 'one of two checks passed')
    const check = result.checks.find((c) => c.name === 'rule-group:veltrix-fw-windows')
    assert.ok(check)
    assert.equal(check.passed, false)
    assert.match(String(check.message), /does not exist in the tenant/)
  } finally {
    restore()
  }
})

test('firewall-rule-groups healthCheck: fails when the group was disabled in the console', async () => {
  // A disabled rule group still exists and still looks deployed. It enforces
  // nothing, so every rule in it stops applying.
  const { restore } = recordFetch([
    TOKEN,
    EMPTY,
    idsPage(['frg-live-1']),
    entityPage([{ ...LIVE_GROUP, enabled: false }]),
  ])
  try {
    const result = await healthCheck(healthContext([GROUP]))

    assert.equal(result.healthy, false)
    const check = result.checks.find((c) => c.name === 'rule-group:veltrix-fw-windows')
    assert.ok(check)
    assert.equal(check.passed, false)
    assert.match(String(check.message), /is disabled but should be enabled/)
  } finally {
    restore()
  }
})

test('firewall-rule-groups healthCheck: does not look for groups when the tenant is unreachable', async () => {
  // The presence check is only meaningful once reachability passed — otherwise
  // every group would read as "does not exist" because nothing could be read.
  const { calls, restore } = routeFetch([{ url: /oauth2\/token/, respond: TOKEN }], serverError())
  try {
    const result = await healthCheck(healthContext([GROUP]))

    assert.equal(result.healthy, false)
    assert.equal(
      result.checks.some((c) => c.name.startsWith('rule-group:')),
      false,
      'an unreadable tenant must not be reported as the group being absent',
    )
    assert.equal(
      calls.filter((c) => c.url.includes('filter=')).length,
      0,
      'no per-group lookup may follow a failed reachability probe',
    )
  } finally {
    restore()
  }
})

test('firewall-rule-groups healthCheck: reports a failed per-group lookup as failed, not as absent', async () => {
  // The reachability probe succeeds and the per-group query then 500s. That is
  // "I could not look", and it must not pass.
  const { restore } = recordFetch([TOKEN, EMPTY, serverError('internal server error')])
  try {
    const result = await healthCheck(healthContext([GROUP]))

    assert.equal(result.healthy, false)
    const check = result.checks.find((c) => c.name === 'rule-group:veltrix-fw-windows')
    assert.ok(check)
    assert.equal(check.passed, false)
    assert.match(String(check.message), /internal server error/)
  } finally {
    restore()
  }
})

test('firewall-rule-groups healthCheck: fails a lookup whose candidate hydration could not be read', async () => {
  // The id query answered, the per-id read then 500d. Letting that fall through
  // to "does not exist" would report a live rule group as deleted.
  const { restore } = recordFetch([TOKEN, EMPTY, idsPage(['frg-live-1']), serverError()])
  try {
    const result = await healthCheck(healthContext([GROUP]))

    const check = result.checks.find((c) => c.name === 'rule-group:veltrix-fw-windows')
    assert.ok(check)
    assert.equal(check.passed, false)
    assert.equal(
      /does not exist in the tenant/.test(String(check.message)),
      false,
      'an unreadable group must not be reported as absent',
    )
  } finally {
    restore()
  }
})
