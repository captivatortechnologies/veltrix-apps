...
/**
 * Health check for FileVantage policy configuration:
 ...
 *      state — a policy that should be enabled but is not means its files are
 *      not being monitored.
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 1: API reachable and the client has the FileVantage scope
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', FILEVANTAGE_POLICY_ENDPOINTS.queries, {
      query: { limit: 1 },
    })
