// rollback for sensor-update-policies.
//
...
// what this deploy created, patch the prior build/uninstall-protection back and
// reverse exactly the host-group deltas it applied — and above all the entries
// that must produce NO write because there is nothing safe to restore.
...
const UPDATED_ENTRY = {
  ...
  prior: {
    ...
    enabled: false,
    settings: { build: '7.10.16101|n-3|Tagged|14', uninstall_protection: 'DISABLED' },
...
test('sensor-update-policies rollback: disables then deletes a policy this deploy created', async () => {
...
test('sensor-update-policies rollback: treats a 404 on delete as the policy already being gone', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
...
test('sensor-update-policies rollback: still deletes when the pre-delete disable is refused', async () => {
  const { calls, restore } = routeFetch([
...
test('sensor-update-policies rollback: restores the recorded prior build and uninstall protection', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
...
test('sensor-update-policies rollback: restores the prior enablement through an actions call', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
  ])
...
test('sensor-update-policies rollback: reverses exactly the host-group deltas the deploy applied', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
  ])
...
test('sensor-update-policies rollback: writes nothing for an updated entry whose prior state was never captured', async () => {
  // Deploy overwrote a live policy but recorded no prior body. Restoring an
  // invented build here is strictly worse than leaving the policy alone.
  const { calls, restore } = routeFetch([], EMPTY)
  try {
    await rollback(
      rollbackContext({
        previousState: [
          { name: 'Corp Windows Sensor Updates', platform: 'Windows', existed: true, id: 'pol-live-1' },
        ],
      }),
    )
...
test('sensor-update-policies rollback: writes nothing for an updated entry whose id was never captured', async () => {
  const { calls, restore } = routeFetch([], EMPTY)
  try {
    await rollback(
      rollbackContext({
...
test('sensor-update-policies rollback: writes nothing for a created entry whose id was never captured', async () => {
  ...
  try {
    await rollback(
      rollbackContext({
        previousState: [{ name: 'Corp Windows Sensor Updates', platform: 'Windows', existed: false }],
      }),
    )
...
test('sensor-update-policies rollback: reports a rejected delete rather than throwing', async () => {
...
test('sensor-update-policies rollback: treats HTTP 200 with a populated errors[] as a failed restore', async () => {
...
test('sensor-update-policies rollback: reports how far it got when a later entry fails', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: [ok(), forbidden('access denied, authorization failed')] },
  ])
  try {
    const second = { ...UPDATED_ENTRY, name: 'Corp Linux Sensor Updates', id: 'pol-live-2' }
