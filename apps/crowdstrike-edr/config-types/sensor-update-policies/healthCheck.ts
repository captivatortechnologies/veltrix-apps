...
import { findPolicyByName } from '../../lib/policyAdapter'
import { SENSOR_UPDATE_ENDPOINTS } from './deploy'
import { extractPolicySpecs } from './validate'

/**
 * Health check for sensor update policy configuration:
 *   1. Falcon API reachability + credential validity (Sensor update policies scope)
 *   2. Every declared policy exists on the tenant with the declared enablement
 *      state — a policy that should be enabled but is not means its hosts are
 *      not held at the declared sensor build.
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 1: API reachable and the client has the Sensor update policies scope
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', '/policy/queries/sensor-update/v1', {
      query: { limit: 1 },
    })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "Sensor update policies: Read" scope (403)')
    }
