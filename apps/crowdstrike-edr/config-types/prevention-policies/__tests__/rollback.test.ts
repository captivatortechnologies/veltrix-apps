// rollback for prevention-policies.
//
...
// what this deploy created, patch back what it overwrote and reverse exactly the
...
test('prevention-policies rollback: disables then deletes a policy this deploy created', async () => {
...
test('prevention-policies rollback: treats a 404 on delete as the policy already being gone', async () => {
...
test('prevention-policies rollback: still deletes when the pre-delete disable is refused', async () => {
  // The policy may already be disabled. A refused disable is best-effort and
  // must not stop the delete it was only preparing for.
  const { calls, restore } = routeFetch([
...
test('prevention-policies rollback: restores the recorded prior values of a policy it overwrote', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
...
test('prevention-policies rollback: restores the prior enablement through an actions call', async () => {
  // The deploy enabled a policy the tenant had switched off. Patching the body
  // back without disabling it again leaves the deploy half-applied.
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
  ])
...
test('prevention-policies rollback: reverses exactly the host-group deltas the deploy applied', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
    { url: ENTITY, method: 'PATCH', respond: ok() },
  ])
...
test('prevention-policies rollback: writes nothing for an updated entry whose prior state was never captured', async () => {
  ...
  try {
    await rollback(
      rollbackContext({
        previousState: [{ name: 'Corp Windows Prevention', platform: 'Windows', existed: true, id: 'pol-live-1' }],
      }),
    )
...
test('prevention-policies rollback: writes nothing for an updated entry whose id was never captured', async () => {
  const { calls, restore } = routeFetch([], EMPTY)
  try {
    await rollback(
      rollbackContext({
        previousState: [
          { name: 'Corp Windows Prevention', platform: 'Windows', existed: true, prior: UPDATED_ENTRY.prior },
        ],
      }),
    )
...
test('prevention-policies rollback: writes nothing for a created entry whose id was never captured', async () => {
  ...
  // operator is told the rollback completed. What is asserted is only the half
  // that is certainly right: it invents no id and writes nothing. The success it
  // claims is NOT asserted.
  const { calls, restore } = routeFetch([], EMPTY)
  try {
    await rollback(
      rollbackContext({
        previousState: [{ name: 'Corp Windows Prevention', platform: 'Windows', existed: false }],
      }),
    )
...
test('prevention-policies rollback: reports a rejected delete rather than throwing', async () => {
...
test('prevention-policies rollback: treats HTTP 200 with a populated errors[] as a failed restore', async () => {
...
test('prevention-policies rollback: reports how far it got when a later entry fails', async () => {
  const { calls, restore } = routeFetch([
    { url: ACTIONS, respond: ok() },
    {
      url: ENTITY,
  ...
  try {
    const second = { ...UPDATED_ENTRY, name: 'Corp Linux Prevention', id: 'pol-live-2' }
