...
/**
 * Report the current deployment status of Identity Protection policy rule
 * configurations. Reads deployment and component records through the platform
 * data API.
 */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['falcon-tenant'] })
