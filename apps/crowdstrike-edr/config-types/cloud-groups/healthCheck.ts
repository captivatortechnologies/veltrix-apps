...
import { CLOUD_GROUP_ENDPOINTS } from './deploy'
import { extractCloudGroupSpecs } from './validate'

/**
 * Health check for cloud group configuration:
 *   1. Falcon API reachability + credential validity (Cloud Security scope)
 *   2. Every declared group exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 1: API reachable and the client has the Cloud Security scope
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', CLOUD_GROUP_ENDPOINTS.queries, { query: { limit: 1 } })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "Cloud security cloud groups: Read" scope (403)')
    }
