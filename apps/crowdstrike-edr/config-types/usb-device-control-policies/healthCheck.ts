...
import { DEVICE_CONTROL_ENDPOINTS, extractDeviceControlSpecs } from './validate'

/**
 * Health check for device control policy configuration:
 *   1. Falcon API reachability + credential validity (Device control policies scope)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  // Check 1: API reachable and the client has the Device control policies scope
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', '/policy/queries/device-control/v1', {
      query: { limit: 1 },
    })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "Device control policies: Read" scope (403)')
    }
