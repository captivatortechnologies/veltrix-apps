// healthCheck for usb-device-control-policies.
//
...
// through the Device Control combined query AND carry the declared enablement —
// a policy that exists but is switched off enforces nothing, so removable media
// is unrestricted on every host it targets.
...
registerHealthCheckContract({
  ...
  handler: healthCheck,
  probePath: '/policy/queries/device-control/v1',
  scopePattern: /Device control policies: Read/,
})
...
test('usb-device-control-policies healthCheck: passes when every declared policy is present and enabled', async () => {
  ...
  try {
    ...
    const check = result.checks.find((c) => c.name === 'policy:Corp Windows USB Control')
...
test('usb-device-control-policies healthCheck: fails when a declared policy has been deleted in the tenant', async () => {
...
test('usb-device-control-policies healthCheck: fails a policy that exists but is switched off', async () => {
...
test('usb-device-control-policies healthCheck: does not look for policies when the tenant is unreachable', async () => {
  ...
  try {
    ...
    assert.equal(
      calls.filter((c) => c.url.includes('/policy/combined/device-control/v1')).length,
      0,
...
test('usb-device-control-policies healthCheck: reports a failed per-policy lookup as failed, not as absent', async () => {
