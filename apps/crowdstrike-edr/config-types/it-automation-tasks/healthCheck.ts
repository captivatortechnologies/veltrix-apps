...
import { extractITTaskSpecs } from './validate'

/**
 * Health check for IT automation task configuration:
 *   1. Falcon API reachability + credential validity (IT automation scope)
 *   2. Every declared task exists in the tenant
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  const reachable = await timedCheck('falcon_reachable', async () => {
    const res = await client.request('GET', IT_TASK_ENDPOINTS.queries, { query: { limit: 1 } })
    if (res.status === 401) throw new Error('Falcon API client rejected (401) — check the client secret')
    if (res.status === 403) {
      throw new Error('Falcon API client lacks the "IT automation tasks: Read" scope (403)')
    }
  ...
  if (reachable.passed) {
    ...
    for (const spec of specs) {
      checks.push(
        await timedCheck(`task:${spec.name}`, async () => {
          const live = await findEntityByIdentity(client, IT_TASK_ENDPOINTS, spec.name)
          if (!live) {
            throw new Error(`Task "${spec.name}" does not exist in the tenant`)
          }
          return `Task "${spec.name}" is present`
        }),
      )
    }
  }
