...
import { ROLLBACK_COMMENT } from './exclusionShared'
import { ML_EXCLUSION_ENDPOINTS, type MlExclusionRollbackEntry } from './deploy'

/**
 * Roll back ML exclusions using the state captured during deploy:
...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  const previousState = (ctx.rollbackData as { previousState?: MlExclusionRollbackEntry[] })
    ?.previousState
  ...
  try {
    for (const entry of previousState) {
      if (!entry.existed) {
        ...
        const live = await findExclusion(client, ML_EXCLUSION_ENDPOINTS, entry.value)
        if (live?.id) {
          await deleteExclusion(client, ML_EXCLUSION_ENDPOINTS, live.id, ROLLBACK_COMMENT)
        }
        ...
        await updateExclusion(client, ML_EXCLUSION_ENDPOINTS, {
          id: entry.id,
          value: entry.value,
          excluded_from: entry.prior.excludedFrom,
    ...
    return {
      success: true,
      message: `Rolled back ${reverted.length} ML exclusion(s): ${reverted.join(', ')}`,
    }
  } catch (error) {
    return {
      success: false,
