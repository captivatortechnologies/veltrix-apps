# 🛡️ Bitdefender GravityZone — Request Flows
...
**App:** `bitdefender-gravityzone` · **Category:** EDR · **Version:** 0.1.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Bitdefender GravityZone API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 9 config types · 6 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Bitdefender GravityZone API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + network-groupswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + network-groupsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: network-groupswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
