...
import { listConnectors } from './deploy'
import { connectorKey, extractConnectorSpecs, type LiveConnector } from './_shared'

/**
 * Health check for Connector configuration:
 *   1. Twingate GraphQL reachability + API key validity (a connector list)
 *   2. Every declared connector (by name) still exists
 * Score is the percentage of passed checks (0-100). This does not check the
 * Connector's operational `state` (ALIVE/DEAD_*) — that reflects whether the
 * deployed Connector process is running, not this app's declared config.
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable.passed && reachable.live) {
    const names = new Set(reachable.live.filter((c) => c.name).map((c) => connectorKey(c.name as string)))
    for (const spec of specs) {
      const present = names.has(connectorKey(spec.name))
      checks.push({
        name: `connector:${spec.name}`,
        passed: present,
        message: present ? `Connector "${spec.name}" is present` : `Connector "${spec.name}" is missing`,
      })
    }
  }
...
async function timedCheck(
  name: string,
  fn: () => Promise<{ message: string; live?: LiveConnector[] }>,
): Promise<{ name: string; passed: boolean; message: string; latencyMs: number; live?: LiveConnector[] }> {
