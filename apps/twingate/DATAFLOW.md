...
**App:** `twingate` · **Category:** NETWORK · **Version:** 0.1.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Twingate API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Twingate API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + resourceswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + resourcesread
  rbac-->>pipeline: permitted ✓ / denied
