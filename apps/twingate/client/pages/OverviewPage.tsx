...
/**
 * Shows what this app manages in a Twingate network, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/twingate/meta')
  ...
  if (loading) return <Spinner label="Loading Twingate app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Twingate (Zero Trust Network Access) configuration as code through the Twingate GraphQL
          API. Create a configuration in the Configuration Canvas and deploy it through the pipeline —
          validate, deploy, health check, drift detection and rollback are all handled per configuration
          type. The app authenticates with a Twingate API key against your network's GraphQL endpoint.
        </p>
