// ========================================================================
// Twingate App — Server Entry Module
//
// Registers Twingate-specific API routes as a Fastify plugin, prefixed with
// /api/apps/twingate/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration
// Canvas, and every write goes through the pipeline handlers (Twingate
// GraphQL API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('resources', 'read')],
