...
**App:** `onelogin` · **Category:** IAM · **Version:** 0.1.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **OneLogin API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as OneLogin API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + appswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + appsread
  rbac-->>pipeline: permitted ✓ / denied
