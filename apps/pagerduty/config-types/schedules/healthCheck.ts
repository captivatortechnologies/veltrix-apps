...
import { extractScheduleSpecs, findSchedule } from './_shared'
import { listSchedules } from './deploy'

/**
 * Health check for schedules configuration:
 *   1. PagerDuty API reachability + auth (GET /abilities answers 2xx with the key)
 *   2. every declared schedule (by name) still exists in the account
    ...
    if (specs.length > 0) {
      try {
        ...
        for (const spec of specs) {
          const present = Boolean(findSchedule(live, spec.name))
          checks.push({
            name: `schedule:${spec.name}`,
            passed: present,
            message: present ? `Schedule "${spec.name}" is present.` : `Schedule "${spec.name}" is missing.`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'schedules_readable',
          passed: false,
          message: `Could not list schedules: ${error instanceof Error ? error.message : 'error'}`,
        })
      }
    }
  }
