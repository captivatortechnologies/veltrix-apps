...
/**
 * Health check for users configuration:
 *   1. PagerDuty API reachability + auth (GET /abilities answers 2xx with the key)
 *   2. every declared user (by email) still exists in the account
    ...
    if (specs.length > 0) {
      try {
        ...
        for (const spec of specs) {
          const present = Boolean(findUser(live, spec.email))
          checks.push({
        ...
        checks.push({
          name: 'users_readable',
          passed: false,
          message: `Could not list users: ${error instanceof Error ? error.message : 'error'}`,
        })
      }
    }
  }
