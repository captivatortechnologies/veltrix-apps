...
/**
 * Health check for services configuration:
 *   1. PagerDuty API reachability + auth (GET /abilities answers 2xx with the key)
 *   2. every declared service (by name) still exists in the account
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable) {
    ...
    if (specs.length > 0) {
      try {
        const live = await listServices(client)
        for (const spec of specs) {
          const present = Boolean(findService(live, spec.name))
          checks.push({
            name: `service:${spec.name}`,
            passed: present,
            message: present ? `Service "${spec.name}" is present.` : `Service "${spec.name}" is missing.`,
          })
        }
      } catch (error) {
        checks.push({
