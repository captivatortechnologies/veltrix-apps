...
/**
 * Health check for business services configuration:
 *   1. PagerDuty API reachability + auth (GET /abilities answers 2xx with the key)
 *   2. every declared business service (by name) still exists in the account
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (reachable) {
    ...
    if (specs.length > 0) {
      try {
        const live = await listBusinessServices(client)
        for (const spec of specs) {
          const present = Boolean(findBusinessService(live, spec.name))
          checks.push({
            name: `business_service:${spec.name}`,
            passed: present,
            message: present
              ? `Business service "${spec.name}" is present.`
              : `Business service "${spec.name}" is missing.`,
          })
        }
      } catch (error) {
        checks.push({
          name: 'business_services_readable',
          passed: false,
          message: `Could not list business services: ${error instanceof Error ? error.message : 'error'}`,
        })
      }
    }
  }
