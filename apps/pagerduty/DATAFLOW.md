...
**App:** `pagerduty` · **Category:** SOAR · **Version:** 0.4.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **PagerDuty API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as PagerDuty API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + escalation-policieswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + escalation-policiesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: escalation-policieswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
