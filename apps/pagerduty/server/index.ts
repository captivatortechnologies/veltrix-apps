// ========================================================================
// PagerDuty App — Server Entry Module
//
// Registers PagerDuty-specific API routes as a Fastify plugin, prefixed with
// /api/apps/pagerduty/ and protected by app-level auth + permission middleware.
// Read-only on the platform side: authoring happens in the Configuration Canvas,
// and every write goes through the pipeline handlers (PagerDuty REST API v2).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('escalation-policies', 'read')],
