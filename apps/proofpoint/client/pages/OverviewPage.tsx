...
/**
 * Shows what this app manages in a Proofpoint Essentials organization, using the
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/proofpoint/meta')
  ...
  if (loading) return <Spinner label="Loading Proofpoint Essentials app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Proofpoint Essentials email security configuration as code through the Essentials
          Interface API (v1). Create a configuration in the Configuration Canvas and deploy it through
          the pipeline — validate, deploy, health check, drift detection and rollback are all handled
          per configuration type. Objects are scoped to the organization named in the Organization
