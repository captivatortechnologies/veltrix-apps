# 📧 Proofpoint Essentials — Request Flows
...
**App:** `proofpoint` · **Category:** NETWORK · **Version:** 1.1.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Proofpoint Essentials API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Proofpoint Essentials API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + pp-domainswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + pp-domainsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: pp-domainswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
