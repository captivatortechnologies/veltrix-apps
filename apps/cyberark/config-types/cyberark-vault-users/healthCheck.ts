...
/**
 * Health check for Vault-user configuration:
 *   1. PVWA reachability + logon (a /Users list)
...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  let byKey: Map<string, LiveVaultUser> | null = null
  try {
    byKey = await mapUsers(client)
  ...
  if (byKey) {
    for (const spec of extractVaultUserSpecs(ctx.canvas).filter((s) => s.username)) {
      const present = byKey.has(usernameKey(spec))
      checks.push({
