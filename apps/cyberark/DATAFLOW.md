# 🔐 CyberArk Privileged Access Manager — Request Flows
...
**App:** `cyberark` · **Category:** IAM · **Version:** 1.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **CyberArk Privileged Access Manager API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 12 config types · 7 API families.</sub>
...
sequenceDiagram
  ...
  participant api as CyberArk Privileged Access Manager API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + cyberark-safeswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + cyberark-safesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: cyberark-safeswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
