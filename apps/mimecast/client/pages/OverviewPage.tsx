...
/**
 * Shows what this app manages in Mimecast, using the platform design-system
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/mimecast/meta')
  ...
  if (loading) return <Spinner label="Loading Mimecast app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Mimecast configuration as code through the Mimecast API. Create a configuration in
          the Configuration Canvas and deploy it through the pipeline — validate, deploy, health
          check, drift detection, and rollback are all handled per configuration type. Managed URLs
          have no update API, so a change is applied as delete + recreate, and reconcile only ever
