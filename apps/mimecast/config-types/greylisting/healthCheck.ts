...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  checks.push({
    name: 'mimecast-greylisting',
    passed,
    message: passed ? 'Reached the Mimecast greylisting policy endpoint' : `Mimecast error: ${resp.error ?? v1ErrorMessage(resp.body, resp.status)}`,
    latencyMs,
  })
