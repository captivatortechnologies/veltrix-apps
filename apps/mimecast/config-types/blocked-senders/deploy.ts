...
import { extractBlockedSenderSpecs, type BlockedSenderSpec, type LivePolicy } from './validate'

const CREATE = '/api/policy/blockedsenders/create-policy'
const GET_ALL = '/api/policy/blockedsenders/get-policy'
const DELETE = '/api/policy/blockedsenders/delete-policy'
...
export function buildPayload(spec: BlockedSenderSpec): Record<string, unknown> {
  return {
    option: spec.option,
...
export function snapshotLive(live: LivePolicy): Record<string, unknown> {
  return {
    option: live.option ?? 'block_sender',
...
export function definitionEquals(live: LivePolicy, spec: BlockedSenderSpec): boolean {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractBlockedSenderSpecs(ctx.canvas).filter((s) => s.description)

  const listed = await client.request(GET_ALL, {})
  if (!listed.ok) return { success: false, message: `Failed to list blocked sender policies: ${mimecastErrorMessage(listed)}` }
  ...
  if (failures.length) {
    return { success: false, message: `Some blocked sender policies failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} blocked sender policy(ies)`, rollbackData: { entries } }
}
