...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (!cred) {
    checks.push({
      name: 'credential',
      passed: false,
      message: 'No usable Mimecast credential configured',
    })
  ...
  checks.push({
    name: 'mimecast-managed-urls',
    passed,
    message: passed ? 'Reached the Mimecast managed-URL endpoint' : `Mimecast error: ${mimecastErrorMessage(resp)}`,
    latencyMs,
  })
