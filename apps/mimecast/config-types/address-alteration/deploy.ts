...
import { extractAddressAlterationSpecs, type AddressAlterationSpec, type LivePolicy } from './validate'

const CREATE = '/api/policy/address-alteration/create-policy'
const GET_ALL = '/api/policy/address-alteration/get-policy'
const DELETE = '/api/policy/address-alteration/delete-policy'
...
export function buildPayload(spec: AddressAlterationSpec): Record<string, unknown> {
  return {
    addressAlterationSetId: spec.addressAlterationSetId,
    policy: {
      description: spec.description,
      fromPart: spec.fromPart,
      from: buildBlock(spec.fromType, spec.fromValue),
      to: buildBlock(spec.toType, spec.toValue),
...
export function snapshotLive(live: LivePolicy): Record<string, unknown> {
  return {
    addressAlterationSetId: live.addressAlterationSetId ?? '',
    policy: {
      description: live.policy?.description ?? '',
      fromPart: live.policy?.fromPart ?? 'envelope_from',
      from: live.policy?.from ?? { type: 'everyone' },
      to: live.policy?.to ?? { type: 'everyone' },
      enabled: live.policy?.enabled ?? true,
    },
  }
}

/** Whether a live policy already equals the desired spec (description matches). */
export function definitionEquals(live: LivePolicy, spec: AddressAlterationSpec): boolean {
  if ((live.addressAlterationSetId ?? '') !== spec.addressAlterationSetId) return false
  if ((live.policy?.fromPart ?? 'envelope_from') !== spec.fromPart) return false
  if ((live.policy?.enabled ?? true) !== spec.enabled) return false
  ...
  return blockValue(live.policy?.to) === blockValue(buildBlock(spec.toType, spec.toValue) as { type?: string; emailAddress?: string; emailDomain?: string })
}
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractAddressAlterationSpecs(ctx.canvas).filter((s) => s.description)

  const listed = await client.request(GET_ALL, {})
  if (!listed.ok) return { success: false, message: `Failed to list address alteration policies: ${mimecastErrorMessage(listed)}` }
  ...
  if (failures.length) {
    return { success: false, message: `Some address alteration policies failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} address alteration policy(ies)`, rollbackData: { entries } }
}
