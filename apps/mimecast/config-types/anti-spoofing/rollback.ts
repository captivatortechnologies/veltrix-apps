...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    if (e.existed && e.prior && e.id) {
      // The policy predates this app's management — PATCH it back to the
      // pre-management snapshot rather than delete + recreate.
  ...
  return { success: true, message: `Rolled back anti-spoofing policies: ${deleted} deleted, ${restored} restored` }
}
