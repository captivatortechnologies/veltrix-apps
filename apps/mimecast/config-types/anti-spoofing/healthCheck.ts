...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  checks.push({
    name: 'mimecast-anti-spoofing',
    passed,
    message: passed ? 'Reached the Mimecast anti-spoofing policy endpoint' : `Mimecast error: ${resp.error ?? v1ErrorMessage(resp.body, resp.status)}`,
    latencyMs,
  })
