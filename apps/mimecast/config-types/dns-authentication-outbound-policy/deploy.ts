...
import { extractDnsAuthOutboundPolicySpecs, type DnsAuthOutboundPolicySpec, type LiveDnsAuthOutboundPolicy } from './validate'
...
export function buildPayload(spec: DnsAuthOutboundPolicySpec): Record<string, unknown> {
  return {
...
export function snapshotLive(live: LiveDnsAuthOutboundPolicy): Record<string, unknown> {
  return {
...
export function definitionEquals(live: LiveDnsAuthOutboundPolicy, spec: DnsAuthOutboundPolicySpec): boolean {
...
export default async function deploy(ctx: DeployContext): Promise<DeployResult> {
  ...
  const specs = extractDnsAuthOutboundPolicySpecs(ctx.canvas).filter((s) => s.description && s.definitionId)

  const listed = await client.requestV1('GET', LIST, { query: { pageSize: 100 } })
  if (!listed.ok)
    return { success: false, message: `Failed to list DNS Authentication - Outbound policies: ${listed.error ?? v1ErrorMessage(listed.body, listed.status)}` }
  ...
  if (failures.length) {
    return { success: false, message: `Some DNS Authentication - Outbound policies failed: ${failures.join('; ')}`, rollbackData: { entries } }
  }
  return { success: true, message: `Deployed ${entries.length} DNS Authentication - Outbound policy(ies)`, rollbackData: { entries } }
}
