...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  checks.push({
    name: 'mimecast-dns-authentication-outbound-policy',
    passed,
    message: passed ? 'Reached the Mimecast DNS Authentication - Outbound policy endpoint' : `Mimecast error: ${resp.error ?? v1ErrorMessage(resp.body, resp.status)}`,
    latencyMs,
  })
