...
import { extractDeliveryRoutePolicySpecs, type LiveDeliveryRoutePolicy } from './validate'
...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  ...
  const specs = extractDeliveryRoutePolicySpecs(ctx.deployedConfig).filter((s) => s.description && s.definitionId)
