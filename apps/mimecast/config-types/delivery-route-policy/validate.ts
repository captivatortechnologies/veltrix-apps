...
// --- Mimecast Delivery Route policy constraints (Policy Management v1) ------

export const FROM_PARTS = ['envelope_from', 'header_from', 'both'] as const

export interface DeliveryRoutePolicySpec {
  itemId?: string
  /** description — the policy identity. */
  description: string
  /** the secure id of the Delivery Route Definition this policy routes to. */
  definitionId: string
  fromPart: string
  fromType: string
  fromValue: string
...
/** A delivery route policy as returned by the v1 API. */
export interface LiveDeliveryRoutePolicy {
  id?: string
  description?: string
  definitionId?: string
  fromPart?: string
...
export function extractDeliveryRoutePolicySpecs(canvas: CanvasSnapshot): DeliveryRoutePolicySpec[] {
...
export default function validate(ctx: PipelineContext): ValidationResult {
  ...
  const specs = extractDeliveryRoutePolicySpecs(ctx.canvas)
  ...
  specs.forEach((spec, i) => {
    ...
    if (!spec.definitionId) {
      errors.push({ field: `${prefix}.definitionId`, message: 'Delivery Route Definition ID is required (the definition this policy routes to)', code: 'required' })
    }
