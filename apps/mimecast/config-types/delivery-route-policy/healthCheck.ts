...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  checks.push({
    name: 'mimecast-delivery-route-policy',
    passed,
    message: passed ? 'Reached the Mimecast delivery route policy endpoint' : `Mimecast error: ${resp.error ?? v1ErrorMessage(resp.body, resp.status)}`,
    latencyMs,
  })
