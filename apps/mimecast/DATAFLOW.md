...
**App:** `mimecast` · **Category:** COMPLIANCE · **Version:** 0.6.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Mimecast API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 14 config types · 1 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Mimecast API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + managed-urlswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + managed-urlsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: managed-urlswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
