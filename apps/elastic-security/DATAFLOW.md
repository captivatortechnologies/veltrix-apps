# 🔎 Elastic Security — Request Flows
...
**App:** `elastic-security` · **Category:** SIEM · **Version:** 1.2.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Elastic Security API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
sequenceDiagram
  ...
  participant api as Elastic Security API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + detection-ruleswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + detection-rulesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: detection-ruleswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
