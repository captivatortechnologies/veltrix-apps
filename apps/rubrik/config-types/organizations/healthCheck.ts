...
/**
 * Health for the Organizations config = the Rubrik cluster answers with the
 * service-account session AND the internal organization endpoint is readable.
 * Read-only:
 *   1. open a service-account session (proves auth)
 *   2. GET /api/internal/organization (proves the endpoint this config type writes is reachable)
 */
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  try {
    ...
    try {
      await getJson<unknown>(conn, '/api/internal/organization', 8000)
      checks.push({ name: 'organization_readable', passed: true, message: 'Organization endpoint reachable (GET /api/internal/organization).', latencyMs: Date.now() - epStarted })
    } catch (error) {
      checks.push({ name: 'organization_readable', passed: false, message: `Organization query failed: ${error instanceof Error ? error.message : 'error'}`, latencyMs: Date.now() - epStarted })
    }
  } catch (error) {
