...
/** Deployment status for a Global Cluster Settings configuration, from platform records. */
export default async function getStatus(ctx: PipelineContext): Promise<ConfigStatus> {
  ...
  const components = await platform.listComponents({ types: ['rubrik-cluster'] })
