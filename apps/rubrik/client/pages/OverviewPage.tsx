...
/**
 * What this app manages in a Rubrik cluster, rendered with the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/rubrik/meta')
  ...
  if (loading) return <Spinner label="Loading Rubrik app details…" />
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Rubrik (backup, ransomware recovery and data security) configuration as code through
          the Rubrik CDM v2 REST API. Author an SLA Domain in the Configuration Canvas and deploy it
          through the pipeline — validate, deploy, health check, drift detection and rollback are handled
          per configuration type. Changes are applied over HTTPS with a service-account session
          (self-signed certificates tolerated).
        </p>
