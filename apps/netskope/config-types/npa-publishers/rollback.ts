...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const resp = await client.patch(`${BASE}/${e.id}`, { name: e.prior.name, lbrokerconnect: e.prior.lbrokerconnect })
  ...
  return { success: true, message: `Rolled back NPA publishers: ${deleted} deleted, ${restored} restored` }
}
