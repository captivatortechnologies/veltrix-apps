...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      ...
      if (e.prior.color) body.color = e.prior.color
      const resp = await client.patch(`${BASE}/${e.id}`, body)
  ...
  return { success: true, message: `Rolled back RBAC labels: ${deleted} deleted, ${restored} restored` }
}
