...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  if (!cred) {
    checks.push({
      name: 'credential',
      passed: false,
      message: 'No usable Netskope credential / tenant configured',
    })
  ...
  checks.push({
    name: 'netskope-urllist',
    passed,
    message: passed ? 'Reached the Netskope URL list endpoint' : `Netskope error: ${netskopeErrorMessage(resp)}`,
    latencyMs,
  })
