...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const resp = await client.patch(`${BASE}/${e.id}`, {
        ...
        host: e.prior.host,
        ports: e.prior.ports,
        ai_provider_ids: e.prior.ai_provider_ids,
        mcp_server_ids: e.prior.mcp_server_ids,
        sku_addons: e.prior.sku_addons,
      })
  ...
  return { success: true, message: `Rolled back AI Gateway appliances: ${deleted} deleted, ${restored} restored` }
}
