...
export default async function healthCheck(ctx: HealthCheckContext): Promise<HealthCheckResult> {
  ...
  checks.push({
    name: 'netskope-publisher-upgrade-profiles',
    passed,
    message: passed ? 'Reached the Netskope publisher upgrade profiles endpoint' : `Netskope error: ${netskopeErrorMessage(resp)}`,
    latencyMs,
  })
