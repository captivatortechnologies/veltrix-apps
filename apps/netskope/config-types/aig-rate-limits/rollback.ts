...
export default async function rollback(ctx: RollbackContext): Promise<RollbackResult> {
  ...
  for (const e of entries) {
    ...
    if (e.existed && e.prior) {
      const resp = await client.put(`${BASE}/${e.id}`, {
        name: e.prior.name,
        criteria: e.prior.criteria,
        limit: e.prior.limit,
        appliance_ids: e.prior.appliance_ids,
        response: e.prior.response,
      })
  ...
  return { success: true, message: `Rolled back rate-limit rules: ${deleted} deleted, ${restored} restored` }
}
