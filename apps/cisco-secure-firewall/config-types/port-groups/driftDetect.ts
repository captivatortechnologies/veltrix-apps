...
export default async function driftDetect(ctx: DriftContext): Promise<DriftResult> {
  const specs = extractPortGroupSpecs(ctx.deployedConfig).filter((s) => s.name)
  ...
  if (!listed.ok) {
    return {
      hasDrift: true,
      diffs: [{ field: 'port-groups', expected: 'reachable', actual: `list failed (HTTP ${listed.status})`, severity: 'critical' }],
    }
  }
