...
// Verified against CiscoDevNet/terraform-provider-fmc's gen/definitions/network_group.yaml `rest_endpoint`.
export const NETWORK_GROUPS_PATH = '/object/networkgroups'
...
/** The base fields deploy always sends; `objects` (resolved member refs) is added separately once refs resolve. */
export function buildNetworkGroupBaseFields(spec: NetworkGroupSpec): Record<string, unknown> {
  const fields: Record<string, unknown> = { overridable: spec.overridable }
  if (spec.description) fields.description = spec.description
  return fields
}

/** Validate network groups: a valid name and at least one member are required; names are unique. */
export default async function validate(ctx: PipelineContext): Promise<ValidationResult> {
  ...
  for (const spec of extractNetworkGroupSpecs(ctx.canvas)) {
