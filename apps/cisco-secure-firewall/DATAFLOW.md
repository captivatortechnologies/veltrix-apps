# 🔥 Cisco Secure Firewall (FMC) — Request Flows
...
**App:** `cisco-secure-firewall` · **Category:** NETWORK · **Version:** 0.1.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Cisco Secure Firewall (FMC) API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 9 config types · 2 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Cisco Secure Firewall (FMC) API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + access-control-policieswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + access-control-policiesread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: access-control-policieswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
