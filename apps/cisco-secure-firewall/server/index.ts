// ========================================================================
// Cisco Secure Firewall (FMC) App - Server Entry Module
//
// Registers app-specific API routes as a Fastify plugin, prefixed with
// /api/apps/cisco-secure-firewall/ and protected by app-level auth +
// permission middleware. Read-only on the platform side: authoring happens
// in the Configuration Canvas, and every write to FMC goes through the
// pipeline handlers (the FMC REST API).
// ========================================================================
...
export default async function registerRoutes(fastify: FastifyInstance, ctx: AppRouteContext) {
  ...
  fastify.get('/meta', {
    preHandler: [hasPermission('access-control-policies', 'read')],
