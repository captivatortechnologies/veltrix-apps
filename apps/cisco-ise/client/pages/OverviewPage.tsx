...
/**
 * Shows what this app manages on a Cisco ISE deployment, using the platform
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    authFetch('/api/apps/cisco-ise/meta')
  ...
  if (loading) return <Spinner label="Loading Cisco ISE app details…" />
  if (error) {
  ...
  return (
    <Card>
      <CardBody>
        <p>
          Manages Cisco Identity Services Engine (ISE) — network access control — configuration as code
          through the ISE External RESTful Services (ERS) API. Create a configuration in the Configuration
          Canvas and deploy it through the pipeline — validate, deploy, health check, drift detection and
          rollback are all handled per configuration type. The app authenticates as an ISE administrator
          (HTTP Basic) against the PAN/admin node's dedicated ERS port (9060).
        </p>
