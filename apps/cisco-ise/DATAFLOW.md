...
**App:** `cisco-ise` · **Category:** NETWORK · **Version:** 0.3.0  
**Operations:** Deploy a configuration · Detect drift · Roll back · Test connection  
Talks to **Cisco ISE API** · credentials via the Credential Vault (`ctx.resolveConnection`).
...
<sub>Applies to: 11 config types · 4 API families.</sub>
...
sequenceDiagram
  ...
  participant api as Cisco ISE API
  ...
  Note over rbac: configuration-canvasread · componentread · credentialread + endpoint-identity-groupswrite
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: componentread + endpoint-identity-groupsread
  rbac-->>pipeline: permitted ✓ / denied
...
sequenceDiagram
  ...
  pipeline->>rbac: authorize actor (RBAC)
  Note over rbac: endpoint-identity-groupswrite
  rbac-->>pipeline: permitted ✓ / denied
  pipeline->>approval: request approval
  Note over approval: required for production · human-in-the-loop
  approval-->>pipeline: approved by a human — AI cannot self-approve
